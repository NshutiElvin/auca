...
class RealTimeExamDataConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        user = self.scope["user"]
        if user.is_authenticated:
            self.user_group = f"user_{user.id}_exams"
            self.broadcast_group = "broadcast_exams"
    ...
    async def send_exam_data(self, event):
        message = event["message"]
        await self.send(text_data=json.dumps( message))
