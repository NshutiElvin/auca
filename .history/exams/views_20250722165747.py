...
class ExamViewSet(viewsets.ModelViewSet):
    ...
    def add_new_exam(self, request):
        ...
        try:
            with transaction.atomic():
                # conflicts= verify_groups_compatiblity()

                existing_slot= request.data.get("slot")
                date= request.data.get("day")
                new_group_to_add= request.data.get("course_group")
                merged_groups=[*[group["group"]["id"] for group in existing_slot["exams"]], *[group["group"]["id"] for group in new_group_to_add["groups"]]]
                print(merged_groups)
                print(existing_slot)
                print(date)
                print(new_group_to_add)
                time.sleep(10)
            return Response({
                'success': True,
                'message': 'All exams and student exam assignments have been truncated successfully.'
