...
class ExamViewSet(viewsets.ModelViewSet):
    ...
    def generate_exam_schedule_view(self, request):
            ...
            print(unscheduled_courses[0])
            for unscheduled_ in unscheduled_courses[0]:
                unscheduled_course=unScheduled_["course"]
                group=unScheduled_["groups"]
                c={}
                course=Course.objects.get(id=unscheduled_course)
                courseSerializer=CourseSerializer(enrollement.course)
                c["course"]=courseSerializer.data
                c["group"]=[]
                for g in group:
                    enrollement=Enrollment.objects.filter(course=course, group_id=g)
                    
                    courseGroupSerializer=CourseGroupSerializer(enrollement.group)
                    c["group"].append(courseGroupSerializer.data)
                unScheduled.append(c)
            print(unScheduled)

            courses_with_their_groups= Course.objects.filter(course_id__in=courses).iterator()
            return Response({
                'success': True,
