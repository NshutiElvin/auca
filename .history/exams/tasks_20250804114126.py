...
def check_and_update_exams():
    ...
    exams_today = Exam.objects.filter(date=today).all()
    print(exams_today)

    for exam in exams_today:
        print(exam.date + exam.start_time)
        
        start_time = make_aware(datetime.combine(exam.date, exam.start_time))
        ...
        if 0 < (start_time - now).total_seconds() <= 900 and exam.status != 'READY':
            exam.status = 'READY'
            exam.save()
            _notify_students(exam,  f"Your exam of {exam.group.course.title} that is scheduled at {exam.date} {exam.start_time}-{exam.end_time} will start soon.")
        ...
        # If exam has already started and not marked completed
            ...
            exam.save()
            _notify_students(exam, f"Your exam of {exam.group.course.title} that was scheduled on {exam.date} {exam.start_time}-{exam.end_time} has completed.")
