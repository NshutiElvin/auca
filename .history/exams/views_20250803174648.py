...
class ExamViewSet(viewsets.ModelViewSet):
        ...
        return Response(
            {
                ...
                "statud": MasterTimetableExam.objects.filter(exam=self.queryset[0]).first().master_timetable.status,
    ...
    # @action (detail=False, methods=["GET"], url_path="master_timetables")
    ...
    def verify(self, request, *args, **kwargs):
        try:
            ...
            student_enrollment = Enrollment.objects.filter(
                student_id=student.id, course_id=int(course_id)
            ).first()
