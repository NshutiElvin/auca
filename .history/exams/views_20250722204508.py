...
class ExamViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['delete'], url_path='remove-scheduled-exam', permission_classes=[permissions.IsAuthenticated])
    def remove_exam(self, request):
        try:
            ...
            return Response({
                'success': True,
                'message': 'exam have been deleted successfully.'
            }, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({
                'success': False,
