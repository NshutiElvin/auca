...
class Exam(TimeStampedModel):
    ...
    group= models.ForeignKey("courses.Course", on_delete=models.DO_NOTHING, null=True)

    class Meta:
        unique_together = ('course', 'date')
    def __str__(self):
        return f"{self.course.code} - {self.date} - {self.status}"
...
class StudentExam(TimeStampedModel):
    ...
    def __str__(self):
        return f"{self.student.reg_no} - {self.exam.course.code} - {self.status}"
