...
from schedules.utils import ExamScheduler, generate_exam_schedule # cancel_exam, reschedule_exam
...
class ExamViewSet(viewsets.ModelViewSet):
    ...
    def generate_exam_schedule_view(self, request):
            ...
            if result['success']:
                print(f"Successfully scheduled {len(result['exams'])} exams")
                for exam in result['exams']:
                    print(f"{exam.group.course.title} ({exam.group.group_name}): "
                        f"{exam.date} {exam.start_time}-{exam.end_time}")
                
                if result['conflicts']:
                    print(f"\n{len(result['conflicts'])} unresolved conflicts")
                    print(result["conflicts"])
            else:
                print(f"Failed to generate schedule: {result['error']}")
            # exams,_ = generate_exam_schedule(start_date=start_date,end_date=end_date, course_ids=course_ids, semester=None)
            # queryset = self.filter_queryset(self.get_queryset())
            # serializer = self.get_serializer(queryset, many=True)
            return Response({
                'success': True,
                'message': f'  exams scheduled successfully.',
