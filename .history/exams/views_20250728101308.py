...
from schedules.utils import   generate_exam_schedule, get_slot_name , verify_groups_compatiblity, which_suitable_slot_to_schedule_course_group
...
class ExamViewSet(viewsets.ModelViewSet):
    queryset = Exam.objects.select_related('group', 'room').all().order_by('date', 'start_time')
            ...
            with transaction.atomic():
                ...
                for conf in conflicts:
                    ...
                    if conf[0] in new_groups:
                        ...
                        g1_slot= Exam.objects.filter(group__id=conf[0], date=date_formatted).first()
                        g2_slot= Exam.objects.filter(group__id=conf[1], date=date_formatted).first()
                    ...
                    if conf[1] in new_groups:
                        ...
                        conflict_matrix.append((g1, g2,conf[2] ))
                        conflict_matrix.append((g1, g2,conf[2] ))
                new_group, best_suggestion, all_suggestions, all_conflicts=which_suitable_slot_to_schedule_course_group(date_formatted, new_groups, existing_slot.get("name"))
