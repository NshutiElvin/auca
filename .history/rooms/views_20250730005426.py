    ...
    def students(self, request):
        try:
           ...
           with transaction.atomic():
                ...
                existingRoom= Room.objects.filter(name=course_group["roomName"]).first()
                if not exam or not existingRoom:
                    return Response({"success": False, "message": "Invalid Room, existing romm and Course Group"}, status=status.HTTP_400_BAD_REQUEST)
                student_exams = StudentExam.objects.filter(exam=exam, room=existingRoom)
                if not student_exams.exists():
                    return Response({"success": False, "message": "No student exams found for the specified exam and room"}, status=status.HTTP_404_NOT_FOUND)
                students_exams_serialiser= StudentExamSerializer(student_exams, many=True)
                students_exams_data= students_exams_serialiser.data
                return Response({"success": True,"students": students_exams_data, "message": "Exam room changed successfully"}, status=status.HTTP_201_CREATED)
        ...
        try:
           ...
           with transaction.atomic():
                ...
                for student_exam in student_exams:
                    ...
                    student_exam.save()
                return Response({"success": True, "message": "Exam room changed successfully"}, status=status.HTTP_201_CREATED)
        except:
