...
def has_sufficient_gap(student_exam_dates, proposed_date, min_gap_days=1):
    """
    Check if the proposed exam date has at least `min_gap_days` spacing
    from all of the student's existing exam dates.
    """
    for existing_date in student_exam_dates:
        gap = abs((proposed_date - existing_date).days)
        if gap < min_gap_days:
            return False
    return True
