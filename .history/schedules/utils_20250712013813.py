...
def is_semester_compatible(course1_id, course2_id):
    """
    Check if two courses can be paired based on semester gap (at least one semester apart)
    """
    try:
        course1 = Course.objects.get(id=course1_id)
        course2 = Course.objects.get(id=course2_id)
        semester_gap = abs(int(course1.semester.name.split(" ")[1] )- int(course2.semester.name.split(" ")[1] ))
...
def find_compatible_courses(conflict_matrix):
    """
    Group courses into compatible groups that can be scheduled together
    Compatible means no shared students and at least one semester gap
    """
    all_courses = set()
    ...
    for course1 in all_courses:
        for course2 in all_courses:
            if course1 != course2:
                pair = tuple(sorted([course1, course2]))
                if (pair not in conflict_matrix or conflict_matrix[pair] == 0) and is_semester_compatible(course1, course2):
                    compatibility_graph[course1].add(course2)
    ...
    while remaining_courses:
        ...
        # Try to add exactly one more course for default pairing
...
def detect_scheduling_conflicts(course_pairs, student_assignments, date_slots):
    """
    Detect potential scheduling conflicts considering the day-off rule
    """
    num_pairs = len(course_pairs)
    num_slots = len(date_slots)
    ...
    for student_id, assignments in student_assignments.items():
        for pair_idx1, course_id1 in assignments:
            for pair_idx2, course_id2 in assignments:
                if pair_idx1 != pair_idx2:
                    # Check for consecutive day conflicts
                    for date, slot_indices in slots_by_date.items():
                        prev_date = date - timedelta(days=1)
                        next_date = date + timedelta(days=1)
                        adjacent_dates = [d for d in [prev_date, next_date] if d in slots_by_date]
                        
                        for slot_idx1 in slot_indices:
                            # Add conflicts for same-day scheduling
                            for slot_idx2 in slot_indices:
                                conflict_scores[pair_idx1][slot_idx1] += 1
                                conflict_scores[pair_idx2][slot_idx2] += 1
                            
                            # Add conflicts for adjacent days
                            for adj_date in adjacent_dates:
                                for slot_idx2 in slots_by_date[adj_date]:
                                    conflict_scores[pair_idx1][slot_idx1] += 1
                                    conflict_scores[pair_idx2][slot_idx2] += 1
...
def calculate_room_requirements(course_pairs):
    """
    Calculate student counts for each course pair
    """
    room_requirements = []
        ...
        for pair_idx, _ in pair_difficulties:
            ...
            for date in sorted_dates:
                ...
                for course_id in pair:
                    student_ids = Enrollment.objects.filter(course_id=course_id).values_list('student_id', flat=True)
                    for student_id in student_ids:
                        # Check for exams on same or adjacent days
                        prev_date = date - timedelta(days=1)
                        next_date = date + timedelta(days=1)
                        if (date in student_exam_dates[student_id] or 
                            prev_date in student_exam_dates[student_id] or 
                            next_date in student_exam_dates[student_id]):
                            has_date_conflict = True
                            break
                    if has_date_conflict:
                        break
...
def allocate_shared_rooms(exams):
    ...
    total_students = 0
    for exam in exams:
        enrolled_students = list(Enrollment.objects.filter(
            course=exam.course
    ...
    # Try to allocate exactly two exams per room
    rooms_needed = max(1, len(exams) // 2)
    if len(exams) % 2 == 1:
        rooms_needed += 1
    ...
    if len(exams) <= 2:
        ...
        # Try to pair exams in rooms (2 per room by default)
        exam_pairs = []
        used_exams = set()
        ...
        for exam1 in exams:
            ...
            # Find compatible exam for pairing
            for exam2 in exams:
                if exam2.id not in used_exams and is_semester_compatible(exam1.course.id, exam2.course.id):
                    exam_pair.append(exam2)
                    used_exams.add(exam2.id)
                    break
            
            exam_pairs.append(exam_pair)
        
        current_room_index = 0
        for exam_pair in exam_pairs:
            if current_room_index >= len(rooms):
                # Fallback: allocate more than 2 exams per room
                for student_exam in all_student_exams:
                    if student_exam.exam.id in [e.id for e in exam_pair]:
                        unaccommodated_students.append(student_exam.student)
                continue
            ...
            for exam in exam_pair:
                pair_students.extend([se for se in all_student_exams if se.exam.id == exam.id])
            
            # Allocate capacity equally between exams in the pair
            students_per_exam = room_capacity // len(exam_pair) if exam_pair else room_capacity
            allocated_counts = defaultdict(int)
            
            random.shuffle(pair_students)
            
            for student_exam in pair_students:
                exam_id = student_exam.exam.id
                if allocated_counts[exam_id] < students_per_exam and room_capacity > 0:
                    student_exam.room = room
                    allocated_counts[exam_id] += 1
                    room_capacity -= 1
                else:
                    unaccommodated_students.append(student_exam.student)
    ...
    StudentExam.objects.bulk_create([se for se in all_student_exams if se.room is not None])
    
    return unaccommodated_students

# Remaining functions (allocate_single_exam_rooms, cancel_exam, reschedule_exam, etc.) remain unchanged