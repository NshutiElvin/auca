...
def allocate_shared_rooms():
    """
    Bin packing-based exam allocation with semester constraints and optimal room utilization.
    """
    ...
    with transaction.atomic():
        # Data structure: {date: {slot: {room_id: [student_exams]}}}
        schedule = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
        unaccommodated_students = []

        # Group students by date and slot, then by exam
        ...
        for se in student_exams:
            ...
            for slot_name, slot_start, slot_end in SLOTS:
                if exam_start == slot_start and exam_end == slot_end:
                    date_slot_exams[exam_date][slot_name][se.exam].append(se)
                    break

        # Process each date and slot
        for exam_date, slots in date_slot_exams.items():
            for slot_name in ['Morning', 'Afternoon', 'Evening']:
                ...
                # Step 1: Sort exam groups by student count (descending)
                sorted_exams = sorted(exams_dict.items(), key=lambda x: -len(x[1]))
                
                # Track room usage for this slot
                room_usage = {room.id: 0 for room in rooms}
                remaining_exams = dict(sorted_exams)
                
                # Step 2: Primary Allocation (Semester-Constrained Bin Packing)
                allocated_exams = set()
                ...
                for room in rooms:
                    ...
                    available_capacity = room.capacity - room_usage[room.id]
                    if available_capacity <= 0:
                        continue
                    
                    # Try to find optimal pairing with semester constraints
                    best_allocation = find_best_semester_pairing(
                        remaining_exams, available_capacity
                    )
                    
                    if best_allocation:
                        exam1, students1, exam2, students2 = best_allocation
                        
                        # Allocate the paired exams
                        total_students = students1 + students2
                        schedule[exam_date][slot_name][room.id].extend(total_students)
                        room_usage[room.id] += len(total_students)
                        
                        # Update remaining exams
                        if len(students1) == len(remaining_exams[exam1]):
                            del remaining_exams[exam1]
                        else:
                            remaining_exams[exam1] = remaining_exams[exam1][len(students1):]
                        
                        if len(students2) == len(remaining_exams[exam2]):
                            del remaining_exams[exam2]
                        else:
                            remaining_exams[exam2] = remaining_exams[exam2][len(students2):]
                        
                        allocated_exams.add(room.id)
                        continue
                
                # Step 3: Half-Half Allocation (Fallback Pairing)
                for room in rooms:
                    if not remaining_exams:
                        break
                    
                    available_capacity = room.capacity - room_usage[room.id]
                    if available_capacity < 4:  # Minimum viable split
                        continue
                    
                    half_capacity = available_capacity // 2
                    best_half_allocation = find_best_half_half_pairing(
                        remaining_exams, half_capacity
                    )
                    
                    if best_half_allocation:
                        exam1, students1, exam2, students2 = best_half_allocation
                        ...
                        # Update remaining exams
                        update_remaining_exams(remaining_exams, exam1, students1)
                        update_remaining_exams(remaining_exams, exam2, students2)

                # Step 4: Single Exam Allocation (Unpaired Courses)
                for room in rooms:
                    if not remaining_exams:
                        break
                    ...
                    # Find largest exam that fits
                    best_exam = None
                    best_students = []
                    
                    for exam, students in remaining_exams.items():
                        if len(students) <= available_capacity:
                            if len(students) > len(best_students):
                                best_exam = exam
                                best_students = students
                    
                    if best_exam:
                        schedule[exam_date][slot_name][room.id].extend(best_students)
                        room_usage[room.id] += len(best_students)
                        del remaining_exams[best_exam]

                # Step 5: Mixed Allocation for Remaining Students
                all_remaining_students = []
                for exam, students in remaining_exams.items():
                    all_remaining_students.extend(students)
                
                # Try to fit remaining students into any available space
                for student_exam in all_remaining_students:
                    placed = False
                    for room in rooms:
                        available_capacity = room.capacity - room_usage[room.id]
                        if available_capacity > 0:
                            schedule[exam_date][slot_name][room.id].append(student_exam)
        ...
        for exam_date, slots in schedule.items():
...
def get_semester_number(exam):
    """Extract semester number from exam."""
    try:
...
def can_pair_semesters(exam1, exam2):
    """Check if two exams can be paired based on semester constraints."""
    sem1 = get_semester_number(exam1)
    sem2 = get_semester_number(exam2)
    return abs(sem1 - sem2) > 1  # Non-adjacent semesters


def find_best_semester_pairing(remaining_exams, available_capacity):
    """Find the best pairing of exams that respects semester constraints."""
    best_allocation = None
    best_utilization = 0
    
    exams_list = list(remaining_exams.items())
    
    for i in range(len(exams_list)):
        exam1, students1 = exams_list[i]
        
        for j in range(i + 1, len(exams_list)):
            exam2, students2 = exams_list[j]
            
            if not can_pair_semesters(exam1, exam2):
                continue
            
            # Try different allocation strategies
            max_students1 = min(len(students1), available_capacity)
            max_students2 = min(len(students2), available_capacity - max_students1)
            
            if max_students1 + max_students2 > best_utilization:
                best_utilization = max_students1 + max_students2
                best_allocation = (
                    exam1, students1[:max_students1],
                    exam2, students2[:max_students2]
                )
    
    return best_allocation


def find_best_half_half_pairing(remaining_exams, half_capacity):
    """Find the best half-half pairing when semester constraints can't be met."""
    best_allocation = None
    best_utilization = 0
    ...
    for i in range(len(exams_list)):
        ...
        for j in range(i + 1, len(exams_list)):
            ...
            # Allocate roughly half capacity to each
            alloc1 = min(len(students1), half_capacity)
            alloc2 = min(len(students2), half_capacity)
            
            total_utilization = alloc1 + alloc2
            
            if total_utilization > best_utilization:
                best_utilization = total_utilization
                best_allocation = (
                    exam1, students1[:alloc1],
                    exam2, students2[:alloc2]
                )
    
    return best_allocation


def update_remaining_exams(remaining_exams, exam, allocated_students):
    """Update the remaining exams dictionary after allocation."""
    if len(allocated_students) == len(remaining_exams[exam]):
        del remaining_exams[exam]
    else:
        remaining_exams[exam] = remaining_exams[exam][len(allocated_students):]
