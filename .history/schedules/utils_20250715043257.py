...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    with transaction.atomic():
        # Get course group information for each course
        course_groups = {}
        for course in Course.objects.all():
            course_groups[course.id] = course.group
        ...
        for pair_idx, _ in pair_difficulties:
            ...
            # Determine preferred time slots based on course groups
            group_preferences = []
            for course_id in pair:
                group = course_groups.get(course_id, 'C')  # Default to 'C' (mixed) if not found
                preference = GROUP_PREFERENCES.get(group, 'mixed')
                group_preferences.append(preference)
            
            # Determine overall preference for this pair
            if all(p == 'mostly morning' for p in group_preferences):
                preferred_slots = ['Morning']
            elif all(p == 'evening' for p in group_preferences):
                preferred_slots = ['Evening']
            elif 'mixed' in group_preferences:
                # If any course in pair is mixed, consider all slots
                preferred_slots = ['Morning', 'Afternoon', 'Evening']
            else:
                # Mixed preferences (some morning, some evening)
                preferred_slots = ['Morning', 'Afternoon', 'Evening']
            ...
            for date in sorted(slots_by_date.keys()):
                ...
                # Find available slot on this date that matches preference
                available_slots = [
                    (slot_idx, label, start, end) 
                    for slot_idx, label, start, end in slots_by_date[date]
                    if slot_idx not in assigned_slots and label in preferred_slots
                ]
                
                # For mixed groups, randomize the order of preferred slots
                if 'mixed' in group_preferences and available_slots:
            ...
            if best_slot is None:
                # If no preferred slot found, try any available slot
                for date in sorted(slots_by_date.keys()):
                    can_schedule, conflicts = can_schedule_on_date(pair, date, student_exam_dates)
                    if not can_schedule:
                        continue
