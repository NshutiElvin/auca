...
def allocate_shared_rooms(exams):
    ...
    with transaction.atomic():
        ...
        for date, student_exams in exams.items():
            ...
            for slot_name in ['morning', 'afternoon', 'evening']:
                ...
                for pair in exam_pairs:
                    ...
                    # Calculate available capacity (considering existing allocations)
                    current_usage = StudentExam.objects.filter(
                        room=room,
                        exam__date=date,
                        exam__start_time=SLOTS[0 if slot_name == 'morning' else 1 if slot_name == 'afternoon' else 2][1]
                    ).count()
                    available_capacity = room.capacity - current_usage
                    ...
                    for exam_se in exams_to_allocate:
                        ...
                        allocated = exam_students[:available_capacity]
                        overflow = exam_students[available_capacity:]
                ...
                while room_index < len(available_rooms) and remaining_exams:
                    room = available_rooms[room_index]
                    room_index += 1
                    room_usage[date][slot_name].add(room.id)
                        ...
                        for j in range(i+1, len(exam_ids)):
                            ...
                            try:
                                ...
                                if abs(sem1 - sem2) > 1:
                                    ...
                                    exam1_students = remaining_exams[exam1_id][:available_capacity//2]
                                    exam2_students = remaining_exams[exam2_id][:available_capacity//2]
                                    ...
                                    remaining_exams[exam1_id] = remaining_exams[exam1_id][available_capacity//2:]
                                    remaining_exams[exam2_id] = remaining_exams[exam2_id][available_capacity//2:]
                    ...
                    if not paired and exam_ids:
                        ...
                        exam_id = exam_ids[0]
                        students = remaining_exams[exam_id][:available_capacity]
                        ...
                        remaining_exams[exam_id] = remaining_exams[exam_id][available_capacity:]
        ...
        if unaccommodated_students:
            for date, student_exams in exams.items():
                for slot_name in ['morning', 'afternoon', 'evening']:
                    slot_start = SLOTS[0 if slot_name == 'morning' else 1 if slot_name == 'afternoon' else 2][1]
                    used_rooms = room_usage[date].get(slot_name, set())
                    available_rooms = [r for r in rooms if r.id not in used_rooms]
                    ...
                    for room in available_rooms:
                        ...
                        available_seats = room.capacity - current_usage
                        
                        if available_seats > 0 and unaccommodated_students:
                            # Get students for this slot
                            slot_students = [s for s in unaccommodated_students 
                                            if any(se.exam.start_time == slot_start 
                                                  for se in student_exams 
                                                  if se.student == s)]
                            
                            students_to_place = slot_students[:available_seats]
                            student_ids = [s.id for s in students_to_place]
                            exam_ids = [se.exam.id for se in student_exams 
                                      if se.student.id in student_ids 
                                      and se.exam.start_time == slot_start]
                            ...
                            unaccommodated_students = [s for s in unaccommodated_students 
                                                     if s not in students_to_place]
