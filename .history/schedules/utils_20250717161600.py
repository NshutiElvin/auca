                ...
                for course_id in course_group:
                    ...
                    if len( dates[:step+1]) >1:
                        for innerdate in dates[:step]:
                            print(innerdate)
                            can_schedule, conflicts = can_schedule_course_group_on_slot(
                                exam.course, innerdate, student_exam_dates
                            )
                            if can_schedule:
                                print(f"update from {exam.date} to ${innerdate}")
                                exam.date=innerdate
                                exam.save()
                                
                            else:
                                print("Cant'reschedule ", conflicts)
                                continue
...
def allocate_shared_rooms(exams):
    ...
    # Get rooms ordered by capacity (largest first)
    rooms = list(Room.objects.order_by('-capacity'))
    if not rooms:
        raise Exception("No rooms available for allocation.")

    # Preload all students for each exam
    exam_students = {
        exam.id: list(Enrollment.objects.filter(course=exam.course).select_related('student'))
        for exam in exams
    }

    # Create compatibility graph (weighted by semester distance and student count)
    compatibility_graph = []
    for i in range(len(exams)):
        for j in range(i+1, len(exams)):
            semester_distance = abs(int(exams[i].course.semester.name.split()[1]) - 
                                  int(exams[j].course.semester.name.split()[1]))
            if semester_distance > 1:  # Only consider compatible pairs
                total_students = len(exam_students[exams[i].id]) + len(exam_students[exams[j].id])
                # Higher weight = better compatibility (distance + student count)
                weight = semester_distance * 1000 + total_students
                compatibility_graph.append((weight, exams[i], exams[j]))

    # Sort pairs by compatibility weight (best pairs first)
    compatibility_graph.sort(reverse=True, key=lambda x: x[0])

    allocated_exams = set()
    assignments = []
    unaccommodated = []

    # Phase 1: Fill largest rooms with most compatible pairs
    for room in rooms:
        if not compatibility_graph:
            break

        # Find best fitting pair for this room
        best_pair_index = None
        best_fit = 0
        room_capacity = room.capacity
        half_capacity = room_capacity // 2

        for i, (weight, exam1, exam2) in enumerate(compatibility_graph):
            if exam1.id in allocated_exams or exam2.id in allocated_exams:
                continue

            exam1_students = len(exam_students[exam1.id])
            exam2_students = len(exam_students[exam2.id])
            
            # Calculate how well this pair fits the room
            fit_score = min(exam1_students, half_capacity) + min(exam2_students, half_capacity)
        ...
        if best_pair_index is not None:
            weight, exam1, exam2 = compatibility_graph.pop(best_pair_index)
            allocated_exams.update({exam1.id, exam2.id})
            
            # Allocate students up to half capacity for each exam
            for exam in [exam1, exam2]:
                students = exam_students[exam.id][:half_capacity]
                for enrollment in students:
                    assignments.append(StudentExam(
                        student=enrollment.student,
                        exam=exam,
                        room=room
                    ))
                exam_students[exam.id] = exam_students[exam.id][half_capacity:]

    # Phase 2: Assign remaining exams to remaining rooms as singles
    remaining_exams = [exam for exam in exams if exam.id not in allocated_exams]
    remaining_exams.sort(key=lambda e: -len(exam_students[e.id]))  # Largest groups first

    remaining_rooms = rooms[len(assignments)//2:]  # Only unused rooms
    ...
    for room in remaining_rooms:
        ...
        students = exam_students[exam.id][:room.capacity]
        for enrollment in students:
            assignments.append(StudentExam(
                student=enrollment.student,
                exam=exam,
        ...
        exam_students[exam.id] = exam_students[exam.id][room.capacity:]
        allocated_exams.add(exam.id)

    # Collect unaccommodated students
    for exam in exams:
        unaccommodated.extend(exam_students[exam.id])
