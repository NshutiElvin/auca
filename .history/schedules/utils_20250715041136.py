...
def find_compatible_courses(course_conflict_matrix):
    ...
    # Get group preferences for courses
    course_groups = {}
    for course in Course.objects.filter(id__in=all_courses):
        group = course.group  # Assuming Course model has a 'group' field
        course_groups[course.id] = group
    
    # Group compatible courses considering group preferences
    remaining_courses = set(all_courses)
    course_groups_list = []
    ...
    while remaining_courses:
        ...
        # Pick initial course considering group diversity
        if remaining_courses:
            # Prioritize courses from groups we haven't placed yet
            unplaced_groups = {course_groups[c] for c in remaining_courses} - group_prefs
            if unplaced_groups:
                # Pick from unplaced groups first
                initial_candidates = [
                    c for c in remaining_courses 
                    if course_groups[c] in unplaced_groups
                ]
                if initial_candidates:
                    course1 = random.choice(initial_candidates)
                else:
                    course1 = random.choice(list(remaining_courses))
            else:
                course1 = random.choice(list(remaining_courses))
            
            course_group.append(course1)
            group_prefs.add(course_groups[course1])
            remaining_courses.remove(course1)
            
            # Find compatible courses considering group preferences
            compatible_with_group = set(compatibility_graph[course1]) & remaining_courses
            
            # Sort compatible courses by:
            # 1. Group diversity (prefer different groups)
            # 2. Number of remaining compatible options (hard-to-schedule first)
            sorted_compatible = sorted(
                compatible_with_group,
                key=lambda c: (
                    -len([g for g in group_prefs if g == course_groups[c]]),  # Prefer different groups
                    len([rc for rc in compatibility_graph[c] if rc in remaining_courses])  # Hard-to-schedule first
                )
            )
            
            # Add courses to group (limit to 10)
            for course in sorted_compatible[:10 - len(course_group)]:
                if course in remaining_courses:
                    course_group.append(course)
                    group_prefs.add(course_groups[course])
                    remaining_courses.remove(course)
        
        if course_group:
            course_groups_list.append(course_group)
    
    return course_groups_list
def get_group_preferred_slots(group_name, date):
    """Get preferred slots for a group based on their preference"""
    ...
    if preference == "mostly morning":
        return [s for s in available_slots if s[0] == 'Morning']
    elif preference == "evening":
        return [s for s in available_slots if s[0] == 'Evening']
    else:  # mixed
...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    # Get group information for courses
    course_group_map = {
        course.id: course.group
        for course in Course.objects.filter(id__in=[c for pair in course_pairs for c in pair])
    }
    
    estimated_slots_needed = len(course_pairs) * 20
    ...
    with transaction.atomic():
        ...
        for pair_idx, _ in pair_difficulties:
            ...
            # Determine primary group for this pair (most common group)
            pair_groups = [course_group_map[c] for c in pair]
            primary_group = max(set(pair_groups), key=pair_groups.count)
            
            best_slot = None
            best_date = None
            
            # Try dates in order, considering group preferences
            for date in sorted(slots_by_date.keys()):
                # Check day-off constraints first
                can_schedule, conflicts = can_schedule_on_date(pair, date, student_exam_dates)
                if not can_schedule:
                    continue
                
                # Get preferred slots for this group
                preferred_slots = get_group_preferred_slots(primary_group, date)
                available_slots = [
                    (slot_idx, label, start, end) 
                    for slot_idx, label, start, end in slots_by_date[date]
                    if slot_idx not in assigned_slots and (label, start, end) in preferred_slots
                ]
                
                # If no preferred slots, try any available
                ...
                if available_slots:
                    # Select best slot based on group preference
                    if preferred_slots:
                        # Try to find matching preferred slot
                        for pref_slot in preferred_slots:
                    ...
                    # If no preferred slot matched, take first available
