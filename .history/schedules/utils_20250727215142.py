...
def check_rooms_availability_for_slots(n_students):
    ...
    if total_capacity < n_students:
        return False, "Not enough room capacity for the number of students"
...
def which_suitable_slot_to_schedule_course_group(date, new_group, suggested_slot):
    ...
    # filter exams that are in the same slot as the suggested slot
    all_Students_enrolled_for_to_days_exams= Enrollment.objects.filter(
        group__in=to_days_exams.values_list('group', flat=True)
    ...
    todays_suggestions={"Morning": {}, "Afternoon": [], "Evening": []}
    # check if the suggested slot is already has conflict
    for student in enrolled_students_new_group:
        if suggested_slot == "Morning":
            morning_students= Enrollment.objects.filter(
                group__in=slots_with_exams["Morning"]
            ).values_list('student_id', flat=True)
            if student in morning_students:
                conflicts['Morning'].append(f"Student {student} already has an  of  { get_student_exam( student,date, time(8,0)).exam.group.course.title} in the morning slot.")
        elif suggested_slot == "Afternoon":
            afternoon_students= Enrollment.objects.filter(
                group__in=slots_with_exams["Afternoon"]
            ).values_list('student_id', flat=True)
            if student in afternoon_students:
            ...
            evening_students= Enrollment.objects.filter(
                group__in=slots_with_exams["Evening"]
            ).values_list('student_id', flat=True)
            if student in evening_students:
    ...
    if suggested_slot_has_conflict:
        print("The suggested slot is not available because it has conflict")
    else:
        print("The suggested slot is available")
        return suggested_slot, conflicts[suggested_slot]
    # if the suggested slot has conflict, then we need to find another slot
    for slot, _ in slots_with_exams.items():
        if slot == suggested_slot:
            continue
        # check if the slot has conflict 
        slot_has_conflict= True if len(conflicts[slot]) == 0 else False
        if slot_has_conflict:
            print (f"The {slot} slot is not available")
        else:
            print(f"The {slot} slot is available")
