...
def allocate_shared_rooms():
    """
    Bin packing-based exam allocation algorithm with mixed reallocation for remaining students.
    Ensures all students get rooms as long as capacity is available.
    """
    ...
    # Sort rooms by capacity (largest first) - bin packing optimization
    ...
    with transaction.atomic():
        # Data structure: {date: {slot: {room_id: {'students': [student_exams], 'capacity_used': int}}}}
        schedule = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: {'students': [], 'capacity_used': 0})))
        unaccommodated = []
        ...
        for date, slots in date_slot_students.items():
            for slot_name, slot_start, slot_end in SLOTS:
                student_exams_in_slot = slots.get(slot_name, [])
                if not student_exams_in_slot:
                    continue

                # Group by exam and sort by student count (largest first - bin packing principle)
                exams = defaultdict(list)
                for se in student_exams_in_slot:
                    exams[se.exam].append(se)
                
                sorted_exams = sorted(exams.items(), key=lambda x: -len(x[1]))
                remaining_students = student_exams_in_slot.copy()

                # Phase 1: Primary Allocation with Semester Constraints
                print(f"Phase 1: Processing {date} {slot_name} - {len(remaining_students)} students")
                remaining_students = _allocate_with_semester_constraints(
                    sorted_exams, remaining_students, rooms, schedule, date, slot_name
                )

                # Phase 2: Half-Half Allocation (Fallback Pairing)
                print(f"Phase 2: Half-half allocation - {len(remaining_students)} students remaining")
                remaining_students = _allocate_half_half(
                    sorted_exams, remaining_students, rooms, schedule, date, slot_name
                )

                # Phase 3: Single Course Allocation
                print(f"Phase 3: Single course allocation - {len(remaining_students)} students remaining")
                remaining_students = _allocate_single_courses(
                    sorted_exams, remaining_students, rooms, schedule, date, slot_name
                )

                # Phase 4: Mixed Reallocation for Remaining Students
                print(f"Phase 4: Mixed reallocation - {len(remaining_students)} students remaining")
                remaining_students = _mixed_reallocation(
                    remaining_students, rooms, schedule, date, slot_name
                )

                # Track truly unaccommodated students
                unaccommodated.extend([se.student for se in remaining_students])

        # Save all assignments to database
        _save_assignments_to_database(schedule)

        print(f"Final unaccommodated students: {len(unaccommodated)}")
        return unaccommodated


def _get_semester_number(exam):
...
def _can_pair_semesters(exam1, exam2):
    """Check if two exams can be paired based on semester constraints (non-adjacent)."""
    sem1 = _get_semester_number(exam1)
    sem2 = _get_semester_number(exam2)
    return abs(sem1 - sem2) > 1


def _allocate_with_semester_constraints(sorted_exams, remaining_students, rooms, schedule, date, slot_name):
    """Phase 1: Allocate courses with semester constraints."""
    exams_dict = dict(sorted_exams)
    
    for room in rooms:
        if not remaining_students:
            break
            
        available_capacity = room.capacity - schedule[date][slot_name][room.id]['capacity_used']
        if available_capacity <= 0:
            continue
        
        # Find best semester-constrained pair
        best_pair = None
        best_total_students = 0
        
        for i, (exam1, students1) in enumerate(sorted_exams):
            if not any(se in remaining_students for se in students1):
                continue
                
            remaining_exam1_students = [se for se in students1 if se in remaining_students]
            if not remaining_exam1_students:
                continue
                
            for j, (exam2, students2) in enumerate(sorted_exams[i+1:], i+1):
                if not any(se in remaining_students for se in students2):
                    continue
                    
                remaining_exam2_students = [se for se in students2 if se in remaining_students]
                if not remaining_exam2_students:
                    continue
                
                if _can_pair_semesters(exam1, exam2):
                    total_students = len(remaining_exam1_students) + len(remaining_exam2_students)
                    if total_students <= available_capacity and total_students > best_total_students:
                        best_pair = (exam1, exam2, remaining_exam1_students, remaining_exam2_students)
                        best_total_students = total_students
        
        if best_pair:
            exam1, exam2, students1, students2 = best_pair
            all_students = students1 + students2
            
            # Allocate to room
            schedule[date][slot_name][room.id]['students'].extend(all_students)
            schedule[date][slot_name][room.id]['capacity_used'] += len(all_students)
            
            # Remove from remaining
            for se in all_students:
                remaining_students.remove(se)
                
            print(f"  Paired {exam1.group.course.name} ({len(students1)}) with {exam2.group.course.name} ({len(students2)}) in room {room.name}")
    
    return remaining_students


def _allocate_half_half(sorted_exams, remaining_students, rooms, schedule, date, slot_name):
    """Phase 2: Half-half allocation when semester pairing is not possible."""
    exams_dict = dict(sorted_exams)
    ...
    for room in rooms:
        ...
        available_capacity = room.capacity - schedule[date][slot_name][room.id]['capacity_used']
        if available_capacity < 4:  # Minimum viable half-half split
            continue
        
        # Find two courses that can benefit from half-half split
        best_pair = None
        best_efficiency = 0
        
        for i, (exam1, students1) in enumerate(sorted_exams):
            remaining_exam1_students = [se for se in students1 if se in remaining_students]
            if len(remaining_exam1_students) < 2:
                continue
                
            for j, (exam2, students2) in enumerate(sorted_exams[i+1:], i+1):
                remaining_exam2_students = [se for se in students2 if se in remaining_students]
                if len(remaining_exam2_students) < 2:
                    continue
                
                # Calculate half-half allocation
                half_capacity = available_capacity // 2
                exam1_allocation = min(len(remaining_exam1_students), half_capacity)
                exam2_allocation = min(len(remaining_exam2_students), half_capacity)
                
                total_allocation = exam1_allocation + exam2_allocation
                efficiency = total_allocation / available_capacity
                
                if efficiency > best_efficiency and total_allocation >= 4:
                    best_pair = (exam1, exam2, remaining_exam1_students[:exam1_allocation], 
                               remaining_exam2_students[:exam2_allocation])
        ...
        if best_pair:
            ...
            print(f"  Half-half: {exam1.group.course.name} ({len(students1)}) + {exam2.group.course.name} ({len(students2)}) in room {room.name}")
    
    return remaining_students


def _allocate_single_courses(sorted_exams, remaining_students, rooms, schedule, date, slot_name):
    """Phase 3: Allocate single courses to rooms."""
    for exam, students in sorted_exams:
        remaining_exam_students = [se for se in students if se in remaining_students]
        if not remaining_exam_students:
            continue
        
        # Find best fitting room
        for room in rooms:
            available_capacity = room.capacity - schedule[date][slot_name][room.id]['capacity_used']
            if available_capacity <= 0:
                continue
            
            allocation_size = min(len(remaining_exam_students), available_capacity)
            if allocation_size > 0:
                allocated_students = remaining_exam_students[:allocation_size]
                
                # Allocate to room
                schedule[date][slot_name][room.id]['students'].extend(allocated_students)
                schedule[date][slot_name][room.id]['capacity_used'] += allocation_size
                
                # Remove from remaining
                for se in allocated_students:
                    remaining_students.remove(se)
                
                print(f"  Single: {exam.group.course.name} ({allocation_size}) in room {room.name}")
                
                # Update remaining for this exam
                remaining_exam_students = remaining_exam_students[allocation_size:]
                if not remaining_exam_students:
                    break
...
def _mixed_reallocation(remaining_students, rooms, schedule, date, slot_name):
    """Phase 4: Mixed reallocation - fit remaining students anywhere possible."""
    print(f"  Starting mixed reallocation with {len(remaining_students)} students")
    
    # Sort remaining students by exam for better tracking
    remaining_by_exam = defaultdict(list)
    for se in remaining_students:
        remaining_by_exam[se.exam].append(se)
    
    students_to_remove = []
    
    # Try to fit remaining students in any available space
    for room in rooms:
        available_capacity = room.capacity - schedule[date][slot_name][room.id]['capacity_used']
        if available_capacity <= 0:
            continue
        
        # Fill up this room with any remaining students
        allocated_count = 0
        for exam, exam_students in remaining_by_exam.items():
            if allocated_count >= available_capacity:
                break
            
            # Take as many students as possible from this exam
            can_allocate = min(len(exam_students), available_capacity - allocated_count)
            if can_allocate > 0:
                allocated_students = exam_students[:can_allocate]
                
                # Add to room
                schedule[date][slot_name][room.id]['students'].extend(allocated_students)
                schedule[date][slot_name][room.id]['capacity_used'] += can_allocate
                
                # Mark for removal
                students_to_remove.extend(allocated_students)
                allocated_count += can_allocate
                
                # Update the exam_students list
                remaining_by_exam[exam] = exam_students[can_allocate:]
                
                print(f"  Mixed: {exam.group.course.name} ({can_allocate}) fitted in room {room.name}")
    
    # Remove allocated students from remaining list
    for se in students_to_remove:
        if se in remaining_students:
            remaining_students.remove(se)
    
    print(f"  Mixed reallocation complete. {len(remaining_students)} students still unassigned")
    return remaining_students


def _save_assignments_to_database(schedule):
    ...
    for date, slots in schedule.items():
        for slot_name, room_assignments in slots.items():
            for room_id, room_data in room_assignments.items():
                student_exams = room_data['students']
                if student_exams:
                    StudentExam.objects.filter(
                        id__in=[se.id for se in student_exams]
                    ).update(room_id=room_id)
                    
                    print(f"  Saved {len(student_exams)} students to room {room_id} on {date} {slot_name}")


# Helper function for debugging/monitoring
def print_allocation_summary(schedule):
    """Print a summary of the allocation for debugging."""
    print("\n=== ALLOCATION SUMMARY ===")
    for date, slots in schedule.items():
        print(f"Date: {date}")
        for slot_name, room_assignments in slots.items():
            ...
            for room_id, room_data in room_assignments.items():
                ...
                capacity_used = room_data['capacity_used']
                if students:
                    exams_in_room = defaultdict(int)
                    for se in students:
                        exams_in_room[se.exam.group.course.name] += 1
                    
                    exam_summary = ", ".join([f"{course}({count})" for course, count in exams_in_room.items()])
                    print(f"    Room {room_id}: {capacity_used} students - {exam_summary}")
    print("========================\n")
