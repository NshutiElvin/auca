...
# def can_schedule_course_group_on_slot(course, proposed_date, student_exam_dates):
#     conflicts = []
#     student_ids = Enrollment.objects.filter(course_id=course).values_list('student_id', flat=True)

#     for student_id in student_ids:
#         exam_dates = student_exam_dates.get(student_id, [])
#         exam_dates = [d if isinstance(d, proposed_date) else d.date() for d in exam_dates]

#         if proposed_date in exam_dates:
#             conflicts.append(f"Student {student_id} already has an exam on {proposed_date}")
#         elif not has_sufficient_gap(exam_dates, proposed_date):
#             conflicts.append(f"Student {student_id} does not have enough day-off before {proposed_date}")
...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    Uses backtracking to fill gaps optimally before saving to database
    """
    if not start_date:
    ...
    # Get available dates (excluding Saturdays)
    available_dates = []
    for date, label, start, end in date_slots:
    ...
    best_schedule = find_optimal_schedule(compatible_groups, available_dates)
    
    if not best_schedule:
        return [], "Could not find valid schedule"
    
    # Now save to database with the optimal schedule
    exams_created = []
    
    with transaction.atomic():
        for group_idx, date in best_schedule.items():
            ...
            for course_id in course_group:
                ...
                start_time, end_time = get_time_slot_for_course(course, date)
            ...
            group_exams = [e for e in exams_created if e.date == date]
            try:
                unaccommodated = allocate_shared_rooms(group_exams)
                print(f"✓ Scheduled group {group_idx} on {date}")
            except Exception as e:
                print(f"Room allocation error for group {group_idx}: {e}")
    
    return exams_created, None


def find_optimal_schedule(compatible_groups, available_dates):
    """
    Use backtracking to find the most compact schedule that respects all constraints
    """
    def is_valid_assignment(group_idx, date, current_schedule):
        """Check if assigning a group to a date is valid"""
        course_group = compatible_groups[group_idx]
        student_exam_dates = defaultdict(list)
        
        # Build student exam dates from current schedule
        for scheduled_group_idx, scheduled_date in current_schedule.items():
            scheduled_course_group = compatible_groups[scheduled_group_idx]
            for course_id in scheduled_course_group:
                try:
                    course = Course.objects.get(id=course_id)
                    student_ids = Enrollment.objects.filter(course=course).values_list('student_id', flat=True)
                    for student_id in student_ids:
                        student_exam_dates[student_id].append(scheduled_date)
                except Course.DoesNotExist:
                    continue
        
        # Check if current group can be scheduled on this date
        for course_id in course_group:
            try:
                course = Course.objects.get(id=course_id)
                can_schedule, conflicts = can_schedule_course_group_on_slot(
                    course, date, student_exam_dates
                )
                if not can_schedule:
                    return False
            except Course.DoesNotExist:
    ...
    def backtrack(group_idx, current_schedule):
        """Recursively try to schedule all groups"""
        if group_idx >= len(compatible_groups):
            return current_schedule.copy()
        
        # Try each available date for this group
        for date in available_dates:
            if is_valid_assignment(group_idx, date, current_schedule):
                # Try this assignment
                current_schedule[group_idx] = date
                result = backtrack(group_idx + 1, current_schedule)
                if result:
                    return result
                # Backtrack
                del current_schedule[group_idx]
...
def find_optimal_schedule_with_gap_minimization(compatible_groups, available_dates):
    """
    Enhanced version that tries to minimize gaps in the schedule
    """
    def calculate_schedule_score(schedule):
        ...
        dates_used = sorted(schedule.values())
        if not dates_used:
            return float('inf')
        
        # Calculate total span
        total_span = (dates_used[-1] - dates_used[0]).days
        
        # Calculate number of gaps
        gaps = 0
        for i in range(len(dates_used) - 1):
            gap_size = (dates_used[i + 1] - dates_used[i]).days - 1
            if gap_size > 0:
                gaps += gap_size
        
        # Prefer schedules with fewer gaps and shorter total span
        return total_span + gaps * 2
    ...
    def backtrack_with_pruning(group_idx, current_schedule, best_schedule, best_score):
        """Recursively try to schedule all groups with pruning"""
        if group_idx >= len(compatible_groups):
            score = calculate_schedule_score(current_schedule)
            if score < best_score[0]:
                best_score[0] = score
                best_schedule.clear()
                best_schedule.update(current_schedule)
            return
        
        # Sort dates to try earlier dates first (helps with gap minimization)
        dates_to_try = sorted(available_dates)
        
        for date in dates_to_try:
            if is_valid_assignment(group_idx, date, current_schedule):
                # Pruning: if current partial schedule already has worse score, skip
                current_schedule[group_idx] = date
                partial_score = calculate_schedule_score(current_schedule)
                
                if partial_score < best_score[0]:
                    backtrack_with_pruning(group_idx + 1, current_schedule, best_schedule, best_score)
    ...
    backtrack_with_pruning(0, {}, best_schedule, best_score)
    
    return best_schedule if best_schedule else None


def get_time_slot_for_course(course, date):
    """Get appropriate time slot for a course based on its group preference"""
    course_preference = course.group
    weekday = date.strftime('%A')
    
    if course_preference in ["A", "B"]:
        start_time = time(8, 0)
        end_time = time(11, 0)
    elif course_preference in ["C", "D"]:
        if weekday == "Friday":
            start_time = time(13, 0)
            end_time = time(16, 0)
        else:
            # For C/D groups, prefer afternoon but can use other slots
...
# Alternative: Iterative improvement approach
def generate_exam_schedule_iterative(start_date=None, course_ids=None, semester=None):
    """
    Alternative approach using iterative improvement to fill gaps
    """
    if not start_date:
        start_date = now().date() + timedelta(days=1)
    if course_ids:
    ...
    initial_schedule = greedy_schedule(compatible_groups, available_dates)
    
    if not initial_schedule:
        return [], "Could not find initial schedule"
    
    # Improve the schedule by filling gaps
    improved_schedule = improve_schedule_fill_gaps(initial_schedule, compatible_groups, available_dates)
    
    # Save to database
    exams_created = []
    
    with transaction.atomic():
        for group_idx, date in improved_schedule.items():
            course_group = compatible_groups[group_idx]
...
def greedy_schedule(compatible_groups, available_dates):
    ...
    for group_idx, course_group in enumerate(compatible_groups):
        ...
        for date in available_dates:
            ...
            # Build current student exam dates
            for scheduled_group_idx, scheduled_date in schedule.items():
                scheduled_course_group = compatible_groups[scheduled_group_idx]
                for course_id in scheduled_course_group:
                    try:
                        course = Course.objects.get(id=course_id)
            ...
            # Check if this group can be scheduled on this date
            can_schedule_group = True
            for course_id in course_group:
                try:
                    course = Course.objects.get(id=course_id)
                    can_schedule, conflicts = can_schedule_course_group_on_slot(
...
def improve_schedule_fill_gaps(initial_schedule, compatible_groups, available_dates):
    """Improve schedule by trying to fill gaps"""
    schedule = initial_schedule.copy()
    
    # Find gaps in the current schedule
    used_dates = sorted(schedule.values())
    if len(used_dates) < 2:
        return schedule
    
    # Try to move exams to earlier dates to fill gaps
    for gap_date in available_dates:
        if gap_date in used_dates:
            continue
        
        # Find groups that could potentially move to this gap date
        for group_idx, current_date in list(schedule.items()):
            if gap_date < current_date:
                # Try to move this group to the gap date
                temp_schedule = schedule.copy()
                temp_schedule[group_idx] = gap_date
                
                if is_schedule_valid(temp_schedule, compatible_groups):
...
def is_schedule_valid(schedule, compatible_groups):
    """Check if a complete schedule is valid"""
    student_exam_dates = defaultdict(list)
    
    for group_idx, date in schedule.items():
        course_group = compatible_groups[group_idx]
        for course_id in course_group:
            try:
                course = Course.objects.get(id=course_id)
    ...
    # Check for conflicts
    for student_id, dates in student_exam_dates.items():
        if len(dates) != len(set(dates)):
            return False  # Student has multiple exams on same date
...
#             # Try each group that hasn't been scheduled yet
#             for group_idx, course_group in enumerate(compatible_groups):
...
#                             slot = "Afternoon"
#                     else:
#                         # Default case if preference not recognized
#                         start_time = time(13, 0)
#                         end_time = time(16, 0)
#                         slot = "Afternoon"
