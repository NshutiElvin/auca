...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    """
    Generate exam schedule for individual courses (no group scheduling logic)
    """
    if not start_date:
    ...
    estimated_slots_needed = len(enrolled_courses) * 2  # Buffer for rescheduling
    ...
    with transaction.atomic():
        ...
        for date in dates:
            ...
            # Process each unscheduled course
            for course in enrolled_courses.exclude(id__in=scheduled_course_ids):
                ...
                # Determine time slot based on course group
                ...
                if course_preference in ["A", "B"]:
                    ...
                    if weekday == "Friday":
                        ...
                        slot_choice = random.choice([
                            (time(8, 0), time(11, 0)),
                            (time(13, 0), time(16, 0)),
                            (time(18, 0), time(20, 0))
                        ])
                        start_time, end_time = slot_choice
                ...
                # Allocate room for this exam
                try:
                    allocate_shared_rooms([exam])
                    print(f"Scheduled {course.code} on {date}")
                except Exception as e:
                    print(f"Room allocation failed for {course.code}: {e}")
                    exam.delete()
                    scheduled_course_ids.remove(course.id)
                    continue
                
                # Optional: Try to move to earlier date
