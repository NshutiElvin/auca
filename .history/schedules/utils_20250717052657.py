    ...
    with transaction.atomic():
        ...
        for group_idx, total_students, preference in group_difficulties:
            ...
            if not scheduled:
                ...
                unscheduled.add(group_idx)

        # Back-check and reschedule remaining groups if slots are available
        for date, slot_list in slots_by_date.items():
            for slot_idx, label, start_time, end_time in slot_list:
                if slot_idx in assigned_slots:
                    continue

                for group_idx in list(unscheduled):
                    course_group = all_course_groups[group_idx]
                    preference = group_preferences[group_idx]

                    if date.strftime('%A') == 'Friday' and label == 'Evening':
                        continue
                    ...
                    if can_schedule:
                        ...
                        assigned_slots.add(slot_idx)
                        unscheduled.remove(group_idx)

                        try:
                            unaccommodated = allocate_shared_rooms(group_exams)
                            print(f"[Back-check] Scheduled {preference} group {course_group} on {date} {label} ({start_time}-{end_time})")
                            if unaccommodated:
                                print(f"  Warning: {len(unaccommodated)} students unaccommodated")
                        except Exception as e:
                            print(f"  Room allocation error (back-check): {e}")
