    ...
    with transaction.atomic():
        for date, student_exams in exams.items():
            ...
            for slot_name in ['morning', 'afternoon', 'evening']:
                ...
                # Track room allocations and student assignments
                room_allocations = {room.id: {'capacity': room.capacity, 'used': 0} for room in rooms}
                ...
                for pair in exam_pairs:
                    ...
                    room_capacity = room_allocations[room.id]['capacity']
                    room_used = room_allocations[room.id]['used']
                    remaining_capacity = room_capacity - room_used
                    ...
                    # Calculate how many students we can take from each exam
                    capacity_per_exam = remaining_capacity // len(exams_to_allocate)
                    ...
                    for exam_se in exams_to_allocate:
                        ...
                        if allocated:
                            ...
                            room_allocations[room.id]['used'] += len(allocated)

                        if overflow:
                            # These will be handled in the overflow phase
                            pass

                    # Move to next room if current one is full
                    if room_allocations[room.id]['used'] >= room_allocations[room.id]['capacity']:
                        room_index += 1
                ...
                while room_index < len(rooms) and remaining_exams:
                        ...
                        for j in range(i+1, len(exam_ids)):
                            ...
                            try:
                                ...
                                if abs(sem1 - sem2) > 1:
                                    ...
                                    capacity_per_exam = remaining_capacity // 2
                                    ...
                                    assigned_students.update(student_ids)
                                    room_allocations[room.id]['used'] += len(student_ids)
                    ...
                    if not paired and exam_ids:
                        ...
                        exam_id = exam_ids[0]
                        students = remaining_exams[exam_id][:remaining_capacity]
                        ...
                        assigned_students.update(student_ids)
                        room_allocations[room.id]['used'] += len(students)
                        
                        remaining_exams[exam_id] = remaining_exams[exam_id][remaining_capacity:]
