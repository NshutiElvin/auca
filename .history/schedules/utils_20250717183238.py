...
def find_compatible_courses_within_group(courses, max_group_size=None):
    """
    Find compatible course groups such that students can get days off.
    Enhanced to prioritize disjoint student sets and minimize student impact per group.
    
    Args:
        courses (list): List of course IDs to group.
        max_group_size (int, optional): Maximum number of courses per group.
    
    Returns:
        list of tuples: Each tuple is (course_group, student_ids)
    """
    if not courses:
        return []
    ...
    while remaining_courses:
        ...
        # Choose course with most compatible options
        course1 = max(
            remaining_courses,
        ...
        course_group.append(course1)
        group_students.update(course_students[course1])
        remaining_courses.remove(course1)

        compatible_candidates = set()
        for candidate in remaining_courses:
            if not course_students[candidate] & group_students:  # strict disjoint
        ...
        while compatible_candidates:
            ...
            # Check student disjointness again
            if course_students[next_course] & group_students:
                compatible_candidates.remove(next_course)
                continue

            course_group.append(next_course)
            group_students.update(course_students[next_course])
            remaining_courses.remove(next_course)

            if max_group_size and len(course_group) >= max_group_size:
                break

            # Update compatible candidates
            new_candidates = set()
            for candidate in compatible_candidates:
                if candidate != next_course and not course_students[candidate] & group_students:
                    new_candidates.add(candidate)
            compatible_candidates = new_candidates & remaining_courses

        grouped_courses.append((course_group, group_students))
...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    Args:
        start_date: Optional starting date for exams (defaults to tomorrow)
        course_ids: Optional list of specific course IDs to schedule
        semester: Optional semester filter (not currently implemented)
    
    Returns:
        tuple: (list of created Exam objects, error message if any)
    """
    try:
        logger.info("Starting exam schedule generation")
        
        # Initialize default start date if not provided
        if not start_date:
            ...
            for step, date in enumerate(dates):
                ...
                for group_idx, course_group in enumerate(compatible_groups):
                    ...
                    for course_id in course_group:
                        ...
                        logger.debug(f"Course preference group: {course_preference}")
                        ...
                        if course_preference in ["A", "B"]:
                            ...
                            logger.warning(f"Unknown preference group {course_preference} - using default")
                            start_time = time(13, 0)
                            end_time = time(16, 0)
                            slot = "Afternoon"
