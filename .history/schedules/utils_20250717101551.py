...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    including checking past available dates before current date.
    """
    if not start_date:
    ...
    def try_schedule_group_on_date(course_group, date):
        """Helper to try scheduling a group on a specific date"""
        ...
        for course_id in course_group:
            ...
            # Slot selection based on preference
            if course_preference in ["A", "B"]:
                ...
                if weekday == "Friday":
                    ...
                    start_time, end_time, _ = (
                        (8, 0, 11, 0) if guess == 1 else
                        (13, 0, 16, 0) if guess == 2 else
                        (18, 0, 20, 0)
                    )
    ...
    with transaction.atomic():
        for group_idx, course_group in enumerate(compatible_groups):
            ...
            # 1. Try all previous dates (in order)
            for prev_date in all_dates:
                if prev_date >= start_date:
                    break
                if try_schedule_group_on_date(course_group, prev_date):
                    print(f"✓ Scheduled group {group_idx} on earlier date {prev_date}")
            ...
            # 2. Try current and future dates
            for date in all_dates:
                if try_schedule_group_on_date(course_group, date):
