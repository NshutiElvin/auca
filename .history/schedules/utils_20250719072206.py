...
def generate_exam_schedule(start_date=None, end_date=None,course_ids=None, semester=None):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            date_index=0
            for course_group in compatible_groups:
        ...
        return [], f"Error generating schedule: {str(e)}"from collections import defaultdict
from django.db import transaction
from datetime import time
...
def allocate_shared_rooms():
    ...
    with transaction.atomic():
        ...
        # Helper function to get current room usage
        def get_room_usage(date, slot_name, room_id):
            return len(schedule[date][slot_name].get(room_id, []))
        
        # Helper function to get available capacity
        def get_available_capacity(room, date, slot_name):
            current_usage = get_room_usage(date, slot_name, room.id)
            return max(0, room.capacity - current_usage)
        
        # Helper function to safely assign students to room
        def assign_students_to_room(students_to_assign, room, date, slot_name):
            available = get_available_capacity(room, date, slot_name)
            if available <= 0:
                return []
            
            # Only assign as many as we can fit
            assignable = students_to_assign[:available]
            schedule[date][slot_name][room.id].extend(assignable)
            return assignable
        ...
        for date, slots in date_slot_students.items():
            for slot_name, slot_start, slot_end in SLOTS:
                ...
                # Sort exams by student count (descending) for better utilization
                sorted_exams = sorted(exams.items(), key=lambda x: -len(x[1]))
                remaining_students = student_exams.copy()

                # First pass: Try to pair compatible exams
                used_exams = set()
                ...
                for room in rooms:
                    ...
                    # Look for compatible exam pairs that can share this room
                    best_pair = None
                    best_total_size = 0

                    for i, (exam1, students1) in enumerate(sorted_exams):
                        if exam1 in used_exams or not students1:
                            continue
                        
                        exam1_remaining = [se for se in students1 if se in remaining_students]
                        if not exam1_remaining:
                            continue
                            
                        for j, (exam2, students2) in enumerate(sorted_exams[i+1:], i+1):
                            if exam2 in used_exams or not students2:
                                continue
                            
                            exam2_remaining = [se for se in students2 if se in remaining_students]
                            if not exam2_remaining:
                                continue
                            ...
                            try:
                                ...
                                if abs(sem1 - sem2) <= 1:  # Too close in semester
                                    continue
                            except (ValueError, IndexError, AttributeError):
                                # Skip if semester parsing fails
                                continue
                            
                            # Calculate how many students we can fit from each exam
                            total_students = len(exam1_remaining) + len(exam2_remaining)
                            if total_students <= available and total_students > best_total_size:
                                best_pair = (exam1, exam2, exam1_remaining, exam2_remaining)
                                best_total_size = total_students

                    # Assign the best pair if found
                    if best_pair:
                        exam1, exam2, students1, students2 = best_pair
                        all_students = students1 + students2
                        assigned = assign_students_to_room(all_students, room, date, slot_name)
                        ...
                        used_exams.add(exam1)
                        used_exams.add(exam2)

                # Second pass: Assign single exams to remaining capacity
                for room in rooms:
                    if not remaining_students:
                        break
                    ...
                    # Find the largest unused exam that fits
                    for exam, students in sorted_exams:
                        if exam in used_exams:
                            continue
                            
                        exam_remaining = [se for se in students if se in remaining_students]
                        if not exam_remaining:
                            continue
                        
                        # Assign as many students as possible from this exam
                        assigned = assign_students_to_room(exam_remaining, room, date, slot_name)
                        ...
                        if assigned:  # If we assigned any students, mark exam as used
                            used_exams.add(exam)
                            break

                # Third pass: Fill any remaining capacity with any remaining students
                for room in rooms:
                    if not remaining_students:
                        break
                    ...
                    assigned = assign_students_to_room(remaining_students, room, date, slot_name)
                ...
                # Track any students we couldn't accommodate
        ...
        for date, slots in schedule.items():
            for slot_name, room_assignments in slots.items():
                for room_id, student_exams in room_assignments.items():
                    if student_exams:  # Only update if there are students to assign
                        StudentExam.objects.filter(
                            id__in=[se.id for se in student_exams]
                        ).update(room_id=room_id)

        # Verification: Check for any capacity violations
        capacity_violations = []
        for date, slots in schedule.items():
            for slot_name, room_assignments in slots.items():
                for room_id, student_exams in room_assignments.items():
                    room = next((r for r in rooms if r.id == room_id), None)
                    if room and len(student_exams) > room.capacity:
                        capacity_violations.append({
                            'room_id': room_id,
                            'capacity': room.capacity,
                            'assigned': len(student_exams),
                            'date': date,
                            'slot': slot_name
                        })
        ...
        if capacity_violations:
            ...
            # Optionally raise an exception or handle the violation
