...
def are_semesters_compatible(semester1, semester2):
    """
    Check if two semesters are compatible for sharing a room.
    Adjacent semesters (e.g., 1&2, 2&3) are not compatible.
    """
    if semester1 is None or semester2 is None:
        return True  # If semester info is missing, allow pairing
    
    # Convert to integers for comparison
    try:
        sem1 = int(semester1)
        sem2 = int(semester2)
        return abs(sem1 - sem2) > 1  # Must differ by more than 1
    except (ValueError, TypeError):
        return True  # If conversion fails, allow pairing

def find_compatible_courses(course_conflict_matrix):
    """
    Group courses into compatible groups that can be scheduled together
    Compatible means they don't share students AND are not from adjacent semesters
    """
    all_courses = set()
    for course1, course2 in course_conflict_matrix.keys():
        all_courses.add(course1)
    ...
    course_semesters = {}
    for course in Course.objects.filter(id__in=all_courses):
        course_semesters[course.id] = getattr(course, 'semester', None)
    ...
    for course1 in all_courses:
        for course2 in all_courses:
            if course1 != course2:
                ...
                # Check both student conflicts and semester compatibility
                has_student_conflict = course_conflict_matrix.get(pair, 0) > 0
                semester_compatible = are_semesters_compatible(
                    course_semesters.get(course1),
                    course_semesters.get(course2)
                )
    ...
    while remaining_courses:
        ...
        if remaining_courses:
            ...
            # Add more courses to the group if possible (limit to 2 for room sharing rule)
            while compatible_with_group and len(course_group) < 2:
...
def check_one_day_gap_constraint(student_assignments, exam_date, pair_idx, student_exam_dates):
    """
    Check if scheduling a pair on exam_date violates the one-day gap constraint for any student
    Returns list of conflicted students
    """
    conflicted_students = []
    
    # Get all students involved in this pair
    pair_students = set()
    for student_id, assignments in student_assignments.items():
        for assignment_pair_idx, _ in assignments:
            if assignment_pair_idx == pair_idx:
                pair_students.add(student_id)
    
    # Check each student for gap violations
    for student_id in pair_students:
        student_dates = student_exam_dates[student_id]
        
        # Check if any existing exam date is within 1 day of the proposed date
        for existing_date in student_dates:
            days_diff = abs((exam_date - existing_date).days)
            if days_diff == 1:  # Adjacent days are not allowed
...
def detect_scheduling_conflicts(course_pairs, student_assignments, date_slots):
    ...
    # Calculate conflicts based on student assignments
    for student_id, assignments in student_assignments.items():
        for i, (pair_idx1, _) in enumerate(assignments):
            for j, (pair_idx2, _) in enumerate(assignments[i+1:], i+1):
                if pair_idx1 != pair_idx2:
                    # These two course pairs can't be scheduled on the same day
                    # or adjacent days
                    for date1, slot_indices1 in slots_by_date.items():
                        for date2, slot_indices2 in slots_by_date.items():
                            days_diff = abs((date1 - date2).days)
                            if days_diff <= 1:  # Same day or adjacent days
                                penalty = 10 if days_diff == 0 else 5  # Higher penalty for same day
                                for slot_idx1 in slot_indices1:
                                    for slot_idx2 in slot_indices2:
                                        conflict_scores[pair_idx1][slot_idx1] += penalty
                                        conflict_scores[pair_idx2][slot_idx2] += penalty
...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    """
    Generate exam schedule with enhanced constraints:
    1. One day gap between exams for each student
    2. Maximum 2 exams per room (when possible)
    3. No adjacent semester exams in same room
    """
    if not start_date:
        start_date = now().date() + timedelta(days=1)
    ...
    # Generate more slots to account for one-day gap requirement
    estimated_days = len(course_pairs) * 2  # More days needed due to gap constraint
    date_slots = get_exam_slots(start_date, max_slots=estimated_days * 3)
    ...
    with transaction.atomic():
        # Sort pairs by difficulty (highest conflict scores first)
        pair_difficulties = [(i, max(conflict_scores[i]) if conflict_scores[i] else 0) 
                           for i in range(len(course_pairs))]
        pair_difficulties.sort(key=lambda x: x[1], reverse=True)
        ...
        for pair_idx, _ in pair_difficulties:
            ...
            for date in sorted_dates:
                ...
                # Check one-day gap constraint
                gap_conflicts = check_one_day_gap_constraint(
                    student_assignments, date, pair_idx, student_exam_dates
                )
                if gap_conflicts:
                    continue  # Skip this date due to gap violations
                
                # Check for same-day conflicts
                has_same_day_conflict = False
                for course_id in pair:
                    student_ids = Enrollment.objects.filter(course_id=course_id).values_list('student_id', flat=True)
                    for student_id in student_ids:
                        if date in student_exam_dates[student_id]:
                            has_same_day_conflict = True
                            break
                ...
                for slot_idx in slot_indices:
                    ...
                    slot_score = conflict_scores[pair_idx][slot_idx] if slot_idx < len(conflict_scores[pair_idx]) else 0
            ...
            if best_slot_idx is None:
                raise ValueError(f"Cannot find suitable slot for course pair {pair} while maintaining all constraints.")
            ...
            # Allocate rooms with enhanced constraints
            unaccommodated = allocate_enhanced_rooms(pair_exams)
            unaccommodated_students.extend(unaccommodated)
    
    print(f"Unaccommodated students: {len(unaccommodated_students)}")
    return exams_created, unaccommodated_students

def allocate_enhanced_rooms(exams):
    """
    Allocate rooms with enhanced constraints:
    - Maximum 2 exams per room (when possible)
    - Prioritize room capacity utilization
    """
    if not exams:
        return []
    ...
    # Calculate total students for all exams
    students_by_exam = {}
    total_students = 0
    ...
    if total_students > total_capacity:
        # Calculate proportional accommodation
        accommodation_ratio = total_capacity / total_students
        for exam_id, students in students_by_exam.items():
            max_accommodated = int(len(students) * accommodation_ratio)
            if len(students) > max_accommodated:
                unaccommodated_students.extend([s.student for s in students[max_accommodated:]])
                students_by_exam[exam_id] = students[:max_accommodated]
    
    # Create all StudentExam objects
    all_student_exams = []
    for exam in exams:
        for enrollment in students_by_exam[exam.id]:
            student_exam = StudentExam(student=enrollment.student, exam=exam)
    ...
    # Shuffle to prevent friends from sitting together
    random.shuffle(all_student_exams)
    
    # Enhanced room allocation logic
    if len(exams) <= 2:
        # If we have 2 or fewer exams, try to fit them in the smallest suitable room
        suitable_rooms = [room for room in rooms if room.capacity >= len(all_student_exams)]
        if suitable_rooms:
            target_room = min(suitable_rooms, key=lambda r: r.capacity)
            for student_exam in all_student_exams:
                student_exam.room = target_room
        else:
            # Fall back to largest room
            target_room = rooms[0]
            accommodated = all_student_exams[:target_room.capacity]
            overflow = all_student_exams[target_room.capacity:]
            
            for student_exam in accommodated:
                student_exam.room = target_room
            
            unaccommodated_students.extend([se.student for se in overflow])
            all_student_exams = accommodated
    else:
        # If more than 2 exams, distribute across multiple rooms
        current_room_index = 0
        current_room_capacity = rooms[0].capacity if rooms else 0
    ...
    # Bulk create all student exam records
    StudentExam.objects.bulk_create([se for se in all_student_exams if hasattr(se, 'room')])
    
    return unaccommodated_students

def verify_enhanced_schedule():
    """
    Verify that the current exam schedule meets all enhanced constraints
    """
    conflicts = []
    ...
    for student_id, exam_dates in student_exams.items():
        sorted_dates = sorted(exam_dates)
        for i in range(len(sorted_dates) - 1):
            days_diff = (sorted_dates[i + 1] - sorted_dates[i]).days
            if days_diff == 1:
                conflicts.append({
                    'type': 'adjacent_exam_days',
                    'student_id': student_id,
                    'dates': [sorted_dates[i], sorted_dates[i + 1]]
                })
    
    # Check room exam count constraint
    room_exam_counts = defaultdict(set)
    for student_exam in StudentExam.objects.select_related('exam', 'room'):
        if student_exam.room:
            ...
            room_exam_counts[student_exam.room.id].add(slot_key)
    
    for room_id, slot_set in room_exam_counts.items():
        if len(slot_set) > 2:
            conflicts.append({
                'type': 'room_exam_limit_exceeded',
                'room_id': room_id,
                'exam_count': len(slot_set),
                'limit': 2
            })
    
    # Check semester adjacency constraint
    exams_by_slot = defaultdict(list)
    ...
    for slot, slot_exams in exams_by_slot.items():
        if len(slot_exams) >= 2:
            for i, exam1 in enumerate(slot_exams):
                for exam2 in slot_exams[i+1:]:
                    sem1 = getattr(exam1.course, 'semester', None)
                    sem2 = getattr(exam2.course, 'semester', None)
                    if not are_semesters_compatible(sem1, sem2):
                        conflicts.append({
                            'type': 'adjacent_semester_conflict',
                            'course1': exam1.course.id,
                            'course2': exam2.course.id,
                            'semester1': sem1,
                            'semester2': sem2,
...
def reschedule_exam_enhanced(exam_id, new_date, slot=None):
    """
    Enhanced rescheduling with all new constraints
    """
    with transaction.atomic():
        exam = Exam.objects.get(id=exam_id)
        ...
        for enrollment in enrolled_students:
            other_exams = StudentExam.objects.filter(
                student=enrollment.student
            ).exclude(exam_id=exam_id).select_related('exam')
            
            for other_se in other_exams:
                days_diff = abs((new_date - other_se.exam.date).days)
                if days_diff == 1:
                    gap_conflicts.append(enrollment.student.reg_no)
        
        if gap_conflicts:
            raise ValueError(f"One-day gap violation for students: {', '.join(gap_conflicts[:5])}")
        
        # Check semester compatibility for shared slots
        existing_slot_exams = Exam.objects.filter(
            date=new_date,
            start_time=new_start_time,
            end_time=new_end_time
        ...
        exam_semester = getattr(exam.course, 'semester', None)
        for other_exam in existing_slot_exams:
            other_semester = getattr(other_exam.course, 'semester', None)
            if not are_semesters_compatible(exam_semester, other_semester):
                raise ValueError(f"Semester compatibility conflict: semester {exam_semester} cannot share slot with semester {other_semester}")
        
        # Check room limit (max 2 exams per room per slot)
        total_exams_in_slot = len(existing_slot_exams) + 1
        available_rooms = Room.objects.count()
        
        if total_exams_in_slot > available_rooms * 2:
            raise ValueError(f"Room limit exceeded: {total_exams_in_slot} exams cannot fit in {available_rooms} rooms (max 2 per room)")
        ...
        try:
            unaccommodated = allocate_enhanced_rooms(slot_exams)
            if unaccommodated:
                # Rollback
                exam.date = original_date
                exam.start_time = original_start_time
