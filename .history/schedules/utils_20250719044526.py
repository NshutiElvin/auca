...
def allocate_shared_rooms(exams):
    ...
    with transaction.atomic():
        ...
        # Clear all room assignments for these dates first
        StudentExam.objects.filter(
            exam__date__in=exam_dates
        ).update(room=None)

        for date, exam_instances in exams.items():
            # Get all StudentExam records for this date from database
            student_exams = StudentExam.objects.filter(
                exam__date=date
            ).select_related('student', 'exam', 'exam__group', 'exam__group__course')
