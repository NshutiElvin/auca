...
def find_compatible_courses_within_group(courses):
    """
    Find compatible course groups within a specific group (no shared students),
    using a greedy algorithm plus a back-check merge to avoid unnecessary subgroups.
    """
    if not courses:
        return []
    ...
    # Build conflict matrix: count conflicts between pairs
    ...
    while remaining_courses:
        ...
        # Pick course with most compatible others remaining
        course1 = max(
            remaining_courses,
            key=lambda c: len([
                rc for rc in remaining_courses if rc != c and 
        ...
        # Find courses compatible with all in current group
        compatible_with_group = set()
        for candidate in remaining_courses:
            if all(
                tuple(sorted([candidate, group_course])) not in conflict_matrix or
                conflict_matrix[tuple(sorted([candidate, group_course]))] == 0
                for group_course in course_group
            ):
                compatible_with_group.add(candidate)
        
        # Add more compatible courses greedily
        while compatible_with_group:
            ...
            # Update compatible_with_group to only courses compatible with the newly added course
            compatible_with_group = {
    ...
    # Back-check merging to combine smaller groups if possible
    merged = True
    while merged:
        merged = False
        for i in range(len(course_groups)):
            for j in range(i + 1, len(course_groups)):
                group1, group2 = course_groups[i], course_groups[j]
                # Check if all pairs between group1 and group2 have no conflicts
                if all(
                    tuple(sorted([c1, c2])) not in conflict_matrix or conflict_matrix[tuple(sorted([c1, c2]))] == 0
                    for c1 in group1 for c2 in group2
                ):
                    # Merge group2 into group1 and remove group2
                    course_groups[i] = group1 + group2
                    course_groups.pop(j)
                    merged = True
                    break
            if merged:
