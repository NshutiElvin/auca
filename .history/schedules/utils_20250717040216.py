...
def find_compatible_courses_within_group(courses):
    """
    Use graph theory (independent sets) to find compatible course groups
    — courses that don't share students — and maximize group size.
    """
    if not courses:
        return []
    ...
    for (c1, c2), conflict_count in conflict_matrix.items():
        if conflict_count > 0:
            G.add_edge(c1, c2)

    # Step 3: Use complement graph to find maximal cliques = compatible groups
    compatible_groups = list(nx.find_cliques(nx.complement(G)))
