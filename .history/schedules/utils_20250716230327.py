...
def build_conflict_graph():
    """
    Builds a graph where nodes are course IDs, and edges exist between courses with shared students.
    """
    from collections import defaultdict
...
def greedy_course_coloring(conflict_graph):
    """
    Applies a greedy graph coloring algorithm to group courses without conflicts.
    Each color class is a compatible group.
    """
    coloring = {}
    sorted_courses = sorted(conflict_graph, key=lambda c: len(conflict_graph[c]), reverse=True)

    for course in sorted_courses:
        used_colors = {coloring[neighbor] for neighbor in conflict_graph[course] if neighbor in coloring}
        color = 0
        while color in used_colors:
            color += 1
        coloring[course] = color
...
def find_compatible_courses_within_group(courses):
    ...
    while remaining_courses:
        ...
        while compatible_with_group and len(course_group) < 30:  # Limit group size
            next_course = min(
                compatible_with_group,
                key=lambda c: len([rc for rc in remaining_courses if rc != c and 
                                  tuple(sorted([c, rc])) not in conflict_matrix])
...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    all_course_groups = greedy_course_coloring(conflict_graph)
