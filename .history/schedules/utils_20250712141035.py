    ...
    with transaction.atomic():
        ...
        for pair_idx, _ in pair_difficulties:
            ...
            for date in sorted_dates:
                ...
                if has_date_conflict:
                    continue 
                # Before assigning a slot for a course pair
                for course_id in pair:
                    student_ids = Enrollment.objects.filter(course_id=course_id).values_list('student_id', flat=True)
                    for student_id in student_ids:
                        exam_dates = student_exam_dates[student_id]
                        # Enforce at least one day off between exams
                        if any(abs((date - d).days) <= 1 for d in exam_dates):
                            has_date_conflict = True
                            break
                    if has_date_conflict:
                        break
...
def are_semesters_compatible(exam1, exam2):
    return abs(exam1.course.semester - exam2.course.semester) > 1
...
def allocate_shared_rooms(exams):
    ...
    # Pair exams for room allocation respecting semester gap
    exam_pairs = []
    used = set()
    for i, exam1 in enumerate(exams):
        if exam1.id in used:
            continue
        paired = False
        for j, exam2 in enumerate(exams[i+1:], start=i+1):
            if exam2.id in used:
                continue
            if are_semesters_compatible(exam1, exam2):
                exam_pairs.append([exam1, exam2])
                used.update([exam1.id, exam2.id])
                paired = True
                break
        if not paired:
            exam_pairs.append([exam1])
            used.add(exam1.id)
    
    # Assign rooms to exam pairs, 2 exams per room by default
    all_student_exams = []
    room_index = 0
    room_exam_count = 0
    max_exams_per_room = 2
    
    for pair in exam_pairs:
        if room_index >= len(rooms):
            # Fallback: allow more than 2 exams per room if all rooms are booked
            room_index = 0
            max_exams_per_room = len(pair)
        
        room = rooms[room_index]
        room_capacity = room.capacity
        num_exams = len(pair)
        capacity_per_exam = room_capacity // num_exams if num_exams > 0 else 0
        
        for exam in pair:
            students = students_by_exam.get(exam.id, [])
            assigned_students = students[:capacity_per_exam]
            unassigned_students = students[capacity_per_exam:]
            
            for enrollment in assigned_students:
                student_exam = StudentExam(student=enrollment.student, exam=exam, room=room)
                all_student_exams.append(student_exam)
            
            if unassigned_students:
                unaccommodated_students.extend([e.student for e in unassigned_students])
        
        room_exam_count += 1
        if room_exam_count >= max_exams_per_room:
            room_index += 1
            room_exam_count = 0
