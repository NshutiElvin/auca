...
def find_compatible_courses_within_group(courses):
    ...
    conflict_graph.add_nodes_from(course_ids)

    # Step 1: Build conflict matrix using enrollments
    student_courses = defaultdict(list)
    ...
    # Step 2: Add edges to graph where conflicts exist
    for c1, c2 in conflict_pairs:
        conflict_graph.add_edge(c1, c2)
    
    # Step 3: Find all maximal independent sets (cliques in complement graph)
    compatible_groups = list(nx.find_cliques(nx.complement(conflict_graph)))
