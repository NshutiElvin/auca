...
def remove_scheduled_group(myscheduled_groups, course_id, group_id):
    for course_group in myscheduled_groups:
...
def generate_exam_schedule(start_date=None, end_date=None, course_ids=None, semester=None):
    try:
        ...
        with transaction.atomic():
            ...
            date_index = 0
            all_available_seats = Room.objects.order_by('-capacity').aggregate(total=Sum('capacity'))['total'] or 0
            
            while date_index < len(dates) and myscheduled_groups:
                ...
                # Track seat usage per slot for this day
                slot_seats_usage = {"Morning": 0, "Evening": 0, "Afternoon": 0}
                scheduled_any = False
                
                # Work on a copy of groups to avoid modification during iteration
                current_groups = myscheduled_groups.copy()
                
                for course_group in current_groups:
                    for mycourse_group in course_group["courses"]:
                        course_id = mycourse_group["course_id"]
                        course = Course.objects.get(id=course_id)
                        
                        for group_id in mycourse_group["groups"][:]:  # Iterate copy
                            ...
                            if group.group_name in ["A", "B"]:
                                slot = "Morning" if random.randint(1, 2) == 1 else "Afternoon"
                            elif group.group_name in ["C", "D"]:
                                if weekday == "Friday":
                                    slot = "Afternoon"
                                else:
                                    slot = ["Morning", "Afternoon", "Evening"][random.randint(0, 2)]
                            elif group.group_name in ["E", "F"]:
                                slot = "Evening" if weekday != "Friday" else "Afternoon"
                            else:
                                slot = "Afternoon"
                            
                            # Set times based on slot
                            if slot == "Morning":
                                start_time, end_time = time(8, 0), time(11, 0)
                            elif slot == "Afternoon":
                                start_time, end_time = time(13, 0), time(16, 0)
                            else:  # Evening
                                start_time, end_time = time(18, 0), time(20, 0)
                            
                            # Check capacity before attempting to schedule
                            if slot_seats_usage[slot] + len(student_ids) > all_available_seats:
                                continue
                            ...
                            try:
                                ...
                                # Only update after successful creation
                                slot_seats_usage[slot] += len(student_ids)
                                ...
                                remove_scheduled_group(myscheduled_groups, course_id, group_id)
                                scheduled_any = True
                                
                            except Exception as e:
                                logger.error(f"Failed to schedule group {group_id}: {str(e)}")
                                continue
                
                # Only move to next date if we scheduled something
                if scheduled_any:
                    date_index += 1
                else:
                    # No groups could be scheduled for this date, move to next
                    date_index += 1
        ...
        logger.exception("Error in generate_exam_schedule")
        return [], f"Error generating schedule: {str(e)}", compatible_groups
...
#             dates = sorted(slots_by_date.keys())
#             date_index=0
#             slot_seats_usages={ "Morning":0, "Evening":0, "Afternoon":0}
#             all_available_seats= Room.objects.order_by('-capacity').aggregate(total=Sum('capacity'))['total']

           
#             for idx,course_group in enumerate(compatible_groups):
#                 print(all_available_seats)
...
#                         # Create the exam
#                         try:
#                             slot_to_use= get_slot_name(start_time, end_time)
#                             if slot_seats_usages[slot_to_use]+ len(student_ids) <= all_available_seats:
#                                 exam = Exam.objects.create(
#                                 date=date,
#                                 start_time=start_time,
#                                 end_time=end_time,
...
#                                     student_exam_dates[student_id].append(date)
#                                 remove_scheduled_group(myscheduled_groups, course_id, group_id)
#                                 slot_seats_usages[slot_to_use]+=len(student_ids)
#                             else:
#                                 continue
...
#                             break
#                 group_index=myscheduled_groups.index(course_group)
#                 current_coursegroup_courses= myscheduled_groups[group_index]["courses"]
#                 if(len(current_coursegroup_courses)==0):
