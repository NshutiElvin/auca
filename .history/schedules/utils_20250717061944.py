...
def can_schedule_course_group_on_slot(course, proposed_date, student_exam_dates):
    """Check if scheduling this course would cause student conflicts"""
    conflicts = []
    student_ids = Enrollment.objects.filter(course=course).values_list('student_id', flat=True)
    
    for student_id in student_ids:
        # Check if student already has exam on this date
    ...
    with transaction.atomic():
        ...
        for date in sorted(slots_by_date.keys()):
            ...
            for slot_idx, label, start_time, end_time in date_slots:
                ...
                for group_idx, total_students in group_difficulties:
                    ...
                    for course in course_group:
                        ...
                        can_schedule, _ = can_schedule_course_group_on_slot(
                            course, date, (label, start_time, end_time), 
                            student_exam_dates
                        )
                        if can_schedule:
                            group.append(course)
                ...
                # Schedule the most constrained compatible group
