...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    print(find_compatible_courses_within_group(enrolled_courses))
    ...
    with transaction.atomic():
        ...
        for date in sorted(slots_by_date.keys()):
            ...
            # Continue until no more groups can be scheduled on this date
            while True:
                any_group_scheduled_today = False
                
                # Try to schedule each unscheduled group on this date
                for group_idx, total_students, preference in group_difficulties:
                    # Skip if this group is already scheduled
                    ...
                    for slot_idx, label, start_time, end_time in available_slots:
                        ...
                        if can_schedule:
                            ...
                            # Mark slot as assigned and group as scheduled
                            assigned_slots.add(slot_idx)
            ...
            print(f"--- Finished scheduling for {date} ---")
        
        # Check if all groups were scheduled
        unscheduled_groups = [
            (group_idx, all_course_groups[group_idx]) 
            for group_idx, _, _ in group_difficulties 
            if group_idx not in scheduled_groups
        ]
        
        if unscheduled_groups:
            print(f"\nError: Could not schedule the following groups:")
            for group_idx, course_group in unscheduled_groups:
                preference = group_preferences[group_idx]
                print(f"  {preference} group {course_group}")
