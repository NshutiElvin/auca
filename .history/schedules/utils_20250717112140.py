...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    - Builds schedule in memory first
    - Tries to fill gaps
    - Only saves to DB at the end
    """
    if not start_date:
    ...
    # Sort groups by number of students (smallest group first)
    def group_size(group):
        return sum(Enrollment.objects.filter(course_id=cid).count() for cid in group)

    compatible_groups.sort(key=group_size)
    ...
    scheduled_exams = {}  # {group_idx: list of exams}
    student_exam_dates = defaultdict(list)  # {student_id: [dates]}
    used_dates = set()

    # First pass: Try scheduling all groups
    for group_idx, course_group in enumerate(compatible_groups):
        ...
        for date in sorted(slots_by_date.keys()):
            ...
            for course_id in course_group:
                ...
                preference = course.group
                can_schedule, conflicts = can_schedule_course_group_on_slot(course, date, student_exam_dates)
                if not can_schedule:
                    can_use_date = False
                    break
            ...
            if can_use_date:
                # Schedule this group
                for exam_data in exams_for_group:
                    student_ids = Enrollment.objects.filter(course=exam_data['course']).values_list('student_id', flat=True)
                    for student_id in student_ids:
                        student_exam_dates[student_id].append(exam_data['date'])
                scheduled_exams[group_idx] = exams_for_group
                used_dates.add(date)
                scheduled = True
                print(f"✓ Tentatively scheduled group {group_idx} on {date}")
                break

        if not scheduled:
            print(f"⚠️ Could not tentatively schedule group {group_idx}")

    # Second pass: Fill gaps for unscheduled groups
    unscheduled_groups = [i for i in range(len(compatible_groups)) if i not in scheduled_exams]
    available_dates = sorted(set(slots_by_date.keys()) - used_dates)

    for group_idx in unscheduled_groups:
        ...
        for date in available_dates:
            ...
            for course_id in course_group:
                ...
                # Reuse same time logic as above
                if preference in ["A", "B"]:
                    start_time = time(8, 0)
                    end_time = time(11, 0)
                elif preference in ["C", "D"]:
            ...
            if can_use_date:
                ...
                used_dates.add(date)
                scheduled = True
                print(f"✓ Filled gap: Scheduled group {group_idx} on {date}")
                break

        if not scheduled:
            print(f"❌ Could not reschedule group {group_idx} even after gap-filling")

    # Final check
    if len(scheduled_exams) < len(compatible_groups):
        failed_count = len(compatible_groups) - len(scheduled_exams)
        print(f"⚠️ Failed to schedule {failed_count} groups")
        return [], f"Failed to schedule {failed_count} groups due to constraints"

    # Save all to DB
    all_exams = []
    with transaction.atomic():
        for group_idx, exams in scheduled_exams.items():
            for exam_data in exams:
                exam = Exam(**exam_data)
                exam.save()
                all_exams.append(exam)

        # Allocate rooms after saving
        for group_idx, exams in scheduled_exams.items():
            try:
                unaccommodated = allocate_shared_rooms(exams)
                if unaccommodated:
                    print(f"⚠️ Some rooms could not be allocated for group {group_idx}")
            except Exception as e:
                print(f"Room allocation error for group {group_idx}: {e}")

    print("✅ Successfully generated full exam schedule")
    return all_exams, None
# def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
#     """
#     Generate exam schedule considering course groups and their preferences
#     """
