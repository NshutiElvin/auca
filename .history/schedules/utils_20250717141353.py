...
def choose_slot(group_preference, weekday):
    if group_preference in ["A", "B"]:
        return time(8, 0), time(11, 0), "Morning"
    elif group_preference in ["C", "D"]:
        if weekday == "Friday":
    ...
    elif group_preference in ["E", "F"]:
        if weekday != "Friday":
            return time(18, 0), time(20, 0), "Evening"
        else:
            return time(13, 0), time(16, 0), "Afternoon"
    return time(13, 0), time(16, 0), "Afternoon"  # default


def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    """
    Generate exam schedule with backtracking to earlier unscheduled dates.
    """
    today = now().date()
    if not start_date:
    ...
    # Create past + future dates to try — include today - 7 to today + estimated
    backtrack_days = 7
    total_days = backtrack_days + estimated_slots_needed // 3 + 5

    all_dates = [today - timedelta(days=i) for i in range(backtrack_days, 0, -1)] + \
                [start_date + timedelta(days=i) for i in range(total_days)]

    date_slots = get_exam_slots(all_dates[0], max_slots=total_days * 3)
        ...
        for group_idx, course_group in enumerate(compatible_groups):
            ...
            for date in sorted(slots_by_date.keys()):
                ...
                for course_id in course_group:
                    ...
                    start_time, end_time, slot = choose_slot(course.group, weekday)
                ...
                if scheduled_this_group and group_exams:
                    ...
                    scheduled = True
                    break  # group scheduled, stop trying other dates

            if not scheduled:
                print(f"✗ Could not schedule group {group_idx} on any date")
