...
class RoomBin:
    """Represents a room as a bin for the bin packing algorithm"""
    def __init__(self, room, date, slot_name):
        self.room = room
        self.date = date
        self.slot_name = slot_name
        self.capacity = room.capacity
        self.items = []  # List of StudentExam objects
        self.exam_groups = defaultdict(list)  # Group items by exam
        
    def can_add_item(self, student_exam):
        """Check if a student exam can be added to this bin"""
        if len(self.items) >= self.capacity:
            return False
            
        # Check semester separation constraint
        for existing_se in self.items:
            if existing_se.exam != student_exam.exam:
                sem1 = int(existing_se.exam.group.course.semester.name.split()[1])
                sem2 = int(student_exam.exam.group.course.semester.name.split()[1])
                if abs(sem1 - sem2) <= 1:  # Not allowed if difference <= 1
                    return False
        
        return True
    
    def add_item(self, student_exam):
        """Add a student exam to this bin"""
        if self.can_add_item(student_exam):
            self.items.append(student_exam)
            self.exam_groups[student_exam.exam].append(student_exam)
            return True
        return False
    
    def remaining_capacity(self):
        """Get remaining capacity of the bin"""
        return self.capacity - len(self.items)
    
    def utilization(self):
        """Get utilization percentage of the bin"""
        return len(self.items) / self.capacity if self.capacity > 0 else 0


class BinPackingRoomAllocator:
    ...
    def get_slot_for_exam(self, exam):
        """Determine which slot an exam belongs to"""
        for slot_name, start, end in self.SLOTS:
            if exam.start_time == start and exam.end_time == end:
                return slot_name
        return None
    
    def group_items_by_time_slot(self, student_exams):
        """Group student exams by date and time slot"""
        time_slot_groups = defaultdict(list)
        
        for se in student_exams:
            slot = self.get_slot_for_exam(se.exam)
            if slot:
                key = (se.exam.date, slot)
                time_slot_groups[key].append(se)
        
        return time_slot_groups
    
    def sort_items_by_priority(self, items):
        """Sort items by priority for bin packing"""
        # Group by exam first
        exam_groups = defaultdict(list)
        for item in items:
            exam_groups[item.exam].append(item)
        
        # Sort exams by size (descending) - larger groups first
        sorted_exam_groups = sorted(exam_groups.items(), key=lambda x: -len(x[1]))
        
        # Flatten back to items list, maintaining exam grouping
        sorted_items = []
        for exam, exam_items in sorted_exam_groups:
            sorted_items.extend(exam_items)
        
        return sorted_items
    
    def create_bins(self, rooms, date, slot_name):
        """Create bins for a specific date and time slot"""
        # Sort rooms by capacity (descending) - larger rooms first
        sorted_rooms = sorted(rooms, key=lambda r: -r.capacity)
        return [RoomBin(room, date, slot_name) for room in sorted_rooms]
    
    def first_fit_decreasing(self, items, bins):
    ...
    def best_fit_decreasing(self, items, bins):
        ...
        for item in items:
            ...
            # Find the bin with minimum remaining capacity that can fit this item
            for bin_obj in bins:
                if bin_obj.can_add_item(item):
                    remaining = bin_obj.remaining_capacity()
                    if remaining < min_remaining:
                        min_remaining = remaining
                        best_bin = bin_obj
            
            if best_bin and best_bin.add_item(item):
    ...
    def optimize_bin_utilization(self, bins):
        """Try to optimize bin utilization by redistributing items"""
        # Find bins with low utilization
        low_util_bins = [b for b in bins if 0 < b.utilization() < 0.5 and len(b.items) > 0]
        high_capacity_bins = [b for b in bins if b.remaining_capacity() > 0]
        
        for low_bin in low_util_bins:
            items_to_move = low_bin.items.copy()
            
            # Try to move items to other bins
            for item in items_to_move:
                for target_bin in high_capacity_bins:
                    if target_bin != low_bin and target_bin.can_add_item(item):
                        # Remove from source bin
                        low_bin.items.remove(item)
                        low_bin.exam_groups[item.exam].remove(item)
                        if not low_bin.exam_groups[item.exam]:
                            del low_bin.exam_groups[item.exam]
                        
                        # Add to target bin
                        target_bin.add_item(item)
                        break
    
    def allocate_with_bin_packing(self, student_exams, rooms):
        """Main bin packing allocation algorithm"""
        ...
        time_slot_groups = self.group_items_by_time_slot(student_exams)
        
        # Process each time slot separately
        for (date, slot_name), items in time_slot_groups.items():
            # Sort items by priority (largest exam groups first)
            sorted_items = self.sort_items_by_priority(items)
            
            # Create bins for this time slot
            bins = self.create_bins(rooms, date, slot_name)
            
            # Apply bin packing algorithm - try Best Fit Decreasing first
            unassigned = self.best_fit_decreasing(sorted_items, bins)
            
            # If there are still unassigned items, try First Fit Decreasing
            if unassigned:
                # Create fresh bins for remaining items
                remaining_bins = [b for b in bins if b.remaining_capacity() > 0]
                unassigned = self.first_fit_decreasing(unassigned, remaining_bins)
            
            # Optimize bin utilization
            self.optimize_bin_utilization(bins)
            
            # Collect assignments from bins
            for bin_obj in bins:
                if bin_obj.items:
                    all_assignments.append({
                        ...
                        'date': date,
                        'slot': slot_name,
                        'student_exams': bin_obj.items,
                        'utilization': bin_obj.utilization()
                    })
            
            # Track unassigned items
            unaccommodated.extend(unassigned)
        
        return unaccommodated, all_assignments


def allocate_shared_rooms():
    """Main function using bin packing algorithm for room allocation"""
    ...
    # Initialize bin packing allocator
    allocator = BinPackingRoomAllocator()
    
    with transaction.atomic():
        # Run bin packing allocation
        unaccommodated_exams, assignments = allocator.allocate_with_bin_packing(
            list(student_exams), rooms
        )
        
        # Save assignments to database
        for assignment in assignments:
            student_exam_ids = [se.id for se in assignment['student_exams']]
            StudentExam.objects.filter(
                id__in=student_exam_ids
            ).update(room_id=assignment['room'].id)
        
        # Final attempt for remaining unassigned items using greedy approach
        final_unaccommodated = []
        for se in unaccommodated_exams:
            assigned = False
            slot = allocator.get_slot_for_exam(se.exam)
            
            if slot:
                # Try to find any room with available capacity
                for room in rooms:
                    # Check current room usage for this time slot
                    current_usage = StudentExam.objects.filter(
                        room=room,
                        exam__date=se.exam.date,
                        exam__start_time=se.exam.start_time
                    ).count()
                    
                    if current_usage < room.capacity:
                        # Check semester separation constraint
                        conflicting = StudentExam.objects.filter(
                            room=room,
                            exam__date=se.exam.date,
                            exam__start_time=se.exam.start_time
                        ).select_related('exam__group__course__semester')
                        
                        can_assign = True
                        se_semester = int(se.exam.group.course.semester.name.split()[1])
                        
                        for conf_se in conflicting:
                            if conf_se.exam != se.exam:
                                conf_sem = int(conf_se.exam.group.course.semester.name.split()[1])
                                if abs(se_semester - conf_sem) <= 1:
                                    can_assign = False
                                    break
            ...
            if not assigned:
                final_unaccommodated.append(se.student)
...
def analyze_allocation_efficiency(assignments):
    """Analyze the efficiency of the bin packing allocation"""
    total_capacity = sum(assignment['room'].capacity for assignment in assignments)
    total_used = sum(len(assignment['student_exams']) for assignment in assignments)
    
    overall_utilization = total_used / total_capacity if total_capacity > 0 else 0
    
    utilization_stats = [assignment['utilization'] for assignment in assignments]
    avg_utilization = sum(utilization_stats) / len(utilization_stats) if utilization_stats else 0
    
    return {
        'total_rooms_used': len(assignments),
        'total_capacity': total_capacity,
        'total_students_assigned': total_used,
        'overall_utilization': overall_utilization,
        'average_room_utilization': avg_utilization,
        'min_utilization': min(utilization_stats) if utilization_stats else 0,
        'max_utilization': max(utilization_stats) if utilization_stats else 0
    }
...
def get_allocation_report():
    ...
    student_exams = StudentExam.objects.filter(
        room__isnull=False
    ).select_related('exam', 'room', 'student')
    
    # Group by room and time slot
    room_schedule = defaultdict(lambda: defaultdict(list))
    
    for se in student_exams:
        date = se.exam.date
        time_key = f"{se.exam.start_time}-{se.exam.end_time}"
        key = f"{date}_{time_key}"
        room_schedule[se.room][key].append(se)
    ...
    for room, schedule in room_schedule.items():
        ...
        for time_slot, students in schedule.items():
            utilization = len(students) / room.capacity
            room_info['schedule'][time_slot] = {
                'students_count': len(students),
                'utilization': utilization,
                'exams': list(set(se.exam for se in students))
            }
