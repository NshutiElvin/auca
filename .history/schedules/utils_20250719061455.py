...
def allocate_rooms_for_all_upcoming_exams():
    SLOTS = [
        ('morning', time(8, 0), time(11, 0)),
        ('afternoon', time(13, 0), time(16, 0)),
    ...
    future_exams = Exam.objects.filter(date__gte=dt_date.today()).select_related(
        'group__course__semester'
    )

    # Preload all StudentExam records
    student_exams = StudentExam.objects.filter(
        exam__in=future_exams
    ).select_related('exam', 'exam__group__course__semester', 'student')

    # Organize by date and slot
    exams_by_date_slot = defaultdict(lambda: defaultdict(list))
    for se in student_exams:
        for slot_name, start, end in SLOTS:
            if se.exam.start_time == start and se.exam.end_time == end:
                exams_by_date_slot[se.exam.date][slot_name].append(se)
                break

    all_rooms = list(Room.objects.order_by('-capacity'))

    with transaction.atomic():
        for exam_date, slots in exams_by_date_slot.items():
            for slot_name in ['morning', 'afternoon', 'evening']:
                ses = slots[slot_name]
                if not ses:
                    continue

                rooms = all_rooms.copy()  # Reset room usage for each slot
                ...
                # Utility to find compatible exam pairs
                def find_pair():
                ...
                while rooms and exam_queues:
                    ...
                    cap1 = cap2 = room.capacity // 2 if e2 else room.capacity
                    ...
                    assigned_students.update([se.student.id for se in to_place1])
                    ...
                    if e2:
                        ...
                        assigned_students.update([se.student.id for se in to_place2])
                        exam_queues[e2] = q2[len(to_place2):]
                        if not exam_queues[e2]:
                            del exam_queues[e2]

                # Leftovers = unaccommodated
