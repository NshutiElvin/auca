...
def remove_scheduled_group(scheduled_groups, course_id, group_id):
    """Remove a specific group from scheduled groups and clean up empty courses/groups"""
    for course_group in scheduled_groups:
        for course_dict in course_group["courses"]:
            if course_dict["course_id"] == course_id and group_id in course_dict["groups"]:
                course_dict["groups"].remove(group_id)
                return True
    return False

def clean_empty_groups(scheduled_groups):
    """Remove courses with no groups and course_groups with no courses"""
    for course_group in scheduled_groups[:]:  # Create a copy to iterate over
        # Remove courses with no groups
        course_group["courses"] = [
            course for course in course_group["courses"] 
            if course["groups"]
        ]
        # Remove course_groups with no courses
        if not course_group["courses"]:
            scheduled_groups.remove(course_group)
...
def generate_exam_schedule(start_date=None, end_date=None, course_ids=None, semester=None):
    ...
    try:
        ...
        if not start_date:
            start_date = now().date() + timedelta(days=2)  # Fixed: was adding 1 day twice
        ...
        all_available_seats = Room.objects.order_by('-capacity').aggregate(
            total=Sum('capacity')
        )['total'] or 0
        ...
        with transaction.atomic():
            ...
            for date in dates:
                ...
                # Reset slot usage for each date
                slot_seats_usage = {"Morning": 0, "Evening": 0, "Afternoon": 0}
                scheduled_today = False
                
                print(f"Processing date: {date} ({weekday})")
                
                # Process remaining groups for this date
                groups_to_remove = []
                
                for course_group_idx, course_group in enumerate(remaining_groups):
                    courses = course_group["courses"]
                    
                    for course_dict in courses[:]:  # Create copy to iterate over
                        course_id = course_dict["course_id"]
                        course_groups = course_dict["groups"][:]  # Copy the groups list
                        ...
                        for group_id in course_groups:
                            ...
                            if not student_ids:
                                # Remove empty groups
                                remove_scheduled_group(remaining_groups, course_id, group_id)
                                continue
                            
                            # Determine exam time based on group and day
                            start_time, end_time = get_exam_time_for_group(group.group_name, weekday)
                            
                            if not start_time or not end_time:
                                continue  # Skip if no valid time slot
                            ...
                            if slot_seats_usage[slot_name] + len(student_ids) <= all_available_seats:
                                try:
                                    ...
                                    print(f"Scheduled exam for course {course_id}, group {group_id} on {date}")
                                    
                                except Exception as e:
                                    print(f"Failed to create exam for course {course_id}, group {group_id}: {str(e)}")
                                    logger.error(f"Failed to create exam: {str(e)}")
                            else:
                                print(f"Not enough seats for course {course_id}, group {group_id} in {slot_name} slot")
                
                # Clean up empty groups after processing this date
                clean_empty_groups(remaining_groups)
                
                # If no groups were scheduled today and we still have groups, 
                # it might indicate a capacity or time slot issue
                if not scheduled_today and remaining_groups:
                    print(f"No groups could be scheduled on {date}")
            
            # Any remaining groups are unscheduled
        ...
        return exams_created, unaccommodated_students, unscheduled_groups
...
def get_exam_time_for_group(group_name, weekday):
    """Determine exam time based on group name and weekday"""
    import random
    from datetime import time
    
    if group_name in ["A", "B"]:
        # Random choice between morning and afternoon
        if random.randint(1, 2) == 1:
            return time(8, 0), time(11, 0)  # Morning
        else:
            return time(13, 0), time(16, 0)  # Afternoon
            
    elif group_name in ["C", "D"]:
        if weekday == "Friday":
            return time(13, 0), time(16, 0)  # Afternoon only on Friday
        else:
            ...
            if choice == 1:
                ...
                return time(18, 0), time(20, 0)  # Evening
                
    elif group_name in ["E", "F"]:
        if weekday != "Friday":
            return time(18, 0), time(20, 0)  # Evening on non-Friday
        else:
            return time(13, 0), time(16, 0)  # Afternoon on Friday
    else:
        # Default case
        return time(13, 0), time(16, 0)  # Afternoon
...
#                 # Sort exams by descending student count
#                 sorted_exams = sorted(exams.items(), key=lambda x: -len(x[1]))
...
#                     # --- 1️⃣ Try to pair two different exams ---
#                     best_pair = None
#                     max_fill = 0
...
#                         if not students1:
#                             continue
#                         for j, (exam2, students2) in enumerate(sorted_exams):
#                             if i == j or not students2:
#                                 continue
#                             sem1 = int(exam1.group.course.semester.name.split()[1])
#                             sem2 = int(exam2.group.course.semester.name.split()[1])
#                             if abs(sem1 - sem2) > 1:
#                                 # Allocate half-half or as close as possible
#                                 take1 = min(len(students1), available // 2)
#                                 take2 = min(len(students2), available - take1)
#                                 fill = take1 + take2
#                                 if fill > max_fill and fill <= available:
#                                     best_pair = (exam1, take1, exam2, take2)
#                                     max_fill = fill

#                     if best_pair:
#                         e1, c1, e2, c2 = best_pair
#                         for exam, count in [(e1, c1), (e2, c2)]:
#                             exam_students = [se for se in remaining_students if se.exam == exam][:count]
#                             assigned.extend(exam_students)
#                             for se in exam_students:
#                                 remaining_students.remove(se)

#                     # --- 2️⃣ If no pair found, try single exam filling ---
#                     if not assigned:
#                         exam, students = next(((e, s) for e, s in sorted_exams if s), (None, None))
#                         if exam:
#                             to_assign = [se for se in remaining_students if se.exam == exam][:available]
...
#         # Final attempt to fit remaining students
#         if unaccommodated:
#             remaining_exams = StudentExam.objects.filter(
#                 student__in=unaccommodated,
#                 room__isnull=True
