...
def find_compatible_courses_within_group(courses):
    ...
    for enrollment in Enrollment.objects.filter(course_id__in=course_ids).iterator():
        ...
        print(course_graph)
        if len(courses_graph) ==0:
            courses_graph[enrollment.course_id].append({enrollment.group_id:[enrollment.student_id]})
        else:
            
            course_graph=courses_graph[enrollment.course_id]
            print(course_graph)
            for course_group in courses_graph:
                group= course_group.get(f"{enrollment.group}")
                if group:
                    group.append(enrollment.student_id)
                else:
                    group={enrollment.group_id:[enrollment.student_id]}
                course_graph.remove(course_graph)
                course_graph.append(group)
                break
            courses_graph[enrollment.course_id]=course_graph
