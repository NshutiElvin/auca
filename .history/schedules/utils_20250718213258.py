...
def find_compatible_courses_within_group(courses):
    ...
    conflict_matrix = defaultdict(int)
    course_ids = courses
    courses_with_conflicts=defaultdict()
    all_courses= set()
    ...
    courses = courses_graph.keys()

    for i,course in enumerate(list(courses)):
        print(courses)
        course1= courses_graph.get(course)
        course2= courses_graph.get(courses[i+1])
        course1_groups= course1[course1]
        course2_groups=course2[course2]

        course1_students= []
        course2_students=[]
        for g in course1_groups:
            students=course1_groups[g]
            course1_students.append(students)
        for g in course2_groups:
            students=course2_groups[g]
            course2_students.append(*students)
        print(course1_students)
