...
def find_compatible_courses_within_group(courses):
    ...
    for enrollment in Enrollment.objects.filter(course_id__in=course_ids).iterator():
        ...
        graph=courses_graph.get(enrollment.course_id)
        if graph:
          
            for course_group in graph:
                group= course_group.get(enrollment.group)
                if group:
                    group.append(enrollment.student_id)
                else:
                    group={enrollment.group_id:[enrollment.student_id]}
                    graph.remove(course_group)
                    graph.append(group)
                break
            courses_graph[enrollment.course_id]=graph
