...
def generate_exam_schedule(course_groups, course_semester_map, student_course_map, rooms, start_date):
    from datetime import timedelta

    def get_slots_for_day(date):
        # Skip Saturdays
        if date.weekday() == 5:
            return []
    ...
    def is_compatible(group1, group2):
        return all(len(set(student_course_map.get(c1, [])) & set(student_course_map.get(c2, []))) == 0 for c1 in group1 for c2 in group2)

    def assign_group_to_day_slot(group, date, slot):
        schedule.append((group, date, slot))
        assigned_groups.add(tuple(group))
        for c in group:
            course_to_date[c] = date
    ...
    while len(assigned_groups) < total_groups:
        slots = get_slots_for_day(current_date)
        used_slots = set()

        for slot in slots:
            if len(assigned_groups) >= total_groups:
                break

            for group in course_groups:
                if tuple(group) in assigned_groups:
                    continue

                # Ensure all courses in this group haven't been scheduled yet
                if any(c in course_to_date for c in group):
                    continue

                # Check if this group respects the per-student 1 exam/day rule
                violates_day_gap = False
                for course in group:
                    students = student_course_map.get(course, [])
                    for s in students:
                        for c2, d2 in course_to_date.items():
                            if s in student_course_map.get(c2, []) and d2 == current_date:
                                violates_day_gap = True
                                break
                        if violates_day_gap:
                            break
                    if violates_day_gap:
                ...
                # Assign to the slot
                assign_group_to_day_slot(group, current_date, slot)
                used_slots.add(slot)
    ...
    # Optional: Backfill pass for unused early slots
    rebalanced = True
    while rebalanced:
        rebalanced = False
        new_schedule = []
        ...
        for group, date, slot in schedule:
            used_slots_by_day.setdefault(date, set()).add(slot)

        for group, date, slot in schedule:
            # Try to move this group earlier if earlier slots exist
            for check_date in sorted(used_slots_by_day):
                if check_date >= date:
                    break
                available_slots = set(get_slots_for_day(check_date)) - used_slots_by_day.get(check_date, set())
                for new_slot in available_slots:
                    # Check if group can be scheduled here
                    violates = False
                    for course in group:
                        students = student_course_map.get(course, [])
                        for c2, d2 in course_to_date.items():
                            if s in student_course_map.get(c2, []) and d2 == check_date:
                                violates = True
                                break
                        if violates:
                            break
                    ...
                    if not violates:
                        new_schedule.append((group, check_date, new_slot))
                        used_slots_by_day[check_date].add(new_slot)
                        course_to_date.update({c: check_date for c in group})
                        rebalanced = True
                        break
                if rebalanced:
                    break
            ...
            if not rebalanced:
                new_schedule.append((group, date, slot))
