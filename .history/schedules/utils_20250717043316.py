...
def find_compatible_courses_within_group(courses):
    """
    Greedy grouping + iterative merging + singleton insertion to find maximal compatible course groups.
    """
    if not courses:
        return []
    ...
    while remaining_courses:
        # Pick course with most compatible options
        course1 = max(
            remaining_courses,
    ...
    while merged:
        ...
        for i in range(len(course_groups)):
            for j in range(i+1, len(course_groups)):
    ...
    # Insert singleton courses into other groups if possible
    singletons = [g for g in course_groups if len(g) == 1]
    non_singletons = [g for g in course_groups if len(g) > 1]
    
    for singleton_group in singletons:
        course = singleton_group[0]
        inserted = False
        for group in non_singletons:
            if all(
                tuple(sorted([course, g_course])) not in conflict_matrix or conflict_matrix[tuple(sorted([course, g_course]))] == 0
                for g_course in group
            ):
                group.append(course)
                inserted = True
                break
        if inserted:
            course_groups.remove(singleton_group)
    
    # Optional: sort groups by size descending for neatness
