    ...
    with transaction.atomic():
        ...
        for date, slots in date_slot_students.items():
            for slot_name, slot_start, slot_end in SLOTS:
                ...
                remaining_students = [se for se in slot_student_exams if se.id not in assigned_ids]
                ...
                while remaining_students and room_index < len(rooms):
                    ...
                    current_occupancy = len(schedule[date][slot_name][room.id])
                    available = room.capacity - current_occupancy
                    ...
                    # Search for a compatible exam pair
                    for i in range(len(sorted_exams)):
                        exam1, students1 = sorted_exams[i]
                        students1 = [se for se in students1 if se.id not in assigned_ids]
                        if not students1:
                            continue
                        for j in range(i + 1, len(sorted_exams)):
                            exam2, students2 = sorted_exams[j]
                            students2 = [se for se in students2 if se.id not in assigned_ids]
                            if not students2:
                                continue
                    ...
                    if best_pair:
                        ...
                        for exam in [exam1, exam2]:
                            students = [se for se in remaining_students if se.exam == exam and se.id not in assigned_ids]
                            to_assign = students[:pair_size]
                            assigned.extend(to_assign)
                            for se in to_assign:
                                assigned_ids.add(se.id)
                                remaining_students.remove(se)
                    else:
                        # Assign a single exam
                        for exam, students in sorted_exams:
                            students = [se for se in students if se.id not in assigned_ids]
                            if students:
                    ...
                    if assigned:
                        print(len(schedule[date][slot_name][room.id]), len(assigned))

                        print(len(schedule[date][slot_name][room.id]) + len(assigned) <= room.capacity)
                        if len(schedule[date][slot_name][room.id]) + len(assigned) <= room.capacity:
                            schedule[date][slot_name][room.id].extend(assigned)
                        else:
                            # Split to fit room capacity
                            can_fit = room.capacity - len(schedule[date][slot_name][room.id])
                            schedule[date][slot_name][room.id].extend(assigned[:can_fit])
                            for se in assigned[:can_fit]:
                                assigned_ids.add(se.id)
                            for se in assigned[can_fit:]:
                                unaccommodated.append(se)
                    else:
                        break  # No assignment possible in this room

                # Leftover students
                unaccommodated.extend([se for se in remaining_students if se.id not in assigned_ids])
        ...
        # Fallback assignment
        fallback_candidates = StudentExam.objects.filter(
            id__in=[se.id for se in unaccommodated],
            room__isnull=True
        ).select_related('exam')

        for se in fallback_candidates:
            date = se.exam.date
            for slot_name, start, end in SLOTS:
                if se.exam.start_time == start and se.exam.end_time == end:
                    for room in rooms:
                        current = len(schedule[date][slot_name][room.id])
        ...
        # Return still unassigned student exams
        final_unassigned = StudentExam.objects.filter(
            room__isnull=True
        ).select_related('student')

        return list(final_unassigned.values_list('student', flat=True))
