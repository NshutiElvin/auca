...
def which_suitable_slot_to_schedule_course_group(date, new_group, suggested_slot):
    ...
    enrolled_students = Enrollment.objects.filter(group_id__in=new_group).values_list('student_id', flat=True)
    ...
    # check if the suggested slot is already occupied by more than 2 studen
