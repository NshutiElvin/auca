...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    with improved date utilization to minimize skipped days.
    """
    if not start_date:
    ...
    compatible_groups.sort(key=lambda g: len(g))
    ...
    with transaction.atomic():
        # First pass: Try to schedule each group on the earliest possible date
        unscheduled_groups = set(range(len(compatible_groups)))
        ...
        for date in sorted(slots_by_date.keys()):
            ...
            # Try all unscheduled groups for this date
            for group_idx in list(unscheduled_groups):
                ...
                for course_id in course_group:
                    ...
                    if course_preference in ["A", "B"]:
                        ...
                        if weekday == "Friday":
                            ...
                            # Assign based on group index for consistency
                            slot_choice = group_idx % 3
                            if slot_choice == 0:
                                start_time = time(8, 0)
                                end_time = time(11, 0)
                                slot = "Morning"
                ...
                if scheduled_this_group and group_exams:
                    ...
                    unscheduled_groups.remove(group_idx)
                    try:
                        ...
                        # Rollback this group's scheduling
                        for exam in group_exams:
                            exam.delete()
                        scheduled_groups.remove(group_idx)
                        unscheduled_groups.add(group_idx)
                        scheduled_this_group = False
                        continue
        
        # Second pass: Try to fill empty days with remaining groups
        remaining_groups = unscheduled_groups.copy()
        if remaining_groups:
            print(f"Attempting to schedule {len(remaining_groups)} remaining groups in empty slots")
            ...
            for date in sorted(slots_by_date.keys()):
                ...
                # Check if this date has any exams scheduled
                date_has_exams = any(exam.date == date for exam in exams_created)
                if date_has_exams:
                    continue
                
                # Try to schedule any remaining group on this empty date
                for group_idx in list(remaining_groups):
                    course_group = compatible_groups[group_idx]
                    group_exams = []
                    scheduled_this_group = True
                    ...
                    for course_id in course_group:
                        ...
                        # Time slot assignment (same as above)
                        if course_preference in ["A", "B"]:
                            start_time = time(8, 0)
                            end_time = time(11, 0)
                            slot = "Morning"
                    ...
                    if scheduled_this_group and group_exams:
                        ...
                        try:
                            unaccommodated = allocate_shared_rooms(group_exams)
                            print(f"✓ Rescheduled group {group_idx} on empty day {date}")
                        except Exception as e:
                            print(f"Room allocation error for group {group_idx}: {e}")
                            for exam in group_exams:
                                exam.delete()
                            scheduled_groups.remove(group_idx)
                            remaining_groups.add(group_idx)
                            continue
    
    # Final report
    if unscheduled_groups:
        print(f"Warning: Could not schedule {len(unscheduled_groups)} groups")
        for group_idx in unscheduled_groups:
            print(f"- Group {group_idx}: {compatible_groups[group_idx]}")
