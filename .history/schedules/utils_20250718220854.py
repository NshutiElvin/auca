...
def find_compatible_courses_within_group(courses):
    if not courses:
        return {"conflict_matrix": {}, "conflict_graph": defaultdict(list)}
    ...
    for enrollment in Enrollment.objects.filter(course_id__in=courses).iterator():
        student_courses[enrollment.student_id].append((enrollment.course_id, enrollment.group_id))
    ...
    # Initialize complete matrix (includes zero conflicts)
    conflict_matrix = {}
