...
def generate_exam_schedule(start_date=None, end_date=None,course_ids=None, semester=None):
 ...
 def allocate_shared_rooms(exams):
    ...
    with transaction.atomic():
        # First clear all existing room assignments for these exams
        exam_ids = [exam.id for student_exams in exams.values() for exam in student_exams]
        StudentExam.objects.filter(exam_id__in=exam_ids).update(room=None)
        ...
        if unaccommodated_students:
            ...
            for room in remaining_rooms:
                ...
                if available_seats > 0 and unaccommodated_students:
                    ...
                    # Get their exam information and update records
