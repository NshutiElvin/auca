...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    """
    Generate exam schedule by grouping all compatible courses and maximizing room usage
    """
    if not start_date:
    ...
    # Calculate minimum days needed (3 slots per day)
    ...
    date_slots = get_exam_slots(start_date, max_slots=min_days_needed * slots_per_day)
    ...
    with transaction.atomic():
        # Sort course groups by size (largest first)
        group_info = []
