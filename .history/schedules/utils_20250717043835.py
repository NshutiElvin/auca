...
def find_compatible_courses_within_group(courses):
    """
    Find maximal compatible groups by greedy grouping + merging + exhaustive relocation.

    Ensures no course can be moved to another group without conflicts.
    """
    if not courses:
        return []
    ...
    # Initial greedy grouping
    while remaining_courses:
        course1 = max(
            remaining_courses,
            key=lambda c: len([rc for rc in remaining_courses if rc != c and tuple(sorted([c, rc])) not in conflict_matrix])
        )
        group = [course1]
        remaining_courses.remove(course1)

        compatible_with_group = set(
            c for c in remaining_courses if all(
                tuple(sorted([c, g])) not in conflict_matrix or conflict_matrix[tuple(sorted([c, g]))] == 0
                for g in group
            )
        )
        ...
        while compatible_with_group:
            ...
            group.append(next_course)
            remaining_courses.remove(next_course)

            compatible_with_group = {
                c for c in compatible_with_group if c != next_course and
                (tuple(sorted([c, next_course])) not in conflict_matrix or conflict_matrix[tuple(sorted([c, next_course]))] == 0)
            } & remaining_courses

        course_groups.append(group)

    # Iterative merging of groups
    merged = True
    while merged:
        merged = False
        outer_len = len(course_groups)
        for i in range(outer_len):
            for j in range(i+1, outer_len):
                g1, g2 = course_groups[i], course_groups[j]
                if all(
                    tuple(sorted([c1, c2])) not in conflict_matrix or conflict_matrix[tuple(sorted([c1, c2]))] == 0
                    for c1 in g1 for c2 in g2
                ):
                    course_groups[i] = g1 + g2
                    course_groups.pop(j)
                    merged = True
                    break
            if merged:
    ...
    # Exhaustive relocation: try to move courses to other groups if possible
    changed = True
    while changed:
        changed = False
        # Iterate over groups and their courses
        for i in range(len(course_groups)):
            group = course_groups[i]
            for course in group[:]:  # iterate over copy because we may remove elements
                # Try to find a different group that can accept this course
                for j in range(len(course_groups)):
                    if i == j:
                        continue
                    target_group = course_groups[j]
                    # Check if course fits in target group
                    if all(
                        tuple(sorted([course, other])) not in conflict_matrix or conflict_matrix[tuple(sorted([course, other]))] == 0
                        for other in target_group
                    ):
                        # Move course to target group
                        group.remove(course)
                        target_group.append(course)
                        changed = True
                        break
                if changed:
                    break
        ...
        # Remove empty groups if any appeared
        course_groups = [g for g in course_groups if g]

    # Sort groups by descending size for neatness
