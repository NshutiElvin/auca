...
def find_compatible_courses_within_group(courses):
    """
    Graph-based approach to find maximal sets of compatible courses
    (no shared students).
    """
    if not courses:
        return []
    ...
    # Fill conflict matrix: each course pair that shares students
    ...
    # Create graph: nodes = courses, edges = conflicts
    G = nx.Graph()
    G.add_nodes_from(course_ids)
    for (c1, c2), count in conflict_matrix.items():
        if count > 0:
            G.add_edge(c1, c2)

    # Complement graph: edges now mean compatibility
    G_comp = nx.complement(G)

    # Find maximal cliques in the complement graph
    # Each clique = compatible group
    compatible_groups = list(nx.find_cliques(G_comp))
