...
def allocate_shared_rooms():
    ...
    with transaction.atomic():
        ...
        # Bucket the StudentExam objects by date & slot
        ...
        for date, slots in by_date_slot.items():
            for slot_name, _, _ in SLOTS:
                ...
                # Count how many students per exam
                # Use list of (exam, [StudentExam,...])
                exam_map = defaultdict(list)
                for se in bucket:
                    exam_map[se.exam].append(se)
                # Build mutable counters
                counts = {exam: list(students) for exam, students in exam_map.items()}

                # 3) Fill each room exactly once
                for room in rooms:
                    cap = room.capacity
                    if not any(counts.values()):
                        break  # nothing left to assign

                    # Pick the exam with the largest remaining count
                    primary_exam = max(counts, key=lambda e: len(counts[e]))
                    take_primary = min(len(counts[primary_exam]), cap)
                    primary_batch = counts[primary_exam][:take_primary]
                    counts[primary_exam] = counts[primary_exam][take_primary:]
                    cap -= take_primary

                    # Try to fill leftover cap with one compatible exam
                    secondary_batch = []
                    if cap > 0:
                        sem1 = int(primary_exam.group.course.semester.name.split()[1])
                        # Among remaining exams, pick the largest whose sem differs by 1
                        candidates = [
                            e for e, lst in counts.items()
                            if lst and abs(int(e.group.course.semester.name.split()[1]) - sem1) == 1
                        ]
                        if candidates:
                            second = max(candidates, key=lambda e: len(counts[e]))
                            take_sec = min(len(counts[second]), cap)
                            secondary_batch = counts[second][:take_sec]
                            counts[second] = counts[second][take_sec:]
                            cap -= take_sec

                    # Commit assignments for this room
                    for se in primary_batch + secondary_batch:
                        assignments.append((se.id, room.id))
                    
                # Any left in counts are truly unaccommodated
                for remaining_list in counts.values():
                    unaccommodated.extend(remaining_list)

        # 4) Bulk‐update DB
        # Set room for each assigned StudentExam
