...
def allocate_shared_rooms():
    # Get all unassigned student exams with related data, ordered by exam size
    student_exams = StudentExam.objects.filter(
        ...
        exam_size=Count('exam__studentexam')
    ).order_by('exam__date', 'exam__start_time', '-exam_size')
    ...
    with transaction.atomic():
        ...
        # Organize by date and slot
        date_slot_exams = defaultdict(lambda: defaultdict(list))
        for se in student_exams:
            for slot_name, start, end in SLOTS:
                if se.exam.start_time == start and se.exam.end_time == end:
                    date_slot_exams[se.exam.date][slot_name].append(se)
                    break

        for date, slots in date_slot_exams.items():
            for slot_name, slot_start, slot_end in SLOTS:
                slot_exams = slots.get(slot_name, [])
                if not slot_exams:
                    continue

                # Group by exam and sort by size (descending)
                exams = defaultdict(list)
                for se in slot_exams:
                    exams[se.exam].append(se)
                sorted_exams = sorted(exams.items(), key=lambda x: -len(x[1]))

                remaining_exams = sorted_exams.copy()
                room_index = 0

                while remaining_exams and room_index < len(rooms):
                    room = rooms[room_index]
                    room_capacity = room.capacity
                    room_index += 1

                    # Try to find the best pair of exams
                    best_pair = None
                    best_remaining = room_capacity

                    # Look for two exams that can share the room
                    for i, (exam1, students1) in enumerate(remaining_exams):
                        if not students1 or len(students1) > room_capacity:
                            continue
                        
                        sem1 = int(exam1.group.course.semester.name.split()[1])
                        
                        # Check if we can split this exam
                        if len(students1) > room_capacity//2:
                            # Try to split this exam and pair with another
                            for j, (exam2, students2) in enumerate(remaining_exams):
                                if i == j or not students2:
                                    continue
                                    
                                sem2 = int(exam2.group.course.semester.name.split()[1])
                                if abs(sem1 - sem2) > 1:
                                    total = len(students1) + len(students2)
                                    if total <= room_capacity:
                                        # Perfect fit
                                        best_pair = (i, j, len(students1), len(students2))
                                        best_remaining = 0
                                        break
                                    else:
                                        # Try partial fit
                                        remaining = room_capacity - len(students2)
                                        if remaining > 0 and remaining < len(students1):
                                            if remaining < best_remaining:
                                                best_pair = (i, j, remaining, len(students2))
                                                best_remaining = room_capacity - remaining - len(students2)
                            if best_remaining == 0:
                                break

                    if best_pair:
                        # Assign the best pair found
                        i, j, size1, size2 = best_pair
                        exam1, students1 = remaining_exams[i]
                        exam2, students2 = remaining_exams[j]
                        
                        # Assign students from both exams
                        assigned1 = students1[:size1]
                        assigned2 = students2[:size2]
                        
                        # Update remaining exams
                        remaining_exams[i] = (exam1, students1[size1:])
                        remaining_exams[j] = (exam2, students2[size2:])
                        
                        # Remove exams if no students left
                        if not remaining_exams[i][1]:
                            remaining_exams.pop(i)
                        if j < len(remaining_exams) and not remaining_exams[j][1]:
                            remaining_exams.pop(j)
                            
                        schedule[date][slot_name][room.id].extend(assigned1 + assigned2)
                    else:
                        # Assign single exam (full or partial)
                        exam, students = remaining_exams[0]
                        assigned = students[:room_capacity]
                        remaining_exams[0] = (exam, students[room_capacity:])
                        
                        if not remaining_exams[0][1]:
                            remaining_exams.pop(0)
                            
                        schedule[date][slot_name][room.id].extend(assigned)

                # Track unassigned students
                for exam, students in remaining_exams:
                    unaccommodated.extend(students)
        ...
        if unaccommodated:
            remaining_se = StudentExam.objects.filter(
                id__in=[se.id for se in unaccommodated]
            ).select_related('exam')
            
            for se in remaining_se:
                date = se.exam.date
                for slot_name, start, end in SLOTS:
                    if se.exam.start_time == start and se.exam.end_time == end:
