...
def generate_exam_schedule_with_day_off(start_date=None, course_ids=None, min_gap_days=2):
    """
    Generate exam schedule with day-off constraints properly enforced.
    """
    if not start_date:
    ...
    estimated_slots_needed = len(course_pairs) * 2  # Conservative estimate
...
def find_next_available_date(start_date, student_exam_dates, min_gap_days=2):
    """
    Find the next available date that satisfies day-off constraints for all students.
    
    Args:
        start_date: Date to start searching from
        student_exam_dates: Dict mapping student_id to list of their exam dates
        min_gap_days: Minimum gap between consecutive exams
    
    Returns:
        date: Next available date, or None if no suitable date found within reasonable range
    """
    current_date = start_date
    max_search_days = 30  # Reasonable limit to prevent infinite loops
    
    for _ in range(max_search_days):
        # Check if current_date is a valid exam day
        weekday = current_date.strftime('%A')
        if weekday in NO_EXAM_DAYS:
            current_date += timedelta(days=1)
            continue
        
        # Check if this date would violate day-off constraints for any student
        date_is_valid = True
        for student_id, exam_dates in student_exam_dates.items():
            if not has_sufficient_gap(exam_dates, current_date, min_gap_days):
                date_is_valid = False
                break
    ...
    return None  # No suitable date found


# Example usage with proper day-off enforcement
def schedule_with_day_off_example():
    """
    Example of how to use the corrected day-off implementation
    """
    try:
        exams, unaccommodated = generate_exam_schedule_with_day_off(
            start_date=now().date() + timedelta(days=1),
            min_gap_days=2  # 1 day off between exams
        )
        
        print(f"Successfully scheduled {len(exams)} exams")
        print(f"Unaccommodated students: {len(unaccommodated)}")
        
        # Verify the schedule maintains day-off constraints
        conflicts = verify_day_off_constraints()
        if conflicts:
            print(f"Day-off constraint violations found: {len(conflicts)}")
