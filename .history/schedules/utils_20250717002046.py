...
def find_compatible_courses(course_conflict_matrix):
    ...
    # Build compatibility graph (courses with zero conflicts)
    ...
    # NEW: Prioritize courses with MOST compatible partners first
    remaining_courses = set(all_courses)
    course_groups = []
    
    while remaining_courses:
        # Pick the course with the MOST compatible remaining courses
        course = max(
            remaining_courses,
        ...
        # Start a new group with this course
        group = {course}
        # Add ALL compatible courses to the group in one step
        compatible_partners = compatibility_graph[course] & remaining_courses
        for partner in list(compatible_partners):
            if all(
                tuple(sorted([partner, g])) not in course_conflict_matrix
                for g in group
            ):
                group.add(partner)
                remaining_courses.remove(partner)
        
        course_groups.append(list(group))
...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    for preference, courses in courses_by_preference.items():
        ...
        print("Group sizes:", [len(g) for g in compatible_groups])
