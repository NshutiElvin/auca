...
def allocate_shared_rooms(exams):
    ...
    # Helper function to check semester compatibility
    def can_pair_exams(exam1, exam2):
        """Check if two exams can be paired based on semester difference"""
        sem_diff = abs(int(exam1.course.semester.name.split(" ")[1]) - int(exam2.course.semester.name.split(" ")[1])) > 1
        return sem_diff >= 2
    
    # Group exams into pairs/groups for room sharing
    def group_exams_for_rooms(exams):
        """Group exams for sharing rooms with semester constraints"""
        exam_groups = []
        used_exams = set()
        
        # First, try to pair exams (exactly 2 per room)
        for i, exam1 in enumerate(exams):
            if exam1.id in used_exams:
                continue
                
            for j, exam2 in enumerate(exams[i+1:], i+1):
                if exam2.id in used_exams:
                    continue
                    
                if can_pair_exams(exam1, exam2):
                    exam_groups.append([exam1, exam2])
                    used_exams.add(exam1.id)
                    used_exams.add(exam2.id)
                    break
        
        # Handle remaining unpaired exams
        remaining_exams = [exam for exam in exams if exam.id not in used_exams]
        
        # If we have remaining exams and all rooms are not fully booked yet,
        # we need to check if we can accommodate them
        if remaining_exams:
            # Try to group remaining exams if possible
            remaining_groups = []
            used_remaining = set()
            
            # Try to create groups of compatible exams
            for i, exam1 in enumerate(remaining_exams):
                if exam1.id in used_remaining:
                    continue
                    
                compatible_group = [exam1]
                used_remaining.add(exam1.id)
                
                for j, exam2 in enumerate(remaining_exams[i+1:], i+1):
                    if exam2.id in used_remaining:
                        continue
                        
                    # Check if exam2 is compatible with all exams in current group
                    if all(can_pair_exams(exam2, group_exam) for group_exam in compatible_group):
                        compatible_group.append(exam2)
                        used_remaining.add(exam2.id)
                
                if compatible_group:
                    remaining_groups.append(compatible_group)
            
            # Add remaining groups to exam_groups
            exam_groups.extend(remaining_groups)
        
        return exam_groups
    
    # Group exams for room sharing
    exam_groups = group_exams_for_rooms(exams)
    
    # Check if all rooms will be fully booked with current grouping
    total_groups = len(exam_groups)
    rooms_needed = total_groups
    
    all_student_exams = []
    room_assignments = {}  # Track which exams are in which rooms
    
    # Track room utilization for redistribution
    room_utilization = {}  # room_id: {capacity_used, total_capacity}
    excess_students = []  # List of (student, exam) tuples that need redistribution
    
    # Initial allocation to primary rooms
    for group_idx, exam_group in enumerate(exam_groups):
        if group_idx >= len(rooms):
            # No more rooms available, add students to excess for redistribution
            for exam in exam_group:
                for enrollment in students_by_exam[exam.id]:
                    excess_students.append((enrollment.student, exam))
            continue
        
        room = rooms[group_idx]
        
        # Calculate capacity per exam in this group
        exams_in_group = len(exam_group)
        
        if exams_in_group <= 2:
            # Exactly 2 exams per room (or 1 if unpaired), split capacity equally
            capacity_per_exam = room.capacity // exams_in_group
        else:
            # More than 2 exams (only if all rooms are fully booked)
            capacity_per_exam = room.capacity // exams_in_group
        
        room_capacity_used = 0
        
        # Allocate students from each exam to this room
        for exam in exam_group:
            exam_students = students_by_exam[exam.id]
            
            # Limit students to available capacity
            allocated_students = exam_students[:capacity_per_exam]
            unallocated_students = exam_students[capacity_per_exam:]
            
            # Add excess students for redistribution
            for enrollment in unallocated_students:
                excess_students.append((enrollment.student, exam))
            
            # Create StudentExam objects for allocated students
            for enrollment in allocated_students:
            ...
            room_capacity_used += len(allocated_students)
        
        # Track room utilization
        room_utilization[room.id] = {
            'capacity_used': room_capacity_used,
            'total_capacity': room.capacity,
            'available_capacity': room.capacity - room_capacity_used
        }
        
        # Track room assignment
        room_assignments[room.id] = exam_group
    
    # Redistribute excess students to available rooms
    for student, exam in excess_students:
        redistributed = False
        
        # Try to find a room with available capacity
        for room in rooms:
            if room.id in room_utilization:
                available_capacity = room_utilization[room.id]['available_capacity']
                ...
                if available_capacity > 0:
                    ...
                    room_utilization[room.id]['capacity_used'] += 1
                    room_utilization[room.id]['available_capacity'] -= 1
                    
                    redistributed = True
                    break
        
        # If no room has capacity, add to unaccommodated list
        if not redistributed:
            unaccommodated_students.append(student)
    
    # Shuffle for random seating within rooms
