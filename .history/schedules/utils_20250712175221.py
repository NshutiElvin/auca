...
def allocate_shared_rooms(exams):
    ...
    # Group exams into pairs for each room, respecting semester gap
    exam_pairs = []
    used = set()
    ...
    # Assign rooms: 2 exams per room by default, fallback to more if needed
    all_student_exams = []
    room_index = 0
    for pair in exam_pairs:
        if room_index >= len(rooms):
            # Fallback: reuse rooms, possibly with more than 2 exams per room
            room_index = 0
        room = rooms[room_index]
        num_exams = len(pair)
        capacity_per_exam = room.capacity // num_exams if num_exams > 0 else 0
        for exam in pair:
            students = students_by_exam.get(exam.id, [])
            assigned = students[:capacity_per_exam]
            unassigned = students[capacity_per_exam:]
            ...
            if unassigned:
                unaccommodated_students.extend([e.student for e in unassigned])
        room_index += 1
