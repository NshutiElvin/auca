...
def analyze_student_course_conflicts():
    ...
    for enrollment in Enrollment.objects.all().distinct():
...
def find_compatible_courses(course_conflict_matrix):
    ...
    # Step 3: Initial greedy grouping
    ...
    while remaining_courses:
        ...
        compatible_with_group = compatibility_graph[course1] & remaining_courses
        ...
        while compatible_with_group:
            ...
            compatible_with_group &= compatibility_graph[next_course]
            compatible_with_group &= remaining_courses

        course_groups.append(course_group)

    # Step 4: Back-check: try to insert missed courses into earlier groups
    all_grouped = set()
    for group in course_groups:
        all_grouped.update(group)
    ungrouped_courses = all_courses - all_grouped

    for course in ungrouped_courses:
        for group in course_groups:
            if all((tuple(sorted([course, g])) not in course_conflict_matrix or course_conflict_matrix[tuple(sorted([course, g]))] == 0) for g in group):
                group.append(course)
                break
        else:
...
def find_compatible_courses_within_group(courses):
    """
    Improved version using graph coloring for better conflict separation
    """
    if not courses:
        return []
    ...
    for student_id, student_course_ids in student_courses.items():
        # Add edges between all pairs of courses this student is taking
        for i, c1 in enumerate(student_course_ids):
            for c2 in student_course_ids[i+1:]:
                G.add_edge(c1, c2)
    
    # Use graph coloring to find compatible groups
    coloring = nx.greedy_color(G, strategy='largest_first')
    
    # Group courses by color
    color_groups = defaultdict(list)
    for course_id, color in coloring.items():
...
#     - Greedy initial groups
#     - Merge compatible groups
#     - Iteratively reassign singletons/small groups to larger groups if possible
#     """
...
#                 conflict_matrix[pair] += 1

#     def no_conflict_between_groups(g1, g2):
#         return all(
#             tuple(sorted([c1, c2])) not in conflict_matrix or conflict_matrix[tuple(sorted([c1, c2]))] == 0
#             for c1 in g1 for c2 in g2
#         )
    
#     # Step 1: Greedy grouping
#     remaining = set(course_ids)
#     groups = []
#     while remaining:
#         # Pick course with max compatible remaining
#         c1 = max(
#             remaining,
#             key=lambda c: len([
#                 rc for rc in remaining if rc != c and 
#                 (tuple(sorted([c, rc])) not in conflict_matrix or conflict_matrix[tuple(sorted([c, rc]))] == 0)
#             ])
#         )
#         group = [c1]
#         remaining.remove(c1)
        
#         compatible = {c for c in remaining if all(
#             (tuple(sorted([c, gc])) not in conflict_matrix or conflict_matrix[tuple(sorted([c, gc]))] == 0)
#             for gc in group
#         )}
...
#             compatible = {
#                 c for c in compatible if c != next_c and
#                 (tuple(sorted([c, next_c])) not in conflict_matrix or conflict_matrix[tuple(sorted([c, next_c]))] == 0)
#             } & remaining
        
#         groups.append(group)

#     # Step 2: Merge groups that can be merged without conflict
#     merged = True
#     while merged:
#         merged = False
#         for i in range(len(groups)):
#             for j in range(i + 1, len(groups)):
#                 if no_conflict_between_groups(groups[i], groups[j]):
...
#     # Step 3: Iterative back-check reassignment of small groups / singletons
#     # Try moving courses from smaller groups into bigger ones if possible
#     changed = True
#     while changed:
#         changed = False
#         # Sort groups by size ascending to focus on small groups first
#         groups.sort(key=len)
        
#         i = 0
#         while i < len(groups):
#             group = groups[i]
#             if len(group) == 1:  # Singleton course?
#                 c = group[0]
#                 # Try to place c into any other group without conflict
#                 placed = False
...
#                     if all(
#                         (tuple(sorted([c, tc])) not in conflict_matrix or conflict_matrix[tuple(sorted([c, tc]))] == 0)
#                         for tc in target_group
#                     ):
#                         target_group.append(c)
#                         groups.pop(i)
#                         changed = True
#                         placed = True
#                         break
...
def can_schedule_course_group_on_date(course_group, proposed_date, student_exam_dates, min_gap_days=2):
    """
    Check if ALL courses in a group can be scheduled on proposed_date
    without causing same-day conflicts or violating gap constraints
    for ANY student
    """
    conflicts = []
    student_conflicts = set()
    
    # Get all students in the entire course group
    ...
    for course_id in course_group:
        ...
        group_student_ids.update(course_students)
    
    # Check each student's existing exams
...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    for i, course_group in enumerate(all_course_groups):
        ...
        group_difficulties.append((i, total_students))
    
    # Sort by difficulty (most students first)
    group_difficulties.sort(key=lambda x: x[1], reverse=True)
    
    with transaction.atomic():
        # Schedule by date first
        for date in sorted(slots_by_date.keys()):
            for slot_idx, label, start_time, end_time in date_slots:
                possible_groups = []
                for group_idx, total_students in group_difficulties:
                    if group_idx in assigned_groups:
                        continue
                        
                    course_group = all_course_groups[group_idx]
                    
                    # Use the enhanced conflict check for the entire group
                    can_schedule, _, _ = can_schedule_course_group_on_date(
                        course_group, date, student_exam_dates
                    )
                    
                    if can_schedule:
                        possible_groups.append((group_idx, total_students))
                
                if not possible_groups:
                    continue
                
                # Schedule the group with most students first
                possible_groups.sort(key=lambda x: x[1], reverse=True)
                group_idx, _ = possible_groups[0]
                course_group = all_course_groups[group_idx]
                ...
                assigned_groups.add(group_idx)
                
                # Allocate rooms
                try:
                    unaccommodated = allocate_shared_rooms(group_exams)
                    print(f"Scheduled group {course_group} on {date} {label}")
                except Exception as e:
                    print(f"Room allocation error: {e}")
        
        # Handle any remaining unscheduled groups
        unscheduled = set(range(len(all_course_groups))) - assigned_groups
        if unscheduled:
            print(f"Warning: Could not schedule {len(unscheduled)} groups")
            for group_idx in unscheduled:
                print(f"- Group {all_course_groups[group_idx]} (preference: {group_preferences[group_idx]})")
