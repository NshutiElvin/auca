...
def find_compatible_courses(course_conflict_matrix):
    ...
    for course1, course2 in course_conflict_matrix.keys():
        all_courses.add(course1)
        all_courses.add(course2)
    enrolled_courses = Course.objects.annotate(enrollment_count=Count('enrollments')).filter(enrollment_count__gt=0)
    for course in enrolled_courses.values_list('id', flat=True):
        all_courses.add(course)

    # Load semester info for all courses
    course_semester = {c.id: c.semester for c in Course.objects.filter(id__in=all_courses)}
    ...
    for course1 in all_courses:
        for course2 in all_courses:
            if course1 != course2:
                pair = tuple(sorted([course1, course2]))
                semester1 = course_semester[course1]
                semester2 = course_semester[course2]
                # Compatible if no shared students and semesters are not adjacent
                if (pair not in course_conflict_matrix or course_conflict_matrix[pair] == 0) and \
                   abs(int(semester1.name.split(" ")[1] )- int(semester2.name.split(" ")[1] ))> 1:
                    compatibility_graph[course1].add(course2)
...
def detect_scheduling_conflicts(course_pairs, student_assignments, date_slots):
    ...
    slots_by_date = defaultdict(list)
    for slot_idx, (date, _, _, _) in enumerate(date_slots):
        slots_by_date[date].append(slot_idx)
    ...
    for student_id, assignments in student_assignments.items():
        if len(assignments) > 1:
            for i, (pair_idx1, _) in enumerate(assignments):
                for pair_idx2, _ in assignments[i+1:]:
                    for date, slot_indices in slots_by_date.items():
                        for slot_idx1 in slot_indices:
                            for slot_idx2 in slot_indices:
                                conflict_scores[pair_idx1][slot_idx1] += 100
                                conflict_scores[pair_idx2][slot_idx2] += 100

    # Consecutive-day conflicts (initial check)
    dates = sorted(slots_by_date.keys())
    for i in range(len(dates) - 1):
        date1, date2 = dates[i], dates[i + 1]
        if (date2 - date1).days == 1:
            slots1 = slots_by_date[date1]
            slots2 = slots_by_date[date2]
            for student_id, assignments in student_assignments.items():
                if len(assignments) > 1:
                    for pair_idx1, _ in assignments:
                        for pair_idx2, _ in assignments:
                            if pair_idx1 != pair_idx2:
                                for s1 in slots1:
                                    for s2 in slots2:
                                        conflict_scores[pair_idx1][s1] += 100
                                        conflict_scores[pair_idx2][s2] += 100
...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    if course_ids:
        course_ids_set = set(course_ids)
        course_pairs = [[c for c in pair if c in course_ids_set] for pair in course_pairs if any(c in course_ids_set for c in pair)]
    ...
    with transaction.atomic():
        ...
        for pair_idx, _ in pair_difficulties:
            best_slot_idx = min(
                range(len(date_slots)),
                key=lambda s: conflict_scores[pair_idx][s] if s not in assigned_slots else float('inf')
            )
            if best_slot_idx in assigned_slots:
                raise ValueError("No available slot found without conflicts.")
            ...
            # Update conflict scores for consecutive days
            prev_date = exam_date - timedelta(days=1)
            next_date = exam_date + timedelta(days=1)
            prev_slots = slots_by_date[prev_date]
            next_slots = slots_by_date[next_date]

            for course_id in pair:
                students = Enrollment.objects.filter(course_id=course_id).values_list('student_id', flat=True)
                for student_id in students:
                    other_pairs = [p_idx for p_idx, _ in student_assignments[student_id] if p_idx != pair_idx and p_idx not in assigned_pairs]
                    for other_p_idx in other_pairs:
                        for s_idx in prev_slots + next_slots:
                            conflict_scores[other_p_idx][s_idx] += 100

            assigned_pairs.add(pair_idx)
            unaccommodated = allocate_shared_rooms(pair_exams)
            if unaccommodated:
                raise ValueError(f"Room allocation failed for {len(unaccommodated)} students.")
...
def allocate_shared_rooms(exams):
    ...
    total_students = sum(Enrollment.objects.filter(course=exam.course).count() for exam in exams)
    total_capacity = sum(room.capacity for room in rooms)
    if total_students > total_capacity:
        raise ValueError(f"Insufficient room capacity: {total_students} students, {total_capacity} seats.")

    # Pair exams (default to 2 per room)
    exam_pairs = []
    for i in range(0, len(exams), 2):
        pair = exams[i:i+2]
        exam_pairs.append(pair)
    ...
    for pair in exam_pairs:
        students = []
        course_to_exam = {exam.course.id: exam for exam in pair}
        pair_courses = list(course_to_exam.keys())
        enrollments = Enrollment.objects.filter(course__in=pair_courses)

        for enrollment in enrollments:
            exam = course_to_exam[enrollment.course_id]
            students.append((enrollment.student, exam))

        pair_students = len(students)
        if pair_students == 0:
            continue

        room_capacity = rooms[0].capacity  # Assume uniform capacity
        num_rooms_needed = (pair_students + room_capacity - 1) // room_capacity

        if room_idx + num_rooms_needed > len(rooms):
            # Fallback: assign remaining exams to available rooms
            remaining_exams = [e for p in exam_pairs[exam_pairs.index(pair):] for e in p]
            return assign_remaining_exams(remaining_exams, rooms[room_idx:], student_exams)

        pair_rooms = rooms[room_idx:room_idx + num_rooms_needed]
        room_idx += num_rooms_needed

        random.shuffle(students)
        for i, (student, exam) in enumerate(students):
            room = pair_rooms[i % num_rooms_needed]
            se = StudentExam(student=student, exam=exam, room=room)
            student_exams.append(se)

    StudentExam.objects.bulk_create(student_exams)
    return []

def assign_remaining_exams(exams, available_rooms, student_exams):
    course_to_exam = {exam.course.id: exam for exam in exams}
    enrollments = Enrollment.objects.filter(course__in=course_to_exam.keys())
    students = [(e.student, course_to_exam[e.course_id]) for e in enrollments]
    random.shuffle(students)

    for i, (student, exam) in enumerate(students):
        if i >= len(available_rooms):
            return [s.student for s, _ in students[i:]]
...
# ... (Include other existing functions like allocate_single_exam_rooms, cancel_exam, etc., unchanged)