...
def can_schedule_course_group_on_slot(course_group, proposed_date, slot_info, student_exam_dates):
    """
    Enhanced version with detailed conflict reporting
    """
    conflicts = []
    slot_label, start_time, end_time = slot_info
    
    # Check day of week constraints
    weekday = proposed_date.strftime('%A')
    if weekday in NO_EXAM_DAYS:
        conflicts.append("Exam cannot be scheduled on a weekend")
        return False, conflicts
    
    # Check Friday slot constraints
    if weekday == 'Friday' and slot_label not in ['Morning', 'Afternoon']:
        conflicts.append("Friday only allows morning and afternoon slots")
        return False, conflicts
    ...
    for course_id in course_group:
        ...
        for student_id in student_ids:
            ...
            if proposed_date in current_exam_dates:
                student_conflicts[student_id].append("Same-day conflict")
                continue
            
            # Minimum gap constraint
            if not has_sufficient_gap(current_exam_dates, proposed_date):
                student_conflicts[student_id].append("Insufficient gap between exams")
    
    if student_conflicts:
        conflicts.append(f"{len(student_conflicts)} student conflicts")
        # Include sample conflicts in the report
        sample_conflicts = list(student_conflicts.items())[:3]
        for student_id, reasons in sample_conflicts:
            conflicts.append(f"Student {student_id}: {', '.join(reasons)}")
    ...
    if total_students > total_capacity:
        conflicts.append(
            f"Insufficient room capacity ({total_students} students, {total_capacity} capacity)"
        )
...
def get_exam_slots(start_date, max_slots=None):
    ...
    return date_slots
def split_and_schedule(course_group, date_slots, student_exam_dates, assigned_slots):
    """
    Attempt to split a problematic course group into smaller compatible subgroups
    """
    # 1. Find the most conflicting courses in the group
    conflict_counts = defaultdict(int)
    for i, course1 in enumerate(course_group):
        for course2 in course_group[i+1:]:
            # Check if these courses share students
            common_students = Enrollment.objects.filter(
                course_id=course1,
                student__in=Enrollment.objects.filter(course_id=course2)
                    .values_list('student', flat=True)
            ).count()
            
            if common_students > 0:
                conflict_counts[course1] += common_students
                conflict_counts[course2] += common_students
    
    # 2. Split the group by removing the most conflicting courses
    if conflict_counts:
        most_conflicting = max(conflict_counts.items(), key=lambda x: x[1])[0]
        subgroup = [c for c in course_group if c != most_conflicting]
        
        print(f"Splitting group by removing most conflicting course {most_conflicting}")
        print(f"New subgroup: {subgroup}")
        
        # Try scheduling the subgroup
        for slot_idx, (date, label, start_time, end_time) in enumerate(date_slots):
            ...
            if can_schedule:
                ...
                try:
                    unaccommodated = allocate_shared_rooms_optimized(subgroup_exams)
                    print(f"Scheduled subgroup {subgroup} on {date} {label}")
                    if unaccommodated:
                        print(f"  Warning: {len(unaccommodated)} students unaccommodated")
                    
                    # Now try to schedule the removed course separately
                    print(f"Attempting to schedule remaining course {most_conflicting}")
                    for slot_idx2, (date2, label2, start_time2, end_time2) in enumerate(date_slots):
                        if slot_idx2 in assigned_slots:
                            continue
                        
                        can_schedule2, _ = can_schedule_course_group_on_slot(
                            [most_conflicting], date2, (label2, start_time2, end_time2), student_exam_dates
                        )
                        
                        if can_schedule2:
                            course = Course.objects.get(id=most_conflicting)
                            exam = Exam.objects.create(
                                course=course,
                                date=date2,
                            ...
                            student_ids = Enrollment.objects.filter(course=course).values_list('student_id', flat=True)
                            for student_id in student_ids:
                                student_exam_dates[student_id].append(date2)
                                student_exam_dates[student_id].sort()
                            
                            assigned_slots.add(slot_idx2)
                            allocate_shared_rooms_optimized([exam])
                            print(f"Scheduled remaining course {most_conflicting} on {date2} {label2}")
                            return True
                    
                    print(f"Could not schedule remaining course {most_conflicting}")
                    # Rollback the subgroup if we can't schedule the remaining course
                    for exam in subgroup_exams:
                        exam.delete()
                    return False
                
                except Exception as e:
                    print(f"Room allocation error for subgroup: {e}")
                    for exam in subgroup_exams:
                        exam.delete()
                    return False
        
        print(f"Could not schedule subgroup {subgroup}")
        return False
    else:
        print("No conflicts found within group - likely a room capacity issue")
        return False
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    if not start_date:
        start_date = now().date() + timedelta(days=1)
    
    # 1. Group courses by compatibility and preference
    course_conflict_matrix = analyze_student_course_conflicts()
    compatible_course_groups = find_compatible_courses(course_conflict_matrix)
    
    # 2. Sort groups by difficulty (largest groups first)
    compatible_course_groups.sort(
    ...
    # 3. Generate enough slots (3 per day)
    estimated_slots_needed = len(compatible_course_groups) * 2  # Extra buffer
    ...
    with transaction.atomic():
        # 4. Schedule each compatible group
        for group_idx, course_group in enumerate(compatible_course_groups):
            scheduled = False
            ...
            for slot_idx, (date, label, start_time, end_time) in enumerate(date_slots):
                ...
                # Check if this slot works for all courses in group
                ...
                if can_schedule:
                    ...
                    try:
                        unaccommodated = allocate_shared_rooms_optimized(group_exams)
                        print(f"Scheduled group {course_group} on {date} {label} ({start_time}-{end_time})")
                        if unaccommodated:
                            print(f"  Warning: {len(unaccommodated)} students unaccommodated")
                    except Exception as e:
                        print(f"  Room allocation error: {e}")
                        # Rollback this group's exams if room allocation fails
            ...
            if not scheduled:
                print(f"Could not schedule group {course_group}")
                problematic_groups.append({
                    'group': course_group,
                    'size': sum(Enrollment.objects.filter(course_id__in=course_group).count()),
                    'conflicts': conflicts if 'conflicts' in locals() else []
                })
    
    # 5. Handle problematic groups (like [108, 20, 45, 5, 74, 88])
    if problematic_groups:
        print("\nProblematic groups that couldn't be scheduled:")
        for group in problematic_groups:
            print(f"- Group: {group['group']} (size: {group['size']})")
            if group['conflicts']:
                print("  Conflicts:")
                for conflict in group['conflicts'][:3]:  # Show first 3 conflicts
                    print(f"  - {conflict}")
        
        # Try splitting problematic groups
        for group in problematic_groups:
            print(f"\nAttempting to split problematic group {group['group']}")
            split_and_schedule(group['group'], date_slots, student_exam_dates, assigned_slots)
    
    return exams_created, problematic_groups


def allocate_shared_rooms_optimized(exams):
    """
    Optimized room allocation that maximizes room utilization
    """
    if not exams:
        return []
    ...
    exam_students = {}
    for exam in exams:
        exam_students[exam.id] = list(
            Enrollment.objects.filter(course=exam.course)
            .select_related('student')
            .order_by('?')  # Randomize to prevent patterns
        )

    # Calculate total students per exam
    exam_counts = {exam.id: len(students) for exam, students in exam_students.items()}
    
    # Sort exams by student count (descending)
    sorted_exams = sorted(exams, key=lambda e: -exam_counts[e.id])
    
    unaccommodated = []
    assignments = []
    
    # Pair exams with compatible semester gaps
    exam_pairs = []
    used_exams = set()
    
    for i, exam1 in enumerate(sorted_exams):
        if exam1.id in used_exams:
            continue
            
        # Find best partner exam
        best_partner = None
        best_gap = -1
        
        for j, exam2 in enumerate(sorted_exams[i+1:], i+1):
            if exam2.id in used_exams:
                continue
                
            gap = abs(int(exam1.course.semester.name.split(" ")[1]) - 
                     int(exam2.course.semester.name.split(" ")[1]))
            
            if gap > best_gap and gap >= 2:
                best_partner = exam2
                best_gap = gap
        
        if best_partner:
            exam_pairs.append((exam1, best_partner))
            used_exams.update({exam1.id, best_partner.id})
        else:
            exam_pairs.append((exam1, None))
            used_exams.add(exam1.id)

    # Allocate rooms to exam pairs
    for pair in exam_pairs:
        if not rooms:
            # No rooms left - mark all students as unaccommodated
            for exam in pair:
                if exam:
                    unaccommodated.extend([e.student for e in exam_students[exam.id]])
            continue
            
        room = rooms.pop(0)
        total_capacity = room.capacity
        
        # Split capacity between exams
        exams_to_allocate = [e for e in pair if e]
        capacity_per_exam = total_capacity // len(exams_to_allocate)
        
        for exam in exams_to_allocate:
            students = exam_students[exam.id]
            accommodated = students[:capacity_per_exam]
            overflow = students[capacity_per_exam:]
            ...
            exam_students[exam.id] = overflow
    
    # Handle any remaining students in overflow
    for exam in exams:
        if exam_students.get(exam.id):
            unaccommodated.extend([e.student for e in exam_students[exam.id]])
    
    # Save all assignments
    StudentExam.objects.bulk_create(assignments)
...
#     estimated_slots_needed = len(all_course_groups) * 5
#     date_slots = get_exam_slots(start_date, max_slots=estimated_slots_needed)
