...
def allocate_shared_rooms():
    # Get all student exams that need room allocation
    ...
    with transaction.atomic():
        # First organize student exams by date and time slot
        exams_by_slot = defaultdict(lambda: defaultdict(list))
        
        for se in student_exams:
            for slot_name, start_time, end_time in SLOTS:
                if se.exam.start_time == start_time and se.exam.end_time == end_time:
                    exams_by_slot[se.exam.date][slot_name].append(se)
                    break

        # Track room usage by date and slot
        room_usage = defaultdict(lambda: defaultdict(set))  # {date: {slot: set(room_ids)}}

        for date, slots in exams_by_slot.items():
            for slot_name in ['Morning', 'Afternoon', 'Evening']:
                slot_student_exams = slots.get(slot_name, [])
                if not slot_student_exams:
                    continue

                # Get available rooms (not used in this slot yet)
                used_rooms = room_usage[date].get(slot_name, set())
                available_rooms = [r for r in rooms if r.id not in used_rooms]
                
                # Group student exams by exam
                students_by_exam = defaultdict(list)
                for se in slot_student_exams:
                    students_by_exam[se.exam.id].append(se)

                # Create exam pairs avoiding adjacent semesters
                exam_pairs = []
                used_exams = set()
                exam_list = list(students_by_exam.keys())
                
                for i in range(len(exam_list)):
                    if exam_list[i] in used_exams:
                        continue
                    exam1 = Exam.objects.get(id=exam_list[i])
                    sem1 = int(exam1.group.course.semester.name.split(" ")[1])
                    
                    for j in range(i + 1, len(exam_list)):
                        if exam_list[j] in used_exams:
                            continue
                        exam2 = Exam.objects.get(id=exam_list[j])
                        sem2 = int(exam2.group.course.semester.name.split(" ")[1])
                        
                        if abs(sem1 - sem2) > 1:
                            exam_pairs.append((exam1, exam2))
                            used_exams.update({exam1.id, exam2.id})
                            break
                    else:
                ...
                for pair in exam_pairs:
                    if room_index >= len(available_rooms):
                        break

                    room = available_rooms[room_index]
                    room_index += 1
                    room_usage[date][slot_name].add(room.id)
                    ...
                    for exam in exams_to_allocate:
                        exam_student_exams = students_by_exam[exam.id]
                        capacity_per_exam = room.capacity // len(exams_to_allocate)
                        to_assign = exam_student_exams[:capacity_per_exam]

                        # Update these student exams
                        student_ids = [se.student.id for se in to_assign]
                        StudentExam.objects.filter(
                            id__in=[se.id for se in to_assign]
                        ).update(room=room)
                        assigned_student_ids.update(student_ids)

                # Handle remaining exams (those not paired)
                remaining_exam_ids = [eid for eid in students_by_exam if eid not in used_exams]
                for exam_id in remaining_exam_ids:
                    if room_index >= len(available_rooms):
                        break
                    ...
                    exam_student_exams = students_by_exam[exam_id]
                    to_assign = exam_student_exams[:room.capacity]
                ...
                for exam_id in students_by_exam:
                    for se in students_by_exam[exam_id]:
                        if se.student.id not in assigned_student_ids:
                            unaccommodated_students.append(se.student)

        # Final pass to try placing unaccommodated students in any remaining seats
        if unaccommodated_students:
            # Get student exams for unaccommodated students
            remaining_student_exams = StudentExam.objects.filter(
                student__in=unaccommodated_students,
                room__isnull=True
            ).select_related('exam')
            ...
            for se in remaining_student_exams:
                ...
                for room in available_rooms:
                    ...
                    if current_usage < room.capacity:
                        ...
                        se.save()
                        try:
                            unaccommodated_students.remove(se.student)
                        except ValueError:
                            pass
                        break
