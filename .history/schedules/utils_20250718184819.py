...
# from .models import Course, CourseGroup, Enrollment, Exam, Room, StudentExam

logger = logging.getLogger(__name__)

# Group to timeslot preferences
GROUP_SLOT_PREFERENCES = {
    'A': ['Morning'],         # Group A strictly morning
    'B': ['Morning'],         # Group B strictly morning  
    'C': ['Morning', 'Afternoon'],  # Group C prefers morning
    'D': ['Afternoon', 'Morning'],  # Group D prefers afternoon
    'E': ['Evening', 'Afternoon'],  # Group E prefers evening
    'F': ['Evening']          # Group F strictly evening
}
...
class ExamScheduler:
    ...
    def generate_schedule(self, start_date, end_date):
        """Main method to generate the complete exam schedule"""
        try:
            with transaction.atomic():
                # 1. Find compatible course groups
                course_groups = self.find_compatible_course_groups()
                
                # 2. Generate time slots
                time_slots = self.generate_time_slots(start_date, end_date)
                
                # 3. Allocate exams based on group preferences
                self.allocate_by_group_name(course_groups, time_slots)
                
                # 4. Assign rooms
                room_results = self.assign_rooms()
                
                return {
                    'success': True,
                    'exams': self.scheduled_exams,
            ...
            return {
                'success': False,
                'error': str(e),
                'partial_schedule': self.scheduled_exams
            }

    def find_compatible_course_groups(self):
        """Find groups of courses that can be scheduled together"""
        ...
        for student, courses in student_courses.items():
            for i in range(len(courses)):
                for j in range(i+1, len(courses)):
                    conflict_matrix[courses[i]][courses[j]] += 1
                    conflict_matrix[courses[j]][courses[i]] += 1
        
        # Graph coloring to find compatible groups
        graph = defaultdict(set)
        all_courses = set(conflict_matrix.keys())
        
        for course1 in all_courses:
            for course2 in conflict_matrix[course1]:
                if conflict_matrix[course1][course2] > 0:
        ...
        courses_ordered = sorted(all_courses, 
                               key=lambda c: len(graph[c]), 
                               reverse=True)
        
        for course in courses_ordered:
            # Reset available colors
            available_colors = list(range(len(all_courses)))
            ...
            if available_colors:
                color_map[course] = available_colors[0]
            else:
                color_map[course] = len(color_map)
    ...
    def generate_time_slots(self, start_date, end_date):
        """Generate available time slots between dates"""
        slots = []
        current_date = start_date
        ...
        while current_date <= end_date:
            ...
            if weekday == 'Friday':
                # Friday slots (morning and afternoon only)
                slots.append((current_date, 'Morning', time(8,0), time(11,0)))
                slots.append((current_date, 'Afternoon', time(13,0), time(16,0)))
            else:
                # Regular weekday slots
                slots.append((current_date, 'Morning', time(8,0), time(11,0)))
                slots.append((current_date, 'Afternoon', time(13,0), time(16,0))) 
                slots.append((current_date, 'Evening', time(17,0), time(20,0)))
                
            current_date += timedelta(days=1)
            
        return slots

    def allocate_by_group_name(self, course_groups, time_slots):
        """Allocate course groups to timeslots based on group names"""
        # Sort by enrollment size (descending)
        course_groups.sort(
            key=lambda g: -max(Enrollment.objects.filter(course_id=c).count() for c in g),
            reverse=True
        )
        
        for group in course_groups:
            group_prefs = self.get_group_preferences(group)
            placed = False
            
            # Try slots in order of preference
            for pref_slot in group_prefs['preferred_slots']:
                if placed:
                    break
                    
                # Find all matching slots
                matching_slots = [s for s in time_slots if s[1] == pref_slot]
                
                for slot in matching_slots:
                    date, slot_label, start_time, end_time = slot
                    
                    # Check constraints
                    conflict_reasons = self.check_group_constraints(
                        group, date, slot_label
                    )
                    
                    if not conflict_reasons:
                        self.create_exams(group, group_prefs['group_names'], 
            ...
            if not placed:
                self.conflicts.append({
                    'courses': group,
                    'groups': group_prefs['group_names'],
                    'reason': "No suitable slot found respecting all constraints"
                })

    def get_group_preferences(self, course_group):
        """Determine group preferences for a course group"""
        group_names = set()
        slot_preferences = defaultdict(int)
        
        for course_id in course_group:
            groups = CourseGroup.objects.filter(course_id=course_id)
            for group in groups:
                if group.group_name in GROUP_SLOT_PREFERENCES:
                    group_names.add(group.group_name)
                    for pref in GROUP_SLOT_PREFERENCES[group.group_name]:
                        slot_preferences[pref] += 1
        
        # Determine dominant preference
        if not slot_preferences:
            preferred_slots = ['Morning', 'Afternoon', 'Evening']
        else:
            preferred_slots = sorted(
                slot_preferences.keys(),
                key=lambda x: -slot_preferences[x]
            )
    ...
    def check_group_constraints(self, course_group, date, slot_label):
        """Check all scheduling constraints"""
        conflicts = []
        weekday = date.strftime('%A')
        
        # 1. Check day of week constraints
        if weekday == 'Saturday':
            return ["Saturday exams not allowed"]
        
        # 2. Check Friday slot constraints
        if weekday == 'Friday' and slot_label == 'Evening':
            return ["Friday evening exams not allowed"]
        
        # 3. Check student constraints
        for course_id in course_group:
            enrollments = Enrollment.objects.filter(
                course_id=course_id,
                group__group_name__in=GROUP_SLOT_PREFERENCES.keys()
            ).select_related('student')
            
            for enrollment in enrollments:
                student_id = enrollment.student_id
                
                # Check same-day exams
                if date in self.student_schedule.get(student_id, []):
                    conflicts.append(
                        f"Student {student_id} already has exam on {date}"
                    )
                
                # Check day-off constraint
                next_day = date + timedelta(days=1)
                if next_day in self.student_schedule.get(student_id, []):
                    conflicts.append(
                        f"Student {student_id} has exam the day after"
                    )
        
        return conflicts

    def create_exams(self, course_ids, group_names, date, start_time, end_time):
        """Create exam records for a course group"""
        exams = []
        ...
        for course_id in course_ids:
            ...
            if group:
                ...
                for e in enrollments:
                    self.student_schedule[e.student_id].append(date)
        
        self.scheduled_exams.extend(exams)
        return exams
    def assign_rooms(self):
        """Assign rooms to scheduled exams with proper student records"""
        results = {
            'assignments': [],
            'unaccommodated': []
        }
        ...
        slot_exams = defaultdict(list)
        for exam in self.scheduled_exams:
            slot_key = (exam.date, exam.start_time, exam.end_time)
            slot_exams[slot_key].append(exam)
        
        # Process each time slot
        for slot, exams in slot_exams.items():
            rooms = list(Room.objects.order_by('-capacity'))
            
            # Get all enrollments for these exams
            exam_enrollments = defaultdict(list)
            for exam in exams:
                enrollments = Enrollment.objects.filter(
                    group=exam.group
                ).select_related('student')
                exam_enrollments[exam.id] = enrollments
            
            # Pair exams with their student lists
            exam_students = [
                (exam, list(exam_enrollments[exam.id]))
                for exam in exams
            ]
            
            # Sort exams by student count (descending)
            exam_students.sort(key=lambda x: -len(x[1]))
            
            # Assign using first-fit decreasing algorithm
            room_assignments = []
            remaining_rooms = rooms.copy()
            
            for exam, students in exam_students:
                placed = False
                student_count = len(students)
                
                # Try existing rooms first
                for assignment in room_assignments:
                    if assignment['remaining'] >= student_count:
                        # Create StudentExam records for all students
                        student_exams = [
                            StudentExam(
                        ...
                        assignment['exams'].append(exam)
                        assignment['remaining'] -= student_count
                        assignment['students'].extend(students)
                        placed = True
                        break
                
                # If not placed, try a new room
                if not placed and remaining_rooms:
                    room = remaining_rooms.pop(0)
                    if room.capacity >= student_count:
                        ...
                        room_assignments.append({
                            ...
                            'exams': [exam],
                            'students': students,
                            'remaining': room.capacity - student_count
                        })
                        placed = True
                
                if not placed:
                    results['unaccommodated'].append({
                        'exam': exam,
                        'students_affected': students
                    })
            
            results['assignments'].extend(room_assignments)
