...
def find_compatible_courses_within_group(courses):
    ...
    for enrollment in Enrollment.objects.filter(course_id__in=course_ids).iterator():
        ...
        # For courses_graph, we'll structure it as: {course_id: {group_id: [student_ids]}}
        if enrollment.group_id not in courses_graph[enrollment.course_id]:
            courses_graph[enrollment.course_id][enrollment.group_id] = []
