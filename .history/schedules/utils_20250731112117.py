...
def generate_exam_schedule(start_date=None, end_date=None, course_ids=None, semester=None):
    try:
        ...
        # Track seat usage per slot per day
        seat_usage = defaultdict(lambda: defaultdict(int))  # {date: {slot: count}}
        all_available_seats_per_slot = Room.objects.order_by('-capacity').aggregate(total=Sum('capacity'))['total'] or 0
        ...
        with transaction.atomic():
            ...
            for idx, course_group in enumerate(compatible_groups):
                ...
                total_students_in_group = 0
                slot_type = None
                
                # First pass: check if we can schedule this group without exceeding capacity
                for mycourse_group in courses:
                    ...
                    for group_id in course_groups:
                        ...
                        total_students_in_group += student_count
                        
                        # Determine slot type for this group
                        if group.group_name in ["A", "B"]:
                            slot_type = "Morning" if random.randint(1, 2) == 1 else "Afternoon"
                        elif group.group_name in ["C", "D"]:
                            if weekday == "Friday":
                                slot_type = "Afternoon"
                            else:
                                guess = random.randint(1, 3)
                                slot_type = "Morning" if guess == 1 else "Afternoon" if guess == 2 else "Evening"
                        elif group.group_name in ["E", "F"]:
                            slot_type = "Evening" if weekday != "Friday" else "Afternoon"
                        else:
                            slot_type = "Afternoon"
                
                # Check if adding this group would exceed capacity for the slot
                if (seat_usage[date].get(slot_type, 0) + total_students_in_group > all_available_seats_per_slot:
                    # Skip this group for now, leave it in myscheduled_groups
                    continue
                
                # Second pass: actually schedule the group
                for mycourse_group in courses:
                    course_id = mycourse_group["course_id"]
                    course_groups = mycourse_group["groups"]
                    course = Course.objects.get(id=course_id)
                    ...
                    for group_id in course_groups:
                        ...
                        try:
                            ...
                            # Update student exam dates and seat usage
                            for student_id in student_ids:
                            ...
                            # Update seat usage for this slot
                            seat_usage[date][slot_type] += len(student_ids)
                ...
                if can_schedule_group:
                    ...
                    # If we failed to schedule part of the group, roll back any exams created
                    continue
