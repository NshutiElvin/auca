...
def detect_scheduling_conflicts(course_pairs, student_assignments, date_slots):
    ...
    # Calculate conflicts
    for student_id, assignments in student_assignments.items():
        for (pair_idx1, course_id1), (pair_idx2, course_id2) in combinations(assignments, 2):
            # These two course pairs can't be scheduled on the same day
            for date, slot_indices in slots_by_date.items():
                for slot_idx1 in slot_indices:
                    for slot_idx2 in slot_indices:
                        conflict_scores[pair_idx1][slot_idx1] += 1
                        conflict_scores[pair_idx2][slot_idx2] += 1
            
            # NEW: Check for consecutive days
            for i in range(len(date_list) - 1):
                date1 = date_list[i]
                date2 = date_list[i+1]
                if abs((date2 - date1).days) == 1:  # Consecutive days
                    for slot_idx1 in slots_by_date[date1]:
                        for slot_idx2 in slots_by_date[date2]:
                            conflict_scores[pair_idx1][slot_idx1] += 1
                            conflict_scores[pair_idx2][slot_idx2] += 1
...
def allocate_shared_rooms(exams):
    ...
    # NEW: Group exams by semester and ensure semester gap
    exams_by_semester = defaultdict(list)
    for exam in exams:
        exams_by_semester[int(exam.course.semester.name.split(" ")[1])].append(exam)
    
    # Sort semesters to find compatible pairs
    sorted_semesters = sorted(exams_by_semester.keys())
    compatible_pairs = []
    
    # Find all possible compatible pairs with at least 1 semester gap
    for i, sem1 in enumerate(sorted_semesters):
        for sem2 in sorted_semesters[i+1:]:
            if abs(sem2 - sem1) > 1:  # At least one semester gap
                for exam1 in exams_by_semester[sem1]:
                    for exam2 in exams_by_semester[sem2]:
                        compatible_pairs.append((exam1, exam2))
    ...
    unaccommodated_students = []
    assigned_rooms = defaultdict(list)
    
    # First try to assign compatible pairs to rooms
    for room in rooms:
        if not compatible_pairs:
            break
            
        # Find the pair that fits best in this room
        best_pair = None
        best_fit = float('inf')
        
        for pair in compatible_pairs:
            exam1, exam2 = pair
            students1 = len(exam_students[exam1.id])
            students2 = len(exam_students[exam2.id])
            
            # Calculate how well this pair fits in the room
            total_students = students1 + students2
            if total_students > room.capacity:
                continue
                
            # Prefer pairs that divide the room more equally
            fit_score = abs(students1 - students2)
        ...
        if best_pair:
            ...
            students1 = exam_students[exam1.id]
            students2 = exam_students[exam2.id]
            
            # Divide room capacity proportionally
            total_students = len(students1) + len(students2)
            if total_students > room.capacity:
                # Need to reduce students
                cap1 = int((len(students1) / total_students) * room.capacity)
                cap2 = room.capacity - cap1
                
                if len(students1) > cap1:
                    unaccommodated_students.extend([s.student for s in students1[cap1:]])
                    students1 = students1[:cap1]
                if len(students2) > cap2:
                    unaccommodated_students.extend([s.student for s in students2[cap2:]])
                    students2 = students2[:cap2]
            
            # Assign students to room
            for student in students1 + students2:
                se = StudentExam(student=student.student, exam=exam1 if student in students1 else exam2, room=room)
                se.save()
            
            assigned_rooms[room.id].extend([exam1.id, exam2.id])
            # Remove this pair from consideration
            compatible_pairs = [p for p in compatible_pairs if p != best_pair]
    
    # Now handle remaining exams (couldn't be paired)
    remaining_exams = [exam for exam in exams if exam.id not in 
                      [eid for eids in assigned_rooms.values() for eid in eids]]
    
    if remaining_exams:
        # As fallback, assign multiple exams to a room (more than 2)
        for exam in remaining_exams:
            ...
            room_found = None
            for room in rooms:
                # Count already assigned students in this room
                assigned_count = StudentExam.objects.filter(room=room).count()
                remaining_capacity = room.capacity - assigned_count
                
                if remaining_capacity >= len(students):
                    room_found = room
                    break
            
            if room_found:
                for student in students:
                    se = StudentExam(student=student.student, exam=exam, room=room_found)
                    se.save()
            else:
                unaccommodated_students.extend([s.student for s in students])
