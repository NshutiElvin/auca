...
def allocate_shared_rooms():
    ...
    with transaction.atomic():
        # Explicit occupancy and schedule structures
        occupancy = defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
        schedule  = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
        assigned_ids   = set()
        unaccommodated = []

        # 2) Bucket exams by date & slot
        ...
        # 3) Main allocation pass
        for date, slots in by_date_slot.items():
            for slot_name, _, _ in SLOTS:
                bucket = [se for se in slots.get(slot_name, [])
                          if se.id not in assigned_ids]
                if not bucket:
                    continue

                # Group by exam and sort by size desc
                ...
                while bucket and room_idx < len(rooms):
                    room = rooms[room_idx]
                    room_idx += 1

                    # If this room already got ANY students in this slot, skip
                    if occupancy[date][slot_name][room.id] > 0:
                        continue

                    cap_left = room.capacity
                    best_pair = None
                    best_pair_size = 0

                    # Find best two‐exam pairing (semester ±1)
                    for i, (ex1, stud1) in enumerate(sorted_exams):
                        studs1 = [s for s in stud1 if s.id not in assigned_ids]
                        if not studs1: continue
                        for ex2, stud2 in sorted_exams[i+1:]:
                            studs2 = [s for s in stud2 if s.id not in assigned_ids]
                            if not studs2: continue
                            sem1 = int(ex1.group.course.semester.name.split()[1])
                            sem2 = int(ex2.group.course.semester.name.split()[1])
                            if abs(sem1 - sem2) == 1:
                                pair_sz = min(len(studs1), len(studs2), cap_left//2)
                                if pair_sz > best_pair_size:
                                    best_pair = (ex1, ex2, pair_sz)
                                    best_pair_size = pair_sz

                    to_assign = []
                    # Assign pairing if found
                    if best_pair:
                        e1, e2, cnt = best_pair
                        for exam in (e1, e2):
                            studs = [s for s in exams[exam] if s.id not in assigned_ids]
                            chunk = studs[:cnt]
                            to_assign.extend(chunk)
                    else:
                        # Otherwise assign one exam to fill room
                        for exam, studs in sorted_exams:
                            avail = [s for s in studs if s.id not in assigned_ids]
                            if not avail: 
                                continue
                            chunk = avail[:cap_left]
                            to_assign.extend(chunk)
                            break

                    # Actually schedule them
                    for se in to_assign:
                        if occupancy[date][slot_name][room.id] < room.capacity:
                            schedule[date][slot_name][room.id].append(se)
                            occupancy[date][slot_name][room.id] += 1
                            assigned_ids.add(se.id)
                            bucket.remove(se)
                        else:
                            unaccommodated.append(se)

                # Any still in bucket are unaccommodated
                unaccommodated.extend(bucket)

        # 4) Persist room assignments
        for date, slots in schedule.items():
            for slot_name, rooms_map in slots.items():
                for room_id, ses in rooms_map.items():
                    StudentExam.objects.filter(
                        id__in=[se.id for se in ses]
                    ).update(room_id=room_id)

        # 5) Fallback: try to fill gaps
        fallback_qs = StudentExam.objects.filter(
            id__in=[se.id for se in unaccommodated],
            room__isnull=True
        ).select_related('exam')
        for se in fallback_qs:
            d, st = se.exam.date, se.exam.start_time
            for slot_name, start, end in SLOTS:
                if st == start:
                    for room in rooms:
                        if occupancy[d][slot_name][room.id] < room.capacity:
                            occupancy[d][slot_name][room.id] += 1
                            se.room = room
                            se.save()
                            assigned_ids.add(se.id)
                            break
        ...
        # 6) Final overflow check (debug)
        for date, slots in occupancy.items():
            for slot, rooms_map in slots.items():
                for r_id, cnt in rooms_map.items():
                    cap = Room.objects.get(id=r_id).capacity
                    if cnt > cap:
                        print(f"[OVERFLOW] {date} {slot} room {r_id}: {cnt}/{cap}")

        # 7) Return students still without rooms
        return list(
