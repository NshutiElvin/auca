...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    day_usage = defaultdict(int)  # Track number of course pairs assigned to each day
    ...
    with transaction.atomic():
        ...
        for pair_idx, _ in pair_difficulties:
            best_slot_idx = min(
                ...
                key=lambda s: conflict_scores[pair_idx][s] + 5 * day_usage[date_slots[s][0]] if s not in assigned_slots else float('inf')
            )
            ...
            day_usage[exam_date] += 1  # Increment usage count for this day
            ...
            prev_slots = slots_by_date.get(prev_date, [])
            next_slots = slots_by_date.get(next_date, [])
