...
def assign_group_to_date(group_idx, compatible_groups, available_dates, student_exam_dates, assignment):
    ...
    for date in available_dates:
        if can_schedule_course_group_on_slot(course_group, date, student_exam_dates):
            # Assign group to this date
            assignment[group_idx] = date
            ...
            # Recursive call to assign next group
            if assign_group_to_date(group_idx + 1, compatible_groups, available_dates, student_exam_dates, assignment):
                return True

            # Backtrack: remove assignments
            for course_id in course_group:
                student_ids = Enrollment.objects.filter(course_id=course_id).values_list('student_id', flat=True)
                for student_id in student_ids:
                    student_exam_dates[student_id].remove(date)
            assignment.pop(group_idx)

    # No valid date found for this group, backtrack
    return False
...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    # Get enrolled courses and compatible groups as before
    enrolled_courses = course_ids if course_ids else Course.objects.annotate(
        enrollment_count=Count('enrollments')
    ).filter(enrollment_count__gt=0)
    compatible_groups = find_compatible_courses_within_group(enrolled_courses)

    # Generate available dates (skip Saturdays etc)
    max_slots = len(compatible_groups) * 2  # some buffer
    available_dates = []
    current_date = start_date
    while len(available_dates) < max_slots:
        if current_date.strftime('%A') != 'Saturday':  # skip Saturday
            available_dates.append(current_date)
        current_date += timedelta(days=1)

    # Backtracking assignment map: group_idx -> date
    assignment = {}
    student_exam_dates = defaultdict(list)

    success = assign_group_to_date(0, compatible_groups, available_dates, student_exam_dates, assignment)
    if not success:
        return [], "Unable to schedule all groups without conflicts."

    # Now create Exam objects in one transaction, assign time slots based on preferences and weekday
    exams_created = []
    with transaction.atomic():
        for group_idx, date in assignment.items():
            course_group = compatible_groups[group_idx]
            weekday = date.strftime('%A')
            for course_id in course_group:
                course = Course.objects.get(id=course_id)
                # Determine time slot (same logic as before)
                if course.group in ["A", "B"]:
                    start_time = time(8, 0)
                    end_time = time(11, 0)
                elif course.group in ["C", "D"]:
                    if weekday == "Friday":
                        start_time = time(13, 0)
                        end_time = time(16, 0)
                    else:
                        start_time = time(8, 0)  # simplified, can randomize if desired
                        end_time = time(11, 0)
                elif course.group in ["E", "F"]:
                    if weekday != "Friday":
    ...
    # Run room allocation if needed here
    try:
        allocate_shared_rooms(exams_created)
