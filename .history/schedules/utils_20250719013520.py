...
def generate_exam_schedule(start_date=None, end_date=None, course_ids=None, semester=None):
    try:
        ...
        if not start_date:
            ...
            start_date += timedelta(days=1)  # Only add if start_date was provided
        ...
        if not enrolled_courses.exists():
            return [], "No courses with enrollments found"
        ...
        with transaction.atomic():
            ...
            for date in dates:
                ...
                for slot_idx, label, start, end in slots_by_date[date]:
                    # Find next unscheduled compatible group
                            ...
                            for group_id in course_groups:
                                ...
                                if group.group_name in ["A", "B"]:
                                    ...
                                    if weekday == "Friday":
                                        ...
                                        # Random selection only if not Friday
                                    ...
                                    logger.warning(f"Unknown group {group.group_name} - using default")
                                    start_time = time(13, 0)
                                    end_time = time(16, 0)
                                    slot = "Afternoon"
                                ...
                                try:
                                    ...
                                    break  # Move to next slot after scheduling
                                    
                                except Exception as e:
                                    logger.error(f"Failed to create exam for group {group.id}: {str(e)}")
                                    continue
                            
                            if group_idx in scheduled_groups:
                                break  # Move to next slot if group was scheduled
                    
                    if len(scheduled_groups) == len(compatible_groups):
