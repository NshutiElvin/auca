...
def allocate_shared_rooms(exams):
    ...
    with transaction.atomic():
        # First get all StudentExam objects we'll be working with
        all_student_exams = []
        for date, exam_list in exams.items():
            for exam in exam_list:
                student_exams = StudentExam.objects.filter(
                    exam=exam.exam,
                    student=exam.student
                ).select_related('exam', 'student')
                all_student_exams.extend(student_exams)

        # Clear all room assignments first
        StudentExam.objects.filter(
            id__in=[se.id for se in all_student_exams]
        ).update(room=None)
        ...
        for date, exam_list in exams.items():
            ...
            for exam in exam_list:
                if exam.exam.start_time == SLOTS[0][1] and exam.exam.end_time == SLOTS[0][2]:
                    time_slots['morning'].append(exam)
                elif exam.exam.start_time == SLOTS[1][1] and exam.exam.end_time == SLOTS[1][2]:
                    time_slots['afternoon'].append(exam)
                elif exam.exam.start_time == SLOTS[2][1] and exam.exam.end_time == SLOTS[2][2]:
                    time_slots['evening'].append(exam)
            ...
            for slot_name in ['morning', 'afternoon', 'evening']:
                ...
                for pair in exam_pairs:
                    ...
                    for exam_se in exams_to_allocate:
                        ...
                        if overflow:
                            students_by_exam[exam_id].extend(overflow)
        ...
        if unaccommodated_students:
            ...
            for room in remaining_rooms:
                available_seats = room.capacity - StudentExam.objects.filter(
                    room=room,
                    exam__date=date
                ).count()
                ...
                if available_seats > 0 and unaccommodated_students:
                    ...
                    for student in students_to_place:
                        exam = next(
                            se.exam for se in exam_list 
                            if se.student.id == student.id
                        )
                        StudentExam.objects.filter(
                            student=student,
                            exam=exam
