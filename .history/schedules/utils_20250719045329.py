...
def generate_exam_schedule(start_date=None, end_date=None,course_ids=None, semester=None):
    ...
    try:
        ...
        unaccommodated_students=allocate_shared_rooms(scheduled_exams_per_date)
...
def allocate_shared_rooms(exams):
    ...
    with transaction.atomic():
        # First get all exam dates we'll be processing
        exam_dates = list(exams.keys())
        
        # Get all StudentExam records for these dates
        student_exams_qs = StudentExam.objects.filter(
            exam__date__in=exam_dates
        ).select_related('student', 'exam', 'room')

        # Create a mapping of student+exam to StudentExam instance
        student_exam_map = {
            (se.student_id, se.exam_id): se
            for se in student_exams_qs
        }

        for date, exam_instances in exams.items():
            # Group by time slot
            time_slots = {
                'morning': [],
                'afternoon': [],
            ...
            for exam_instance in exam_instances:
                se = student_exam_map.get((exam_instance.student.id, exam_instance.exam.id))
                if not se:
                    continue
                    
                if exam_instance.exam.start_time == SLOTS[0][1] and exam_instance.exam.end_time == SLOTS[0][2]:
                    time_slots['morning'].append(se)
                elif exam_instance.exam.start_time == SLOTS[1][1] and exam_instance.exam.end_time == SLOTS[1][2]:
                    time_slots['afternoon'].append(se)
                elif exam_instance.exam.start_time == SLOTS[2][1] and exam_instance.exam.end_time == SLOTS[2][2]:
            ...
            for slot_name in ['morning', 'afternoon', 'evening']:
                ...
                students_by_exam = defaultdict(list)
                room_index = 0

                # Create exam pairs avoiding adjacent semesters
                exam_pairs = []
                used_exams = set()

                for i in range(len(slot_exams)):
                    if slot_exams[i].exam_id in used_exams:
                        continue
                    for j in range(i + 1, len(slot_exams)):
                        if slot_exams[j].exam_id in used_exams:
                            continue
                        try:
                            ...
                            if abs(sem_i - sem_j) > 1:
                                ...
                                used_exams.update({slot_exams[i].exam_id, slot_exams[j].exam_id})
                                break
                        except (AttributeError, IndexError, ValueError):
                            continue
                    else:
                        exam_pairs.append((slot_exams[i], None))
                        used_exams.add(slot_exams[i].exam_id)

                # Allocate rooms to exam pairs
                for pair in exam_pairs:
                    if room_index >= len(rooms):
                        unaccommodated_students.extend([se.student for se in slot_exams])
                        continue
                    ...
                    for exam_se in exams_to_allocate:
                        exam_students = [se for se in slot_exams if se.exam_id == exam_se.exam_id]
                        ...
                        if overflow:
                            students_by_exam[exam_se.exam_id].extend(overflow)

                # Handle overflow students
                remaining_exams = [exam_id for exam_id in students_by_exam if students_by_exam[exam_id]]
                while room_index < len(rooms) and remaining_exams:
                    room = rooms[room_index]
                    room_index += 1
                    capacity_per_exam = room.capacity // 2

                    first_exam_id = remaining_exams.pop(0)
                    second_exam_id = None

                    for i, other_exam_id in enumerate(remaining_exams):
                        try:
                            first_sem = int(Exam.objects.get(id=first_exam_id).group.course.semester.name.split(" ")[1])
                            other_sem = int(Exam.objects.get(id=other_exam_id).group.course.semester.name.split(" ")[1])
                            if abs(first_sem - other_sem) > 1:
                                second_exam_id = other_exam_id
                                remaining_exams.pop(i)
                                break
                        except (AttributeError, IndexError, ValueError):
                            continue

                    exams_in_room = [first_exam_id]
                    if second_exam_id:
                        exams_in_room.append(second_exam_id)

                    for exam_id in exams_in_room:
                        students = students_by_exam[exam_id][:capacity_per_exam]
                        overflow = students_by_exam[exam_id][capacity_per_exam:]
                        ...
                        if overflow:
                            remaining_exams.append(exam_id)

                # Track unaccommodated students
                for exam_id in students_by_exam:
                    for student_exam in students_by_exam[exam_id]:
                        unaccommodated_students.append(student_exam.student)

        # Final attempt to place unaccommodated students
        if unaccommodated_students:
            for date in exam_dates:
                ...
                for room in remaining_rooms:
                    allocated_count = StudentExam.objects.filter(
                        room=room,
                        exam__date=date
                    ).count()
                    available_seats = room.capacity - allocated_count
                    ...
                    if available_seats > 0 and unaccommodated_students:
                        ...
                        for student in students_to_place:
...
#                     for exam_se in exams_to_allocate:
#                         exam_students = [se for se in slot_exams if se.exam.id == exam_se.exam.id]
#                         allocated = exam_students[:capacity_per_exam]
#                         overflow = exam_students[capacity_per_exam:]

#                         for student_exam in allocated:
...
#                         if overflow:
#                             students_by_exam[exam_se.exam.id].extend(overflow)

#                 # Step 3: Handle overflow students
#                 remaining_exams = [exam_id for exam_id in students_by_exam if students_by_exam[exam_id]]
#                 while room_index < len(rooms) and remaining_exams:
#                     room = rooms[room_index]
#                     room_index += 1
...
#                 # Step 4: Collect remaining unaccommodated students
#                 for exam_id in students_by_exam:
#                     for student_exam in students_by_exam[exam_id]:
#                         unaccommodated_students.append(student_exam.student)

#                 # Step 5: Save allocations for this time slot
#                 print(assigned_student_exams)
#                 StudentExam.objects.bulk_create(assigned_student_exams)
...
#                     # Need to get their exam information
#                     for student in students_to_place:
#                         student_exam = next(se for se in student_exams if se.student == student)
