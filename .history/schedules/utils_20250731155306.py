...
def generate_exam_schedule(start_date=None, end_date=None,course_ids=None, semester=None):
    ...
    try:
        ...
        myscheduled_groups=copy.deepcopy(compatible_groups)
        ...
        with transaction.atomic():
            ...
            for idx,course_group in enumerate(compatible_groups):
                print(len(compatible_groups), len(myscheduled_groups), sep=" ")
                ...
                # slot_seats_usages={ "Morning":0, "Evening":0, "Afternoon":0}
                # date_index+=1
                # all_groups_scheduled = True
                # for course_dict in course_group["courses"]:
                        
                         
                #     if len(course_dict["groups"]) > 0:
                #         all_groups_scheduled = False
                #         break

                # # Only remove if all groups are scheduled
                # if all_groups_scheduled:
                #     myscheduled_groups.remove(course_group)

                date_index += 1
                    # Reset slot usage when moving to next date
                slot_seats_usages = {"Morning":0, "Evening":0, "Afternoon":0}
