...
def allocate_shared_rooms():
    # Get all unassigned student exams
    ...
    with transaction.atomic():
        # Organize by date -> slot -> exam -> students
        ...
        for se in student_exams:
            for slot_name, start, end in SLOTS:
                if se.exam.start_time == start and se.exam.end_time == end:
                    schedule[se.exam.date][slot_name][se.exam.id].append(se)
                    break

        # Track room assignments by date and slot
        room_assignments = defaultdict(lambda: defaultdict(dict))  # date -> slot -> room_id: [student_exams]
        ...
        for date, slots in schedule.items():
            for slot_name, slot_start, slot_end in SLOTS:
                ...
                # Get available rooms (can reuse across slots)
                used_rooms = set()
                for other_slot in SLOTS:
                    if other_slot[0] != slot_name:
                        used_rooms.update(room_assignments[date][other_slot[0]].keys())
                available_rooms = [r for r in rooms if r.id not in used_rooms]

                # Process exams in this slot
                remaining_students = []
                for exam_id, exam_student_exams in slot_data.items():
                    remaining_students.extend(exam_student_exams)

                # Assign students to rooms
                room_index = 0
                while remaining_students and room_index < len(available_rooms):
                    room = available_rooms[room_index]
                    room_index += 1

                    # Calculate available capacity (check existing assignments)
                    current_assignment = room_assignments[date][slot_name].get(room.id, [])
                    available_seats = room.capacity - len(current_assignment)

                    if available_seats <= 0:
                        continue

                    # Take next batch of students
                    to_assign = remaining_students[:available_seats]
                    remaining_students = remaining_students[available_seats:]

                    # Update room assignments
                    if room.id not in room_assignments[date][slot_name]:
                        room_assignments[date][slot_name][room.id] = []
                    room_assignments[date][slot_name][room.id].extend(to_assign)
        ...
        for date, slots in room_assignments.items():
            for slot_name, rooms_data in slots.items():
                for room_id, student_exams in rooms_data.items():
            ...
            for se in remaining_exams:
                ...
                for slot_name, start, end in SLOTS:
                    if se.exam.start_time == start and se.exam.end_time == end:
                        ...
                        for room in rooms:
                            assigned = room_assignments[date][slot_name].get(room.id, [])
                            if len(assigned) < room.capacity:
                                se.room = room
                                se.save()
                                try:
