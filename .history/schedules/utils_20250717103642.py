...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    Each group is scheduled by checking every available slot (date + time).
    """
    if not start_date:
    ...
    # Generate enough exam slots
    estimated_slots_needed = len(compatible_groups) * 2  # reduce because we’ll use all 3 slots per day
    ...
    for slot_idx, (date, label, start_time, end_time) in enumerate(date_slots):
        slots_by_date[date].append((slot_idx, label, start_time, end_time))
    ...
    with transaction.atomic():
        for group_idx, course_group in enumerate(compatible_groups):
            ...
            for date in sorted(slots_by_date.keys()):
                ...
                for slot_idx, label, start_time, end_time in slots_by_date[date]:
                    group_exams = []
                    ...
                    # All courses in group can be scheduled at this slot
                    for course_id in course_group:
                        course = Course.objects.get(id=course_id)
                    ...
                    try:
                        ...
                        print(f"✓ Scheduled group {group_idx} on {date} ({label})")
                        break  # Exit slot loop
                    except Exception as e:
                        print(f"Room allocation error for group {group_idx} on {date}: {e}")
                        for exam in group_exams:
                            exam.delete()

                if scheduled_this_group:
                    break  # Exit date loop

            if not scheduled_this_group:
                print(f"✗ Could not schedule group {group_idx}")
