...
def has_sufficient_gap_flexible(student_exam_dates, proposed_date, min_gap_days=1):
    """
    More flexible gap checking - allows 1 day minimum gap instead of strict 2 days
    This allows for better day distribution
    """
    if not student_exam_dates:
        return True
...
def calculate_student_exam_distribution(student_exam_dates, proposed_date):
    """
    Calculate how well distributed a student's exams would be with the proposed date
    Returns a score where higher is better (more even distribution)
    """
    if not student_exam_dates:
        return 100  # Perfect score for first exam
    
    all_dates = sorted(student_exam_dates + [proposed_date])
    ...
    # Penalize very large gaps and very small gaps
    # Ideal gap is around 2-3 days
    ideal_gap = 2.5
    score = 100
    
    for gap in gaps:
        if gap < 1:
            score -= 50  # Heavy penalty for same day or next day
        elif gap == 1:
            score -= 20  # Moderate penalty for consecutive days
        elif 2 <= gap <= 3:
            score += 10  # Bonus for ideal gaps
        elif gap > 7:
            score -= (gap - 7) * 5  # Penalty for very long gaps
    
    return max(0, score)

def get_slot_utilization_score(date, slot_info, scheduled_exams):
    """
    Calculate how well a slot is being utilized
    Returns higher score for better utilization
    """
    slot_label, start_time, end_time = slot_info
    
    # Count exams already scheduled in this slot
    existing_exams = 0
    for exam_info in scheduled_exams:
        if (exam_info['date'] == date and 
            exam_info['start_time'] == start_time and 
            exam_info['end_time'] == end_time):
            existing_exams += 1
    
    # Prefer slots with some exams already (better utilization)
    # but not completely full
    if existing_exams == 0:
        return 0  # Empty slot - less preferred
    elif existing_exams <= 3:
        return existing_exams * 20  # Moderate utilization - preferred
    else:
        return 60 - (existing_exams - 3) * 10  # High utilization - less preferred

def can_schedule_course_group_on_slot_improved(course_group, proposed_date, slot_info, student_exam_dates, scheduled_exams):
    """
    Improved scheduling check with better scoring and flexibility
    """
    conflicts = []
    benefits = []
    slot_label, start_time, end_time = slot_info
    ...
    for course_id in course_group:
        ...
        for student_id in student_ids:
            ...
            # Check for same-day conflict (absolute no-go)
            if proposed_date in current_exam_dates:
                conflicts.append(f"Student {student_id} already has exam on {proposed_date}")
                continue
            
            # Check minimum gap (flexible - allow 1 day gap)
            if not has_sufficient_gap_flexible(current_exam_dates, proposed_date, min_gap_days=1):
                conflicts.append(f"Student {student_id} would have insufficient gap")
                continue
            
            # Calculate distribution score for this student
            distribution_score = calculate_student_exam_distribution(current_exam_dates, proposed_date)
            total_distribution_score += distribution_score
            student_count += 1
    
    # Calculate slot utilization score
    utilization_score = get_slot_utilization_score(proposed_date, slot_info, scheduled_exams)
    
    # Calculate overall score
    avg_distribution_score = total_distribution_score / max(student_count, 1)
    overall_score = avg_distribution_score + utilization_score
    
    can_schedule = len(conflicts) == 0
    
    return can_schedule, conflicts, overall_score

def generate_exam_schedule_improved(start_date=None, course_ids=None, semester=None):
    """
    Improved exam scheduling with better day-off distribution
    """
    if not start_date:
        start_date = now().date() + timedelta(days=1)
    ...
    # Generate enough slots with better distribution
    ...
    student_exam_dates = defaultdict(list)
    assigned_slots = set()
    scheduled_exams = []  # Track scheduled exams for utilization scoring
    
    with transaction.atomic():
        # Sort course groups by constraint difficulty
        group_difficulties = []
        for i, course_group in enumerate(all_course_groups):
            total_students = sum(
                Enrollment.objects.filter(course_id=course_id).count() 
            ...
            # Calculate constraint difficulty (more students = harder to schedule)
            difficulty = total_students
            
            # Add penalty for groups with many conflicting courses
            conflict_penalty = 0
            for course_id in course_group:
                course_students = set(Enrollment.objects.filter(course_id=course_id).values_list('student_id', flat=True))
                for other_group in all_course_groups:
                    if other_group != course_group:
                        for other_course_id in other_group:
                            other_students = set(Enrollment.objects.filter(course_id=other_course_id).values_list('student_id', flat=True))
                            if course_students.intersection(other_students):
                                conflict_penalty += len(course_students.intersection(other_students))
            
            difficulty += conflict_penalty * 0.1  # Small penalty for conflicts
            group_difficulties.append((i, difficulty, group_preferences[i]))
        
        # Sort by difficulty (most difficult first)
        group_difficulties.sort(key=lambda x: x[1], reverse=True)
        
        # Schedule each course group with improved logic
        for group_idx, difficulty, preference in group_difficulties:
            course_group = all_course_groups[group_idx]
            ...
            if preference == "mostly morning":
                ...
                preferred_slot_order = [0, 1, 2]
                if random.random() < 0.3:  # Less random shuffling
                    random.shuffle(preferred_slot_order)
            ...
            # Try multiple dates to find the best option
            dates_to_try = sorted(slots_by_date.keys())[:min(len(slots_by_date), 10)]  # Limit search
            ...
            for date in dates_to_try:
                ...
                # Try each slot and score them
                for slot_idx, label, start_time, end_time in available_slots:
                    can_schedule, conflicts, score = can_schedule_course_group_on_slot_improved(
                        course_group, date, (label, start_time, end_time), 
                        student_exam_dates, scheduled_exams
                    )
                    
                    if can_schedule and score > best_score:
                        best_score = score
                        best_option = (course_group, date, slot_idx, label, start_time, end_time)
            
            # Schedule the best option found
            if best_option:
                course_group, date, slot_idx, label, start_time, end_time = best_option
                ...
                for course_id in course_group:
                    ...
                    # Track scheduled exam for utilization scoring
                ...
                try:
                    unaccommodated = allocate_shared_rooms(group_exams)
                    print(f"Scheduled {preference} group {course_group} on {date} {label} ({start_time}-{end_time}) [Score: {best_score:.1f}]")
                    if unaccommodated:
                        print(f"  Warning: {len(unaccommodated)} students unaccommodated")
                except Exception as e:
                    print(f"  Room allocation error: {e}")
                ...
                print(f"Could not schedule {preference} group {course_group} - no suitable slots found")
                raise ValueError(f"Cannot find suitable slot for {preference} course group {course_group}")
    
    # Print scheduling statistics
    print_scheduling_statistics(student_exam_dates, scheduled_exams)
    
    return exams_created, None

def print_scheduling_statistics(student_exam_dates, scheduled_exams):
    """
    Print statistics about the scheduling
    """
    ...
    print(f"Total exam gaps analyzed: {total_gaps}")
    ...
    for gap in sorted(gap_distribution.keys()):
        ...
        print(f"  {gap} day(s): {count} gaps ({percentage:.1f}%)")
    
    # Calculate exam dates utilization
    exam_dates_used = set()
    for exam_info in scheduled_exams:
        exam_dates_used.add(exam_info['date'])
    
    print(f"\nExam dates used: {len(exam_dates_used)}")
    print(f"Total exams scheduled: {len(scheduled_exams)}")
    
    # Show date utilization
    date_utilization = defaultdict(int)
    for exam_info in scheduled_exams:
        date_utilization[exam_info['date']] += 1
    
    print("\nDate utilization:")
    for date in sorted(date_utilization.keys()):
        count = date_utilization[date]
        print(f"  {date} ({date.strftime('%A')}): {count} exams")

def verify_day_off_constraints_improved(min_gap_days=1):
    """
    Improved verification with more flexible gap requirements
    """
    violations = []
...
def find_compatible_courses_within_group(courses):
    """
    Find compatible courses within a specific group
    """
    if not courses:
        return []
    ...
    while remaining_courses:
        ...
        while compatible_with_group and len(course_group) < 8:
            next_course = min(
                compatible_with_group,
                key=lambda c: len([rc for rc in remaining_courses if rc != c and 
                                  tuple(sorted([c, rc])) not in conflict_matrix])
