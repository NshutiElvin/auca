...
def which_suitable_slot_to_schedule_course_group(date, new_group, suggested_slot):
    ...
    def evaluate_slot(check_date, slot, is_suggested=False):
        ...
        if conflicts:
            ...
            if is_suggested:
                all_suggestions.append({"suggested":is_suggested,"date": check_date, "slot": slot, "reason": f"Suggested slot {check_date} {slot} is not available (conflicts)"})
            else:
                all_suggestions.append({"suggested":is_suggested,"date": check_date, "slot": slot, "reason": f"Slot {check_date} {slot} is not available (conflicts)"})
            return False
            ...
            if is_suggested:
                all_suggestions.append({"suggested":is_suggested,"date": check_date, "slot": slot, "reason": f"Suggested slot {check_date} {slot} is not available (insufficient rooms)"})
            else:
                all_suggestions.append({"suggested":is_suggested,"date": check_date, "slot": slot, "reason": f"Slot {check_date} {slot} is not available (insufficient rooms)"})
            return False
        else:
