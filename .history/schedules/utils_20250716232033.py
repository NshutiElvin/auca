...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    # Schedule exams over two weeks, allowing each student to rest independently via per-student gap checks
    duration_days = 14
    end_date = start_date + timedelta(days=duration_days - 1)
    # Generate slots for all days in the window
    date_slots = []
    current_date = start_date
    while current_date <= end_date:
        if current_date.strftime('%A') not in NO_EXAM_DAYS:
            slots = FRIDAY_SLOTS if current_date.strftime('%A') == 'Friday' else SLOTS
            for label, slot_start, slot_end in slots:
                date_slots.append((current_date, label, slot_start, slot_end))
        current_date += timedelta(days=1)
...
def find_optimal_exam_dates(start_date=None):
    ...
    # Generate enough slots
    date_slots = get_exam_slots(start_date, max_slots=min_days_needed * 10+ 5)  # Add buffer
    
    return {
        'start_date': start_date,
        'suggested_end_date': start_date + timedelta(days=min_days_needed + 10),  # Add buffer
        'min_days_needed': min_days_needed,
        'course_pairs': course_pairs,
        'available_slots': date_slots[:min_days_needed * 3]
    }
