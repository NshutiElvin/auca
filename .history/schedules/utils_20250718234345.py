...
#     conflict_matrix = defaultdict(int)
#     courses_with_conflicts = defaultdict(list)
#     student_courses = defaultdict(list)
#     courses_graph = defaultdict(lambda: defaultdict(list))
...
#         courses_graph[enrollment.course_id][enrollment.group_id].append(enrollment.student_id)
    
#     unique_courses = list(courses_graph.keys())

#     # Compare all unique pairs (including those with 0 conflicts)
#     for course1, course2 in combinations(unique_courses, 2):
#         # Get all students in each course (across all groups)
#         students1 = {s for group in courses_graph[course1].values() for s in group}
#         students2 = {s for group in courses_graph[course2].values() for s in group}
        
#         shared = len(students1 & students2)
#         conflict_matrix[(course1, course2)] = shared
#         conflict_matrix[(course2, course1)] = shared  # Mirror entry
        
#         if shared > 0:
#             courses_with_conflicts[course1].append(course2)
#             courses_with_conflicts[course2].append(course1)
    
    


#     return conflict_matrix, courses_with_conflicts

from collections import defaultdict
from itertools import combinations

def find_compatible_courses(courses):
    if not courses:
        return {
            'conflict_matrix': defaultdict(int),
    ...
    conflict_graph = defaultdict(list)
    student_courses = defaultdict(list)
    course_groups = defaultdict(lambda: defaultdict(list))
    
    # Populate enrollment data
    for enrollment in Enrollment.objects.filter(course_id__in=courses).iterator():
        student_courses[enrollment.student_id].append(enrollment.course_id)
        course_groups[enrollment.course_id][enrollment.group_id].append(enrollment.student_id)
    
    unique_courses = list(course_groups.keys())
    
    # Build conflict data
    for course1, course2 in combinations(unique_courses, 2):
        students1 = {s for group in course_groups[course1].values() for s in group}
        students2 = {s for group in course_groups[course2].values() for s in group}
        shared = len(students1 & students2)
        
        conflict_matrix[(course1, course2)] = shared
        conflict_matrix[(course2, course1)] = shared
        
        if shared > 0:
            conflict_graph[course1].append(course2)
            conflict_graph[course2].append(course1)
    
    # Generate compatible groups (graph coloring approach)
    compatible_groups = []
    unassigned = set(unique_courses)
    color = 0
    ...
    while unassigned:
        ...
        while candidates:
            # Select course with most conflicts with remaining candidates
            course = max(
                candidates,
                key=lambda c: sum(1 for other in candidates if conflict_matrix[(c, other)] > 0)
            )
            
            # Verify compatibility with current group
            if all(conflict_matrix[(course, c)] == 0 for c in current_group):
                current_group.add(course)
                unassigned.remove(course)
            
            candidates.remove(course)
        
        compatible_groups.append({
            'timeslot': color + 1,
            'courses': list(current_group),
            'conflict_free': True
        })
        color += 1
    ...
    return {
        ...
        'conflict_graph': dict(conflict_graph),
...
#     course_ids = courses
#     courses_with_conflicts = defaultdict(list)
#     all_courses = set(course_ids)  # Using the input courses directly

#     # Data structures to store enrollment information
#     student_courses = defaultdict(list)
#     courses_graph = defaultdict(lambda: defaultdict(list))

#     # Populate the data structures
#     for enrollment in Enrollment.objects.filter(course_id__in=course_ids).iterator():
#         student_course = (enrollment.course_id, enrollment.group_id)
#         student_courses[enrollment.student_id].append(student_course)
#         courses_graph[enrollment.course_id][enrollment.group_id].append(enrollment.student_id)
    
#     # Get unique courses from the graph
#     courses = list(courses_graph.keys())

#     # Compare all pairs of courses for conflicts
#     for i in range(len(courses)):
#         course1 = courses[i]
#         course1_groups = courses_graph[course1]
        
#         # Get all students in course1 (flattened across groups)
#         course1_students = set()
#         for group in course1_groups.values():
#             course1_students.update(group)
        
#         for j in range(i + 1, len(courses)):
#             course2 = courses[j]
#             course2_groups = courses_graph[course2]
            
#             # Get all students in course2 (flattened across groups)
#             course2_students = set()
#             for group in course2_groups.values():
#                 course2_students.update(group)
            
#             # Find intersection of students
#             common_students = course1_students & course2_students
#             if common_students:
#                 conflict_matrix[(course1, course2)] = len(common_students)
#                 courses_with_conflicts[course1].append(course2)
#                 courses_with_conflicts[course2].append(course1)
#     pprint(conflict_matrix)
...
def generate_exam_schedule(start_date=None, end_date=None,course_ids=None, semester=None):
    ...
    try:
        ...
        conflict_matrix, conflict_graph= find_compatible_courses_within_group(enrolled_courses)
