...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    with gap-filling optimization before database commits
    """
    if not start_date:
    ...
    # Get all valid dates (excluding Saturdays)
    valid_dates = sorted([date for date in slots_by_date.keys() 
                         if date.strftime('%A') != "Saturday"])
    
    # Phase 1: Generate all possible schedules without database commits
    best_schedule = find_optimal_schedule(compatible_groups, valid_dates, slots_by_date)
    
    if not best_schedule:
        return [], "No valid schedule found"
    
    # Phase 2: Commit the best schedule to database
    exams_created = []
    with transaction.atomic():
        for group_idx, date, group_exams_data in best_schedule:
            group_exams = []
            for course_id, start_time, end_time in group_exams_data:
                try:
...
def find_optimal_schedule(compatible_groups, valid_dates, slots_by_date):
    """
    Find the optimal schedule that minimizes gaps using backtracking
    """
    def backtrack(group_idx, current_schedule, used_dates, student_exam_dates):
        # Base case: all groups scheduled
        if group_idx >= len(compatible_groups):
            return current_schedule.copy()
        
        course_group = compatible_groups[group_idx]
        
        # Try scheduling on each valid date
        for date in valid_dates:
            ...
            group_exams_data = []
            temp_student_dates = defaultdict(list)
            ...
            for course_id in course_group:
                ...
                start_time, end_time = get_time_slot_for_preference(
                    course.group, weekday
                )
                
                group_exams_data.append((course_id, start_time, end_time))
                
                # Track student dates for this attempt
                student_ids = Enrollment.objects.filter(course=course).values_list('student_id', flat=True)
                for student_id in student_ids:
                    temp_student_dates[student_id].append(date)
            
            if not can_schedule_group:
                continue
            
            # Update tracking structures
            new_used_dates = used_dates | {date}
            new_student_exam_dates = student_exam_dates.copy()
            for student_id, dates in temp_student_dates.items():
                new_student_exam_dates[student_id].extend(dates)
            
            # Add to current schedule
            current_schedule.append((group_idx, date, group_exams_data))
            
            # Recursive call
            result = backtrack(group_idx + 1, current_schedule, 
                             new_used_dates, new_student_exam_dates)
    ...
    # Start backtracking
    initial_schedule = []
    initial_used_dates = set()
    initial_student_dates = defaultdict(list)
    
    result = backtrack(0, initial_schedule, initial_used_dates, initial_student_dates)
    
    if result:
        # Apply gap-filling optimization
        return optimize_schedule_gaps(result, valid_dates)
    
    return None


def optimize_schedule_gaps(schedule, valid_dates):
    """
    Optimize the schedule to fill gaps by trying to move groups to earlier dates
    """
    if not schedule:
        return schedule
    
    # Sort schedule by date to identify gaps
    schedule.sort(key=lambda x: x[1])  # Sort by date
    
    # Find gaps
    used_dates = {item[1] for item in schedule}
    gaps = []
    ...
    for i in range(len(valid_dates) - 1):
        ...
        if current_date not in used_dates and next_date in used_dates:
            gaps.append(current_date)
    
    if not gaps:
        return schedule
    
    # Try to fill gaps by moving groups from later dates
    optimized_schedule = schedule.copy()
    
    for gap_date in gaps:
        gap_weekday = gap_date.strftime('%A')
        
        # Find groups that could potentially move to this gap
        for i, (group_idx, current_date, group_exams_data) in enumerate(optimized_schedule):
            if current_date <= gap_date:
                continue
                
            # Check if this group can be moved to the gap date
            can_move = True
            new_group_exams_data = []
            
            for course_id, old_start_time, old_end_time in group_exams_data:
                try:
                    course = Course.objects.get(id=course_id)
                    
                    # Get time slot for the gap date
                    new_start_time, new_end_time = get_time_slot_for_preference(
                        course.group, gap_weekday
                    )
                    
                    # Check if the time slot is compatible
                    # (You may need to add additional conflict checking here)
                    
                    new_group_exams_data.append((course_id, new_start_time, new_end_time))
            ...
            if can_move:
                # Move the group to the gap date
                optimized_schedule[i] = (group_idx, gap_date, new_group_exams_data)
                break
    
    return optimized_schedule


def get_time_slot_for_preference(course_preference, weekday):
    """
    Get time slot based on course preference and weekday
    """
    if course_preference in ["A", "B"]:
        return time(8, 0), time(11, 0)
    elif course_preference in ["C", "D"]:
        if weekday == "Friday":
            return time(13, 0), time(16, 0)
        else:
            # For C/D groups, prefer afternoon but can use any slot
            return time(13, 0), time(16, 0)
    elif course_preference in ["E", "F"]:
        if weekday != "Friday":
            return time(18, 0), time(20, 0)
        else:
            return time(13, 0), time(16, 0)
    else:
        # Default case
...
def can_schedule_course_group_on_slot(course, date, student_exam_dates):
    """
    Check if a course can be scheduled on a specific date
    without creating student conflicts
    """
