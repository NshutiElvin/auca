...
def which_suitable_slot_to_schedule_course_group(date, new_group, suggested_slot):
    ...
    def check_seats_overflow(check_date, slot):
        all_available_seats = Room.objects.order_by('-capacity').aggregate(
            total=Sum('capacity')
        )['total'] or 0
        start_time=time(8, 0) if slot == "Morning" else  (time(13, 0) if slot == "Afternoon" else time(18, 0))
        occupied_seats=get_occupied_seats_by_time_slot(check_date, start_time)
