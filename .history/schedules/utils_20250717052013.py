...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    # Estimate and get enough exam slots
    ...
    # Compute difficulty (number of students) for sorting
    group_difficulties = []
    ...
    with transaction.atomic():
        # Iterate by date first to fill all slots per day
        for date in sorted(slots_by_date.keys()):
            available_slots = [s for s in slots_by_date[date] if s[0] not in assigned_slots]
            ...
            for slot_idx, label, start_time, end_time in available_slots:
                ...
                for group_idx, total_students, preference in group_difficulties:
                    ...
                    if slot_priority[label] not in preferred_slot_order:
                        continue
                    ...
                    if can_schedule:
                        ...
                        slot_filled = True
                        break  # Move to next slot after assigning this group

                if not slot_filled:
                    print(f"No group could be scheduled on {date} {label}")

        # Check for unscheduled groups
        unscheduled = set(range(len(all_course_groups))) - scheduled_groups
        if unscheduled:
            for group_idx in unscheduled:
                print(f"⚠️ Could not schedule group {all_course_groups[group_idx]}")
            raise ValueError(f"{len(unscheduled)} course groups could not be scheduled due to conflicts or slot exhaustion.")
