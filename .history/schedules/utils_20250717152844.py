...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    """
    Generate exam schedule for individual courses (not by groups)
    """
    if not start_date:
    ...
    # Generate enough slots (now based on individual courses)
    estimated_slots_needed = len(enrolled_courses) * 3  # Extra buffer
    ...
    scheduled_courses = set()  # Track scheduled course IDs
    ...
    with transaction.atomic():
        ...
        for date in dates:
            ...
            # Try each course that hasn't been scheduled yet
            for course in enrolled_courses.exclude(id__in=scheduled_courses):
                ...
                # Determine time slot based on course group preference
                ...
                if course_preference in ["A", "B"]:
                    ...
                    if weekday == "Friday":
                        ...
                        # Random slot for variety
                        slot_choice = random.choice(["Morning", "Afternoon", "Evening"])
                        if slot_choice == "Morning":
                            start_time = time(8, 0)
                            end_time = time(11, 0)
                        elif slot_choice == "Afternoon":
                            start_time = time(13, 0)
                            end_time = time(16, 0)
                        else:
                ...
                # Try to allocate room for this single exam
                try:
                    allocate_shared_rooms([exam])  # Pass single exam as list
                    print(f"✓ Scheduled {course.code} on {date} ({slot})")
                except Exception as e:
                    print(f"Room allocation error for {course.code}: {e}")
                    # Rollback this exam if room allocation fails?
                    exam.delete()
                    scheduled_courses.remove(course.id)
                    continue
                
                # Optional: Try to reschedule to earlier date if possible
                for earlier_date in [d for d in dates if d < date]:
                    can_reschedule, _ = can_schedule_course_group_on_slot(
                        course, earlier_date, student_exam_dates
                    )
                    if can_reschedule:
                        exam.date = earlier_date
                        exam.save()
                        print(f"Rescheduled {course.code} to earlier date {earlier_date}")
                        break
