...
def allocate_shared_rooms(exams):
    ...
    # Precompute all students for each exam
    exam_students = {
        exam.id: list(Enrollment.objects.filter(course=exam.course).select_related('student'))
        for exam in exams
    }

    # Generate all possible compatible pairs
    compatible_pairs = []
    for i in range(len(exams)):
        for j in range(i+1, len(exams)):
            if are_semesters_compatible(exams[i], exams[j]):
                compatible_pairs.append((exams[i], exams[j]))

    # Sort pairs by total students (descending) to prioritize large groups
    compatible_pairs.sort(
        key=lambda p: len(exam_students[p[0].id]) + len(exam_students[p[1].id]),
        reverse=True
    )
    ...
    for room in rooms:
        ...
        # Find the best fitting pair for this room
        for i, pair in enumerate(compatible_pairs):
            if pair[0].id in allocated or pair[1].id in allocated:
                continue

            total_students = (len(exam_students[pair[0].id]) + 
                             len(exam_students[pair[1].id]))
            utilization = min(total_students, room.capacity)
        ...
        if best_pair:
            # Allocate this pair to the room
            exam1, exam2 = best_pair
            cap_per_exam = room.capacity // 2
            
            for exam in [exam1, exam2]:
                students = exam_students[exam.id][:cap_per_exam]
                ...
                exam_students[exam.id] = exam_students[exam.id][cap_per_exam:]
                allocated.add(exam.id)

            # Remove this pair from consideration
            compatible_pairs.pop(best_index)

    # Then allocate single exams to remaining rooms
    remaining_exams = [exam for exam in exams if exam.id not in allocated]
    remaining_exams.sort(
        key=lambda e: len(exam_students[e.id]),
        reverse=True
    )

    for room in rooms[len(assignments)//2:]:  # Remaining rooms
