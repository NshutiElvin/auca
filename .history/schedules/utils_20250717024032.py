...
def has_sufficient_gap(student_exam_dates, proposed_date, min_gap_days=1):
    """
    Check if scheduling an exam on proposed_date would maintain reasonable gap
    More flexible approach: allows some consecutive days but prevents excessive clustering
    """
    ...
    # Find position of proposed date
    proposed_index = all_dates.index(proposed_date)
    
    # Check for immediate consecutive days (day before and day after)
    consecutive_count = 1  # Count the proposed date itself
    
    # Count consecutive days before the proposed date
    for i in range(proposed_index - 1, -1, -1):
        if (all_dates[i + 1] - all_dates[i]).days == 1:
            consecutive_count += 1
        else:
            break
    
    # Count consecutive days after the proposed date
    for i in range(proposed_index, len(all_dates) - 1):
        if (all_dates[i + 1] - all_dates[i]).days == 1:
            consecutive_count += 1
        else:
            break
    
    # Allow up to 2 consecutive days, but discourage more
    if consecutive_count > 2:
        return False
    
    # Check for same day conflict (should never happen, but safety check)
    if proposed_date in student_exam_dates:
        return False
...
def can_schedule_course_group_on_slot(course_group, proposed_date, slot_info, student_exam_dates):
    ...
    for course_id in course_group:
        ...
        for student_id in student_ids:
            ...
            # Check for same-day conflict (this is a hard constraint)
            if proposed_date in current_exam_dates:
                conflicts.append(f"Student {student_id} already has exam on {proposed_date}")
                continue
            
            # Check for day-off constraint (more lenient now)
            if not has_sufficient_gap(current_exam_dates, proposed_date):
                conflicts.append(f"Student {student_id} would have too many consecutive exam days")
    
    # Only block scheduling if there are hard conflicts (same day) or excessive consecutive days
    # Count only serious conflicts, not just gap issues
    serious_conflicts = [c for c in conflicts if "already has exam on" in c or "too many consecutive" in c]
    
    return len(serious_conflicts) == 0, conflicts
...
def generate_exam_schedule(start_date=None, course_ids=None, semester=None):
    ...
    Maximizes slot usage per day while respecting individual student constraints
    """
    if not start_date:
    ...
    # Generate enough slots - more realistic estimate
    # We have 3 slots per day (morning, afternoon, evening), so calculate minimum days needed
    import math
    min_days_needed = math.ceil(len(all_course_groups) / 3)
    # Add small buffer for constraints and Friday limitations (only 2 slots)
    estimated_days_needed = min_days_needed + 20
    estimated_slots_needed = estimated_days_needed * 3  # 3 slots per day
    
    date_slots = get_exam_slots(start_date, max_slots=estimated_slots_needed)
    
    print(f"Scheduling {len(all_course_groups)} course groups")
    print(f"Estimated days needed: {estimated_days_needed} (minimum: {min_days_needed})")
    ...
    with transaction.atomic():
        ...
        # Schedule groups with a day-by-day approach to maximize slot usage
        unscheduled_groups = list(range(len(group_difficulties)))
        ...
        for date in sorted(slots_by_date.keys()):
            ...
            # Try to schedule multiple groups on this date
            ...
            for slot_idx, label, start_time, end_time in available_slots:
                ...
                # Find best group for this slot
                best_group_idx = None
                best_score = -1
                
                for i, group_list_idx in enumerate(unscheduled_groups):
                    group_idx, total_students, preference = group_difficulties[group_list_idx]
                    course_group = all_course_groups[group_idx]
                    
                    # Calculate preference score for this slot
                    if preference == "mostly morning":
                        slot_scores = {'Morning': 3, 'Afternoon': 2, 'Evening': 1}
                    elif preference == "evening":
                        slot_scores = {'Morning': 1, 'Afternoon': 2, 'Evening': 3}
                    else:  # mixed
                        slot_scores = {'Morning': 2, 'Afternoon': 3, 'Evening': 2}
                    
                    preference_score = slot_scores.get(label, 0)
                    
                    # Check if this group can be scheduled in this slot
                    can_schedule, conflicts = can_schedule_course_group_on_slot(
                        course_group, date, (label, start_time, end_time), student_exam_dates
                    )
                    
                    if can_schedule:
                        # Calculate total score (preference + urgency)
                        urgency_score = total_students / 100  # Normalize student count
                        total_score = preference_score + urgency_score
                        
                        if total_score > best_score:
                            best_score = total_score
                            best_group_idx = i
                
                # Schedule the best group for this slot
                if best_group_idx is not None:
                    group_list_idx = unscheduled_groups[best_group_idx]
                    group_idx, total_students, preference = group_difficulties[group_list_idx]
                    course_group = all_course_groups[group_idx]
                    ...
                    groups_scheduled_today.append(best_group_idx)
            ...
            for idx in sorted(groups_scheduled_today, reverse=True):
                unscheduled_groups.pop(idx)
                
            print(f"Scheduled {len(groups_scheduled_today)} groups on {date}")
        
        # Check if all groups were scheduled
        if unscheduled_groups:
            unscheduled_details = []
            for group_list_idx in unscheduled_groups:
                group_idx, total_students, preference = group_difficulties[group_list_idx]
                course_group = all_course_groups[group_idx]
                unscheduled_details.append(f"{preference} group {course_group}")
            
            print(f"Could not schedule {len(unscheduled_groups)} groups: {unscheduled_details}")
            raise ValueError(f"Cannot find suitable slots for {len(unscheduled_groups)} course groups")
