"""
ImportEnrollmentsData — Async SSE Streaming Version
=====================================================
- HTTP request returns immediately with an async stream
- Frontend receives real-time progress updates via SSE
- No timeout — connection stays alive with periodic progress events
- All heavy DB/pandas work runs in a thread via sync_to_async
- Compatible with Gunicorn + UvicornWorker (ASGI)
"""
...
def _safe_df(queryset, columns):
    """
    Always returns a DataFrame with correct columns
...
# ── Sync Import Logic (runs in thread pool via sync_to_async) ──────────────────
def _run_import(file_bytes, selected_semester, progress_callback):
    """
    All pandas + Django ORM work lives here in a sync function.
    progress_callback(step, total, message, stats) appends SSE events
    to a shared list that the async generator drains.
    """
    TOTAL_STEPS = 8
    ...
    with transaction.atomic():
            ...
            for row in update_merged.itertuples(index=False):
                ...
                if fn != row.FIRST or ln != row.LAST:
                    users_to_update.append(
                        User(id=int(uid), first_name=row.FIRST, last_name=row.LAST, email=row.email)
                    )
            if users_to_update:
        ...
        # Create new users
        existing_emails = (
            set(existing_users_df["email"].tolist()) if not existing_users_df.empty else set()
        )
        new_user_objs = []
        ...
        for _, row in course_meta_df.iterrows():
            ...
            c_dept_codes = course_dept_mapping.get(course_code, set())

            if not c_dept_codes:
                continue

            dept_frequency = {
                dc: len(df[(df["COURSECODE"] == course_code) & (df["DEPT_CODE"] == dc)])
                for dc in c_dept_codes
            }
            primary_dept_code = max(dept_frequency.items(), key=lambda x: x[1])[0]
            primary_dept_id   = dept_map.get(primary_dept_code)

            if not primary_dept_id:
                continue

            is_cross = len(c_dept_codes) > 1
            ...
            if is_cross:
                other_dept_ids = [
                    dept_map[dc]
                    for dc in (c_dept_codes - {primary_dept_code})
                    if dc in dept_map
                ]
                if other_dept_ids:
                    course.associated_departments.set(other_dept_ids)
                stats["courses_with_associations"] = stats.get("courses_with_associations", 0) + 1

            if created:
                stats["courses_created"] = stats.get("courses_created", 0) + 1
            else:
                stats["courses_updated"] = stats.get("courses_updated", 0) + 1
        ...
        if enr_df.empty:
            ...
            merged_enr = enr_df.merge(
                existing_enr_df[["id", "student_id", "course_id", "group_id", "status"]],
            ...
            g_col = "group_id_new" if "group_id_new" in new_enr_df.columns else "group_id"
            ...
            if is_existing.any():
                ...
                g_new_col = "group_id_new"      if "group_id_new"      in ext_rows.columns else "group_id"
                g_ext_col = "group_id_existing" if "group_id_existing" in ext_rows.columns else "group_id"
                needs_update = (ext_rows[g_new_col] != ext_rows[g_ext_col]) | (ext_rows["status"] != "enrolled")
...
class ImportEnrollmentsData(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Read file bytes immediately — file object is gone once streaming starts
        ...
        async def event_stream():
            ...
            Heavy work runs in a thread via sync_to_async so the event
            loop is never blocked and ASGI stays happy.
            """
            # Queue used to pass progress events from the sync thread to here
            queue = asyncio.Queue()

            def progress_callback(step, total, message, stats=None):
                """Called from the sync thread — puts SSE events into the queue."""
                queue.put_nowait(_progress(step, total, message, stats))

            async def run_import():
                """Runs the sync import in a thread pool."""
                try:
            ...
            # Drain the queue and yield SSE events as they arrive
            while True:
                try:
                ...
                if event is None:
                    # Import finished — stop the generator
                    break

                yield event

            # Ensure the task is complete before exiting
            await import_task
        ...
        response["X-Accel-Buffering"]           = "no"   # disable Nginx buffering on Render
