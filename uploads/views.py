...
# create semester if not exists or update is_active to true
            ...
            existing_departments = {
                d.code: d for d in Department.objects.filter(code__in=department_codes)
            }
            existing_semesters = {
            ...
            existing_students = {
                s.reg_no: s
                for s in Student.objects.filter(reg_no__in=student_nums).select_related(
                    "user"
                )
            }
            existing_courses = {
            ...
            for g in CourseGroup.objects.filter(
                group_name__in=unique_group_names
            ...
            existing_users = {
                u.email: u for u in User.objects.filter(email__in=potential_emails)
            }
            ...
            if departments_to_create:
                Department.objects.bulk_create(
                    departments_to_create, ignore_conflicts=True
                )
            ...
            for _, row in student_data.iterrows():
                ...
                if existing_student:
                    ...
                    department = existing_departments.get(
                        str(int(faculty_code))
                        if isinstance(faculty_code, float)
                        and faculty_code == int(faculty_code)
                        else str(faculty_code)
                    )
                    if department:
            ...
            if students_to_create:
                ...
                all_users = {
                    ...
                    for u in User.objects.filter(
                        email__in=[s["user_email"] for s in students_to_create]
                    )
                }
                ...
                if students_bulk_create:
                    Student.objects.bulk_create(
                        students_bulk_create, ignore_conflicts=True
                    )
            ...
            for _, row in course_data.iterrows():
                ...
                if code not in existing_courses:
                    ...
                    department_code = (
                        str(int(row["FACULTYCODE"]))
                        if isinstance(row["FACULTYCODE"], float)
                        and row["FACULTYCODE"] == int(row["FACULTYCODE"])
                        else str(row["FACULTYCODE"])
                    )
                    ...
                    if semester and department:
                        courses_to_create.append(
                            Course(
                                ...
                                title=row[
                                    "COURSENAME"
                                ],  # Fixed: use course_name instead of title
                                credits=row["CREDITS"],
                                semester=semester,
                                department=department,
                            )
                ...
                if course:
                    ...
                    for existing_key, existing_group_obj in existing_groups.items():
                        if existing_group_obj:
                            if (
                                existing_group_obj.group_name == group_name
                                and existing_key[0] != course_code
                            ):
                    ...
                    if not existing_group:
                        ...
                        conflicting_groups = [
                            ...
                            if g and g.group_name == group_name and g.course != course
                        ]
                        ...
                        if conflicting_groups:
                            ...
                            logger.warning(
                                f"Group name conflict detected: {group_name} exists for multiple courses. Creating unique name: {unique_group_name}"
                            )
                            ...
                            existing_groups[(course_code, unique_group_name)] = (
                                None  # Placeholder
                            )
                        else:
                            groups_to_create.append(
                        ...
                        if existing_group.course != course:
                            logger.warning(
                                f"Group {group_name} exists but belongs to different course. Expected: {course_code}, Found: {existing_group.course.code}"
                            )
            ...
            if group_conflicts:
                ...
                for group_name, courses in group_conflicts.items():
                    logger.warning(
                        f"  Group '{group_name}' appears in courses: {list(courses)}"
                    )
            ...
            for g in CourseGroup.objects.filter(
                ...
                group_name__in=list(course_groups["GROUP"].unique())
                + [
                    f"{gn}_{cc}"
                    for cc in course_codes
                    for gn in course_groups["GROUP"].unique()
                ],
            ).select_related("course"):
            ...
            for enrollment in Enrollment.objects.filter(
                student__reg_no__in=student_nums, course__code__in=course_codes
                    ...
                    if not existing_enrollment:
                        if group:
                            ...
                            enrollment_errors.append(
                                f"Group '{group_name}' not found for course '{course_code}' - enrollment created without group"
                            )
                    else:
                        ...
                        if group and existing_enrollment.group != group:
                            ...
                            enrollments_to_update.append(existing_enrollment)
                            enrollment_errors.append(
                                f"Cleared invalid group assignment for student {student_reg_no} in course {course_code}"
                            )
            ...
            if enrollment_errors:
                ...
                if len(enrollment_errors) > 10:
                    logger.warning(
                        f"  ... and {len(enrollment_errors) - 10} more issues"
                    )
