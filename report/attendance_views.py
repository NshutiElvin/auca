...
def _build_attendance_pdf(timetable: MasterTimetable, student_exams) -> bytes:
    ...
    for line in timetable_lbl:
        story.append(
            Paragraph(line, _s("M", fontSize=9, alignment=TA_CENTER, leading=14))
        )
...
def _build_instructor_attendance_pdf(timetable: MasterTimetable, instructor, room, student_exams) -> bytes:
    ...
    # ── Improved header: each field on its own line ───────────────────────────
    meta_lines = [
        f"Campus: <b>{timetable.location.name.capitalize()}</b> &nbsp;|&nbsp; "
        f"Academic Year: <b>{timetable.academic_year}</b> &nbsp;|&nbsp; "
        f"Semester: <b>{timetable.semester.name.capitalize()}</b>",
        f"Category: <b>{timetable.category}</b> &nbsp;|&nbsp; "
        f"Instructor: <b>{instructor.get_full_name()}</b> &nbsp;|&nbsp; "
        f"Room: <b>{room.name}</b>",
    ]
    ...
    # ── Column definitions — includes Course column ───────────────────────────
    col_widths = [
        0.7 * cm,   # #
        2.0 * cm,   # Reg No
        3.5 * cm,   # Student Name
    ...
    for idx, se in enumerate(all_ses_flat, start=1):
        exam       = se.exam
        si         = se.signin_attendance
        so         = se.signout_attendance
        full_name  = (
        ...
        room_name = (
            se.room.name if se.room else (exam.room.name if exam and exam.room else "–")
        )
        ...
        if si:         grand["signed_in"]  += 1
        if so:         grand["signed_out"] += 1
        if not si:     grand["absent"]     += 1
        if has_report: grand["cheated"]    += 1
        grand["total"] += 1
    ...
    story.append(Spacer(1, 0.15 * cm))
    grand_data = [[
        Paragraph(f"<b>GRAND TOTAL: {grand['total']}</b>",
                  _s("G", fontSize=9)),
        Paragraph(f'<font color="#1E8449"><b>Signed In: {grand["signed_in"]}</b></font>',
                  _s("G", fontSize=9, alignment=TA_CENTER)),
        Paragraph(f'<font color="#C0392B"><b>Absent: {grand["absent"]}</b></font>',
                  _s("G", fontSize=9, alignment=TA_CENTER)),
        Paragraph(f'<font color="#CC6600"><b>Cheating: {grand["cheated"]}</b></font>',
                  _s("G", fontSize=9, alignment=TA_CENTER)),
        Paragraph(f"<b>Signed Out: {grand['signed_out']}</b>",
                  _s("G", fontSize=9, alignment=TA_RIGHT)),
    ]]
    grand_tbl = Table(grand_data, colWidths=["22%", "20%", "18%", "22%", "18%"])
