...
# action to update course group times that will be used by the timetable algorithm to update the course group times after generating the timetable
# this action will accept course_id and group_id inside the url and start_time and end_time inside the request body
    @action(detail=False, methods=["PUT"], url_path="update-course-group-times/(?P<course_id>[^/.]+)/(?P<group_id>[^/.]+)")
    def update_course_group_times(self, request, course_id, group_id, *args, **kwargs):
        try:
            ...
            return Response({
                "success": True,
                "message": "Course group times updated successfully"
            }, status=status.HTTP_200_OK)
            ...
            return Response({
                "success": False,
                "message": "Failed to update course group times"
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
